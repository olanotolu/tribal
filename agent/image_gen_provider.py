...
as ``kind: backend``) or ``~/.tribal/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tribal tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``tribal tools`` model picker.
...
def _images_cache_dir() -> Path:
    """Return ``$TRIBAL_HOME/cache/images/``, creating parents as needed."""
    from tribal_constants import get_tribal_home

    path = get_tribal_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TRIBAL_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$TRIBAL_HOME/cache/images/``.
