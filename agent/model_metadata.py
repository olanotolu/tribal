...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Tribal also honours TRIBAL_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("TRIBAL_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Tribal Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Tribal from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Tribal' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    return get_tribal_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Tribal grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from tribal_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Tribal sends to providers:
