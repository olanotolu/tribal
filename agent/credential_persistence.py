...
``auth.json``.  They intentionally have no dependency on ``tribal_cli.auth`` so
...
# Sources Tribal owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Tribal-owned OAuth/device-code state)
