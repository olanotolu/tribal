...
as ``kind: backend``) or ``~/.tribal/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``tribal tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$TRIBAL_HOME/cache/videos/``, creating parents as needed."""
    from tribal_constants import get_tribal_home

    path = get_tribal_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$TRIBAL_HOME/cache/videos/``.
