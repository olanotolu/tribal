...
Tribal pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.tribal/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<tribal_home>/bin/bws`` on
  first use.  Tribal pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.tribal/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``tribal`` invocations don't
  hammer the API.
* Failures NEVER block Tribal startup.  Missing binary, no network,
...
# In-process cache so repeated load_tribal_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `tribal chat -q ...`
...
# <tribal_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.tribal/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under tribal_home/cache/.

    `home_path` is what `load_tribal_dotenv()` already resolved; falling back
    to `$TRIBAL_HOME` / `~/.tribal` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("TRIBAL_HOME", Path.home() / ".tribal"))
...
def _tribal_bin_dir() -> Path:
    """Where Tribal stores its managed binaries.  Profile-aware."""
    from tribal_constants import get_tribal_home

    return get_tribal_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<tribal_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _tribal_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``tribal secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _tribal_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="tribal-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "tribal-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<tribal_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from tribal_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_tribal_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `tribal secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `tribal secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `tribal secrets bitwarden setup` to install."
        )
        return result
