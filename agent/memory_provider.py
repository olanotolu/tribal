...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - tribal_home (str): The active TRIBAL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.tribal``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "tribal").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'tribal memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], tribal_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'tribal memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``tribal_home`` is the active TRIBAL_HOME directory path.
