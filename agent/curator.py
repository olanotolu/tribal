...
def _state_file() -> Path:
    return get_tribal_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.tribal/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``tribal update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``tribal curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `tribal curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Tribal' background skill CURATOR. This is an "
    ...
    "into ~/.tribal/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: tribal-config-*, tribal-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.tribal/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.tribal/logs/curator/``)
    ...
    authored skill data in ``~/.tribal/skills/``.

    ``ensure_tribal_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_tribal_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every tribal install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: tribal curator status
        keep an umbrella stable: tribal curator pin document-tools
    ...
    lines.append("full report: tribal curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: tribal curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.tribal/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.tribal/skills/.archive/` for "
            "safety and can be restored via `tribal curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.tribal/skills/.archive/`. "
            "Restore any via `tribal curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `tribal curator restore <name>`")
    lines.append("- All archives live under `~/.tribal/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `tribal curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `tribal curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``tribal model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `tribal model` → auxiliary
