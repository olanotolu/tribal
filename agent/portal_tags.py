...
Every Tribal request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Tribal Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=tribal-agent",
        "client=tribal-client-v<__version__>",
    ]

The version is sourced live from ``tribal_cli.__version__`` so it auto-aligns
...
``tribal_cli.__version__`` is the canonical source of truth.
"""
...
def _tribal_version() -> str:
    """Return the current Tribal release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``tribal_cli`` cannot be imported (should
...
def tribal_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=tribal-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=tribal-client-v{_tribal_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=tribal-agent", tribal_client_tag()]
