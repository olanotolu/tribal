...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``TRIBAL_PLATFORM`` env var and
    ``TRIBAL_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("TRIBAL_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.tribal/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.tribal.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_tribal_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.tribal/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.tribal/skills/`` and any ``skills.external_dirs`` so the
