...
from tribal_constants import display_tribal_home as _dhh_fn, PARTIAL_STREAM_STUB_ID
from tribal_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Tribal tool use: "
    ...
    return (
        ...
        f"context, but Tribal needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Tribal config, set `model.ollama_num_ctx: 65536` "
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        from tribal_cli.auth import NOUS_INFERENCE_AUTH_MODE_LEGACY
...
def run_conversation(
    ...
    # ``tribal logs --session <id>`` can filter a single conversation.
    from tribal_logging import set_session_context
    ...
    # are reused.  The system prompt is Tribal's territory; plugins
    ...
    # all run inside Codex). Default Tribal path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Tribal internals.
        #
        # Tribal invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Tribal tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("TRIBAL_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: tribal auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `tribal doctor` for credential-chain diagnostics, or")
                    ...
                    from tribal_constants import display_tribal_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Tribal-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: tribal config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: tribal config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Tribal).  A 429 can
                ...
                # which Tribal' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Tribal' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`tribal",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `tribal auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `tribal model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: tribal auth add nous --type oauth", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: tribal setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        tribal fallback add   (interactive picker — same as `tribal model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Tribal/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `tribal fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("TRIBAL_KANBAN_TASK")
        if _kanban_task:
            try:
