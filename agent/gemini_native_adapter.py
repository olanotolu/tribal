...
Tribal keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Tribal's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Tribal)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Tribal typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Tribal retry/error classification."""
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.tribal/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `tribal setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "tribal-agent (gemini-native)",
        }
