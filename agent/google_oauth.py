...
Storage (``~/.tribal/auth/google_oauth.json``, chmod 0o600):
...
from tribal_constants import get_tribal_home, secure_parent_dir
...
#
# Resolution order:
#   1. TRIBAL_GEMINI_CLIENT_ID / TRIBAL_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "TRIBAL_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "TRIBAL_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "TRIBAL_HEADLESS")
...
def _credentials_path() -> Path:
    return get_tribal_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Tribal looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set TRIBAL_GEMINI_CLIENT_ID and TRIBAL_GEMINI_CLIENT_SECRET in ~/.tribal/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `tribal auth add google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Tribal — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Tribal — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Tribal will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#tribal"
