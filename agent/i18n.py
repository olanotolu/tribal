"""Lightweight internationalization (i18n) for Tribal static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Tribal itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``TRIBAL_LANGUAGE`` environment variable (for tests / quick override)
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a tribal dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`tribal_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("TRIBAL_LANGUAGE")
    if env_lang:
