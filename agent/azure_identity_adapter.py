...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Tribal does
...
class EntraIdentityConfig:
    ...
    Captures the Tribal-managed Entra knobs we need outside Azure SDK
    ...
    ``tribal_cli/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Tribal-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.tribal/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Tribal processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``tribal doctor`` /
    ``tribal auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``tribal doctor`` and the wizard preflight — never
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``tribal_cli/azure_detect.py``).
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `tribal doctor` or `az login` to recover.",
                exc,
            )
