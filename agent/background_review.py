...
See the ``tribal-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Tribal, e.g. 'tribal-agent').\n"
    "  • Hub-installed skills (installed via 'tribal skills install').\n"
    "Pinned skills (marked via 'tribal curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("TRIBAL_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Tribal' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Tribal
            ...
            # scratch (fresh _tribal_now() timestamp, fresh
