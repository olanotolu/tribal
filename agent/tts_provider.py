...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Tribal with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.tribal/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Tribal
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tribal tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``tribal setup`` for
