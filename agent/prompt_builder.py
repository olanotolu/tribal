...
from tribal_constants import get_tribal_home, get_skills_dir, is_wsl
...
_TRIBAL_MD_NAMES = (".tribal.md", "TRIBAL.md")


def _find_tribal_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.tribal.md`` or ``TRIBAL.md``.
...
TRIBAL_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Tribal Agent "
    "itself, load the `tribal-agent` skill with skill_view(name='tribal-agent') "
    "before answering. Docs: https://tribal.dev/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.tribal/kanban.db`. Your task id is in "
    "`$TRIBAL_KANBAN_TASK`; your workspace is `$TRIBAL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $TRIBAL_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `tribal kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Tribal WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Tribal itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Tribal runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Tribal itself is running. The host OS, home, and cwd "
                f"of the Tribal process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Tribal "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_tribal_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.tribal/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("TRIBAL_PLATFORM")
        or get_session_env("TRIBAL_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Tribal Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `tribal-agent` skill "
            "first. It has the actual commands (e.g. `tribal config set …`, `tribal tools`, "
            "`tribal setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: tribal setup, tribal setup tools, tribal setup terminal, tribal status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from TRIBAL_HOME and return its content, or None.
    ...
    """
    try:
        from tribal_cli.config import ensure_tribal_home
        ensure_tribal_home()
    except Exception as e:
        logger.debug("Could not ensure TRIBAL_HOME before loading SOUL.md: %s", e)
...
def _load_tribal_md(cwd_path: Path) -> str:
    """.tribal.md / TRIBAL.md — walk to git root."""
    tribal_md_path = _find_tribal_md(cwd_path)
    if not tribal_md_path:
        return ""
    try:
        content = tribal_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = tribal_md_path.name
        try:
            rel = str(tribal_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".tribal.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", tribal_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .tribal.md / TRIBAL.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TRIBAL_HOME is independent and always included when present.
    ...
    # SOUL.md from TRIBAL_HOME only — skip when already loaded as identity
    if not skip_soul:
