...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with tribal-agent)
2. User plugins: ``$TRIBAL_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$TRIBAL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$TRIBAL_HOME/plugins/model-providers/`` if it exists."""
    try:
        from tribal_constants import get_tribal_home

        d = get_tribal_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple TRIBAL_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_tribal_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$TRIBAL_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with tribal-agent.
    ...
    # 2. User plugins — under $TRIBAL_HOME/plugins/model-providers/<name>/.
