...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or tribal -c.",
    ...
    "Ctrl+Z suspends Tribal to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "tribal -c resumes your most recent CLI session. tribal -c \"project name\" resumes by title.",
    "tribal -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "tribal -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "tribal chat -t web,terminal enables only specific toolsets for a focused session.",
    "tribal chat -s github-pr-workflow preloads a skill at launch.",
    "tribal chat -q \"query\" runs a single non-interactive query and exits.",
    "tribal chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "tribal chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "tribal --yolo bypasses all dangerous command approval prompts for the entire session.",
    "tribal chat --source telegram tags the session for filtering in tribal sessions list.",
    "tribal -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "tribal doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "tribal dump outputs a compact setup summary — great for bug reports.",
    "tribal config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "tribal config edit opens config.yaml in your default editor.",
    "tribal config check scans for missing or stale configuration options.",
    "tribal sessions browse opens an interactive session picker with search.",
    "tribal sessions stats shows session counts by platform and database size.",
    "tribal sessions prune --older-than 30 cleans up old sessions.",
    "tribal skills search react --source skills-sh searches the skills.sh public directory.",
    "tribal skills check scans installed hub skills for upstream updates.",
    "tribal skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "tribal skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "tribal mcp add github --command npx adds MCP servers from the command line.",
    "tribal mcp serve runs Tribal itself as an MCP server for other agents.",
    "tribal auth add lets you add multiple API keys for credential pool rotation.",
    "tribal completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "tribal logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "tribal backup creates a zip backup of your entire Tribal home directory.",
    "tribal profile create coder creates an isolated profile that becomes its own command.",
    "tribal profile create work --clone copies your current config and keys to a new profile.",
    "tribal update syncs new bundled skills to ALL profiles automatically.",
    "tribal gateway install sets up Tribal as a system service (systemd/launchd).",
    "tribal memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "tribal webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: tribal tools disables unused tools, tribal skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "tribal models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as tribal -w).",
    ...
    "TRIBAL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Tribal tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'tribal profile create coder' creates the 'coder' command.",
    "tribal profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Tribal prints a resume command with session ID and stats.",
    "tribal sessions export backup.jsonl exports all sessions for backup or analysis.",
    "tribal -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "tribal skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .tribal/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: tribal cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Tribal runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "tribal gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Tribal via a local macOS server.",
    ...
    "Context files (.tribal.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.tribal/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Tribal loads project context from .tribal.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "tribal mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "tribal mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "tribal mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — tribal tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.tribal/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "tribal chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "tribal plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "tribal acp runs Tribal as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TRIBAL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "tribal auth add nous or tribal auth add openai-codex sets up OAuth-based providers.",
    ...
    "tribal status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.tribal/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Tribal your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.tribal/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Tribal auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.tribal/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at TRIBAL_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "TRIBAL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TRIBAL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.tribal/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'tribal profile create ls' would be rejected.",
    "tribal profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "tribal claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "TRIBAL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via tribal tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.tribal/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.tribal/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Tribal auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Tribal config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Tribal is working.',
    ...
    '/reload reloads ~/.tribal/.env into the running session — pick up new API keys without restarting.',
    ...
    "TRIBAL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.tribal/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.tribal/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'tribal curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "tribal curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'tribal curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'tribal auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'TRIBAL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TRIBAL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'tribal dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'tribal dashboard --tui embeds the full Tribal TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.tribal/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.tribal/dashboard-plugins/ — no npm build required.',
    ...
    'TRIBAL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'TRIBAL_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'TRIBAL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TRIBAL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.tribal/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.tribal/skills/ so drop-in skills appear without restarting the session.',
    ...
    'tribal -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'tribal chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'tribal chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'tribal chat --ignore-user-config skips ~/.tribal/config.yaml — reproducible bug reports and CI runs.',
    "tribal chat --source tool tags programmatic chats so they don't clutter tribal sessions list.",
    'tribal dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'tribal sessions rename <ID> "new title" renames any past session; tribal sessions delete <ID> removes one.',
    'tribal import restores a session export or profile archive produced by sessions export or profile export.',
    'tribal fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'tribal pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'tribal setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'tribal status --deep runs the full health sweep across every component; plain tribal status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'TRIBAL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TRIBAL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.tribal/plugins/ — trust-gated by design.',
    "TRIBAL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TRIBAL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TRIBAL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TRIBAL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TRIBAL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TRIBAL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TRIBAL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'TRIBAL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TRIBAL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TRIBAL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TRIBAL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Tribal block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from tribal sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.tribal/dashboard-plugins/.',
]
