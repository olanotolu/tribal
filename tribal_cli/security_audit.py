"""On-demand supply-chain audit for Tribal Agent installs.

Scans three surfaces a Tribal user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Tribal venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.tribal/plugins``
...
def _discover_plugins(tribal_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.tribal/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from tribal_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = tribal_home or Path(get_tribal_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, tribal_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(tribal_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `tribal security audit`."""
    home = Path(get_tribal_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, tribal_home=home
    )
    if total == 0:
