...
from tribal_cli.dashboard_auth.base import (
    InvalidCodeError,
    ProviderError,
)
from tribal_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from tribal_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``TRIBAL_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /tribal`` (Mission Control deploys) — we
    ...
    from tribal_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``tribal_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from tribal_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("tribal_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from tribal_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
