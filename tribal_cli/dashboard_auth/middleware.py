...
from tribal_cli.dashboard_auth.base import ProviderError
from tribal_cli.dashboard_auth.cookies import read_session_cookies
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /tribal``, the
    ``login_url`` is prefixed (``/tribal/login?next=...``) so the
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from tribal_cli.dashboard_auth.cookies import clear_session_cookies
        from tribal_cli.dashboard_auth.prefix import prefix_from_request
