...
Three cookies in play:
  - tribal_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL)
  - tribal_session_rt:   the OAuth refresh token
  ...
  - tribal_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/tribal) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/tribal`` covers
...
setter wrote ``__Secure-tribal_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "tribal_session_at"
SESSION_RT_COOKIE = "tribal_session_rt"
PKCE_COOKIE = "tribal_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /tribal`` we want ``Path=/tribal`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/tribal``)
