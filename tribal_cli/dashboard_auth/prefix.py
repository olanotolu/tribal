...
prefix (e.g. ``mission-control.tilos.com/tribal/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /tribal`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``TRIBAL_DASHBOARD_PUBLIC_URL`` /
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/tribal"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# TRIBAL_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``TRIBAL_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("TRIBAL_DASHBOARD_PUBLIC_URL", "")
