...
Profile-aware location: ``$TRIBAL_HOME/logs/dashboard-auth.log``.
...
from ``tribal_constants`` or other tribal_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$TRIBAL_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``tribal_constants.get_tribal_home`` semantics: env var wins,
    else ``~/.tribal``. A local copy avoids an import cycle with the
    middleware which lives below ``tribal_cli``.
    """
    home = os.environ.get("TRIBAL_HOME") or str(Path.home() / ".tribal")
