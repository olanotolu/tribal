"""CLI handlers for ``tribal secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``tribal_cli.main`` as part of building the top-level
    ``tribal secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Tribal process."
    )
    console.print(
        "  Status:  [cyan]tribal secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]tribal secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]tribal secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]tribal secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Tribal will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`tribal secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]tribal[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]tribal secrets bitwarden setup[/cyan] and pick EU or "
