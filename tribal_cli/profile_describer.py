...
Used by ``tribal profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``tribal_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Tribal Agent kanban board.
...
Rules:
  ...
  - Never write "Tribal Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_tribal_home())
        else:
