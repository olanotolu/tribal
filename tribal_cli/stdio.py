...
on Japanese locales, etc.).  Tribal's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``tribal_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Tribal is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TRIBAL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TRIBAL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Tribal-managed Git install directories so
    ...
    # already-running shells don't see the change — which means tribal
    ...
    # install without Tribal-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Tribal's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Tribal's
    ``tribal config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Tribal (User env var in Windows Settings, or
    export in a PowerShell profile) and Tribal picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Tribal-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\tribal\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``tribal`` from right after install) retain their old PATH.

    Any subprocess Tribal spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Tribal-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "tribal", "git", "cmd"),
        os.path.join(local_appdata, "tribal", "git", "bin"),
        os.path.join(local_appdata, "tribal", "git", "usr", "bin"),
        # Tribal venv Scripts directory — host of the tribal.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes tribal, but harmless to include.
        os.path.join(local_appdata, "tribal", "tribal-agent", "venv", "Scripts"),
