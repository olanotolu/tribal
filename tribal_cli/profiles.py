"""
Profile management for multiple isolated Tribal instances.

Each profile is a fully independent TRIBAL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.tribal/profiles/<name>/`` by default.

The "default" profile is ``~/.tribal`` itself — backward compatible,
zero migration needed.

Usage::

    tribal profile create coder          # fresh profile + bundled skills
    tribal profile create coder --clone  # also copy config, .env, SOUL.md, skills
    tribal profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    tribal -p coder chat                 # or via flag
    tribal profile use coder             # set as sticky default
    tribal profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See tribal_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.tribal``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   tribal-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `tribal profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `tribal
...
# `tribal skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Tribal infrastructure directories that only the default profile
         (``~/.tribal``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_tribal_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.tribal) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "tribal-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "tribal", "default", "test", "tmp", "root", "sudo",
})

# Tribal subcommands that cannot be used as profile names/aliases
_TRIBAL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the tribal root, NOT to the current TRIBAL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TRIBAL_HOME points outside
    ``~/.tribal``, profiles live under ``TRIBAL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_tribal_home() / "profiles"


def _get_default_tribal_home() -> Path:
    """Return the default (pre-profile) TRIBAL_HOME path.

    In standard deployments this is ``~/.tribal``.
    In Docker/custom deployments where TRIBAL_HOME is outside ``~/.tribal``
    (e.g. ``/opt/data``), returns TRIBAL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_tribal_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Tribal installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TRIBAL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_tribal_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, tribal subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _TRIBAL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a tribal subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``tribal profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Tribal config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_tribal_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``tribal update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.tribal)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.tribal/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `tribal update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`tribal profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `tribal update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `tribal profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `tribal -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TRIBAL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``tribal profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TRIBAL_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.tribal).\n"
            "To remove everything, use: tribal uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from tribal_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TRIBAL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("TRIBAL_HOME")
    try:
        os.environ["TRIBAL_HOME"] = str(profile_dir)
        from tribal_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["TRIBAL_HOME"] = old_home
        elif "TRIBAL_HOME" in os.environ:
            del os.environ["TRIBAL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.tribal/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: tribal profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TRIBAL_HOME.

    Returns ``"default"`` if TRIBAL_HOME is not set or points to ``~/.tribal``.
    Returns the profile name if TRIBAL_HOME points into ``~/.tribal/profiles/<name>``.
    Returns ``"custom"`` if TRIBAL_HOME is set to an unrecognized path.
    """
    from tribal_constants import get_tribal_home
    tribal_home = get_tribal_home()
    resolved = tribal_home.resolve()

    default_resolved = _get_default_tribal_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.tribal itself — its parent is ~/ and its
        # directory name is ".tribal", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: tribal profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.tribal itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.tribal). "
            "Specify a different name: tribal profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="tribal_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"tribal.{old_name}"
    new_host = f"tribal.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_tribal_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TRIBAL_HOME path string.

    Called early in the CLI entry point, before any tribal modules
    are imported, to set the TRIBAL_HOME environment variable.
    """
