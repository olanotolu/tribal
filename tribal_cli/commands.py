"""Slash command definitions and autocomplete for the Tribal CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Tribal config/state", "Session",
    ...
    CommandDef("tribe", "Ask the tribe; the council answers", "Tribal",
               ...
               cli_only=True),
    CommandDef("lore", "Inspect Tribal folklore and canon", "Tribal",
               args_hint="[list|show <id>]", subcommands=("list", "show"),
               cli_only=True),
    CommandDef("outcome", "Record confirming evidence for a lore lemma", "Tribal",
    ...
    CommandDef("busy", "Control what Enter does while Tribal is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.tribal/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Tribal Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`tribal_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/tribal`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Tribal
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``tribal skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``tribal skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``tribal skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/tribal <verb>`` subcommand.
    ...
    can still reach them via ``/tribal <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/tribal`` is always reserved as the first entry so the
    legacy ``/tribal <subcommand>`` form keeps working for anything that
    ...
    # Reserve /tribal as the catch-all top-level command.
    entries.append(("tribal", "Talk to Tribal or run a subcommand", "[subcommand] [args]"))
    seen.add("tribal")
...
def slack_app_manifest(request_url: str = "https://tribal-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /tribal handler.

    Maps both canonical names and aliases so /tribal bg do stuff works
    the same as /tribal background do stuff.

    Plugin-registered slash commands are included so ``/tribal <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from tribal_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
