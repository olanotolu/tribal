...
``<root>`` is the **shared Tribal root** (the parent of any active
...
``tribal -p <profile>`` joins the same board as the dispatcher that
...
the profile's Tribal home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``TRIBAL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``TRIBAL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``tribal kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.tribal``. In Docker / custom
deployments where ``TRIBAL_HOME`` points outside ``~/.tribal`` (e.g.
``/opt/tribal``), ``<root>`` is ``TRIBAL_HOME``. Legacy env-var
overrides still work:

* ``TRIBAL_KANBAN_DB`` — pin the database file path directly.
* ``TRIBAL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``TRIBAL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``TRIBAL_KANBAN_DB``,
``TRIBAL_KANBAN_WORKSPACES_ROOT``, and ``TRIBAL_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/tribal-kanban-v1-spec.pdf`` for the full
...
# ``TRIBAL_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``TRIBAL_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("TRIBAL_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``TRIBAL_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("TRIBAL_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``tribal-agent``
...
def kanban_home() -> Path:
    """Return the shared Tribal root that anchors the kanban board.

    Resolution order:

    1. ``TRIBAL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_tribal_root()``, which already returns ``<root>``
       when ``TRIBAL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``TRIBAL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``TRIBAL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("TRIBAL_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``tribal kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``TRIBAL_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``tribal kanban boards
    ...
    """
    env = os.environ.get("TRIBAL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``tribal kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``TRIBAL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("TRIBAL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``TRIBAL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("TRIBAL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``tribal kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``TRIBAL_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``TRIBAL_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("TRIBAL_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Tribal treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``TRIBAL_KANBAN_DB`` env → ``TRIBAL_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See tribal_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``tribal kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``TRIBAL_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from tribal_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``tribal --skills ...`` alongside the built-in
    ...
    # `tribal --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``tribal kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `tribal kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``tribal kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``tribal kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``tribal kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``tribal kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``TRIBAL_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Tribal' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Tribal-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``tribal kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Tribal
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``tribal kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Tribal profile.
    ...
    if not rows:
        return False
    try:
        from tribal_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Tribal profile.
        # `_default_spawn` invokes ``tribal -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Tribal
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `tribal kanban tail` — without
...
def _module_tribal_argv() -> list[str]:
    """Return the interpreter-bound Tribal CLI invocation."""
    # ``tribal_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``tribal`` package — there is no
    # ``tribal`` package to import.
    return [sys.executable, "-m", "tribal_cli.main"]


def _absolute_tribal_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Tribal shim."""
...
def _tribal_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Tribal executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_tribal_argv()
    return [_absolute_tribal_path(path)]


def _resolve_tribal_argv() -> list[str]:
    """Resolve the ``tribal`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$TRIBAL_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("tribal")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\tribal.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m tribal_cli.main`` — fallback for setups where
       Tribal is launched from a venv and the ``tribal`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_tribal_bin`` for the same reason. Kept
    local (not imported from gateway) because ``tribal_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("TRIBAL_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _tribal_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _tribal_path_argv(resolved_env_bin)
        return _module_tribal_argv()

    tribal_bin = _safe_which_no_cwd("tribal") if _IS_WINDOWS else shutil.which("tribal")
    if tribal_bin:
        return _tribal_path_argv(tribal_bin)
    return _module_tribal_argv()


def _kanban_worker_skill_available(tribal_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``tribal -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset TRIBAL_HOME means the worker falls back to the default root
    # home (``~/.tribal``), which ships the bundled skill.
    base = _Path(tribal_home) if tribal_home else (_Path.home() / ".tribal")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``tribal -p <profile> chat -q ...`` subprocess.
    ...
    ``TRIBAL_KANBAN_DB`` / ``TRIBAL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject TRIBAL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `tribal -p <name>` the
    # _apply_profile_override() runs *before* tribal_constants is imported.
    # If TRIBAL_HOME is absent from the child's env, get_tribal_home() falls
    # back to Path.home() / ".tribal" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from tribal_cli.profiles import resolve_profile_env
    try:
        env["TRIBAL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via TRIBAL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # TRIBAL_HOME never had profiles created.
        pass
    if task.tenant:
        env["TRIBAL_TENANT"] = task.tenant
    env["TRIBAL_KANBAN_TASK"] = task.id
    env["TRIBAL_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["TRIBAL_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["TRIBAL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["TRIBAL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`tribal -p <name>`
    # rewrites TRIBAL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_tribal_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["TRIBAL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["TRIBAL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["TRIBAL_KANBAN_BOARD"] = resolved_board
    # TRIBAL_PROFILE is the author the kanban_comment tool defaults to.
    # `tribal -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["TRIBAL_PROFILE"] = profile_arg

    cmd = [
        *_resolve_tribal_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped TRIBAL_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("TRIBAL_HOME")):
    ...
    # `tribal kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`tribal` executable not found on PATH. "
            "Install Tribal Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``tribal kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled TRIBAL_PROFILE values like "tribal-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Tribal root exists

    Reads profile paths directly so this module has no import dependency on
    ``tribal_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from tribal_constants import get_default_tribal_root
        default_root = get_default_tribal_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``tribal kanban assignees`` for the terminal.
