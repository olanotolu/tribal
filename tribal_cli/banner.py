...
Pure display functions with no TribalCLI state dependency.
"""
...
from tribal_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built tribal — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Tribal/tribal.git"
...
def _fetch_pypi_latest(package: str = "tribal-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Tribal update is available.

    Two paths: if ``TRIBAL_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = tribal_home / ".update_check"
    embedded_rev = os.environ.get("TRIBAL_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $TRIBAL_HOME/tribal-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = tribal_home / "tribal-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Tribal git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$TRIBAL_HOME/tribal-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``tribal_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from tribal_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/Tribal/tribal/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Tribal checkout. Cached per-process. Release URL always points at the
    canonical Tribal/tribal repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Tribal Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else TRIBAL_CADUCEUS
    except Exception:
        _bskin = None
        _hero = TRIBAL_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Tribal Genesis[/]")

    if os.getenv("TRIBAL_YOLO_MODE"):
    ...
    try:
        from tribal_cli.codex_runtime_switch import get_current_runtime
        from tribal_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from tribal_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built tribal; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else TRIBAL_AGENT_LOGO
        console.print(_logo)
        console.print()
