"""
Status command for tribal CLI.

Shows the status of all Tribal Agent components.
"""
...
from tribal_cli.auth import AuthError, resolve_provider
from tribal_cli.colors import Colors, color
from tribal_cli.config import get_env_path, get_env_value, get_tribal_home, load_config
from tribal_cli.models import provider_label
from tribal_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from tribal_cli.nous_subscription import get_nous_subscription_features
from tribal_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``tribal config``'s
...
def show_status(args):
    """Show status of all Tribal Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Tribal Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: tribal auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: tribal model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: tribal auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: tribal auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: tribal model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from tribal_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from tribal_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   tribal gateway")
    ...
    jobs_file = get_tribal_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_tribal_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'tribal doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'tribal setup' to configure", Colors.DIM))
    print()
