...
- ``tribal_cli.providers``        -- canonical provider identity + overlays
- ``tribal_cli.model_normalize``  -- per-provider name formatting
...
_TRIBAL_MODEL_WARNING = (
    "Nous Research Tribal 3 & 4 models are NOT agentic and are not designed "
    "for use with Tribal Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Tribal 3 / Tribal 4 chat families.
# The previous substring check (`"tribal" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``tribal-brain:qwen3-14b-ctx16k`` that just
# happen to carry "tribal" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Tribal-3-Llama-3.1-70B, tribal-4-405b, openrouter/tribal3:70b
# Negative examples it must NOT match:
#   tribal-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_TRIBAL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])tribal[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_tribal_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Tribal 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_TRIBAL_NON_AGENTIC_RE.search(model_name))


def _check_tribal_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Tribal 3/4 chat model."""
    if is_nous_tribal_non_agentic(model_name):
        return _TRIBAL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``tribal config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from tribal_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'tribal model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'tribal doctor' — config issues detected:"
    ...
    # Mirrors the same logic in tribal_cli.runtime_provider.resolve_runtime_provider;
    ...
    tribal_warn = _check_tribal_model_warning(new_model)
    if tribal_warn:
        warnings.append(tribal_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from tribal_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from tribal_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by tribal provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://tribal.dev/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Tribal release. Falls back to the in-repo
    ...
    # --- 1. Check Tribal-mapped providers ---
    for tribal_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(tribal_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TRIBAL_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from tribal_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(tribal_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `tribal model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(tribal_id)
        if not model_ids:
            model_ids = curated.get(tribal_id, [])
            if tribal_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(tribal_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Tribal-only providers (nous, openai-codex, copilot, opencode-go) ---
    from tribal_cli.providers import TRIBAL_OVERLAYS
    from tribal_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Tribal provider ID.
    # TRIBAL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Tribal IDs ("copilot").
    _mdev_to_tribal = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TRIBAL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Tribal slug — e.g. "github-copilot" → "copilot"
        tribal_slug = _mdev_to_tribal.get(pid, pid)
        if tribal_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(tribal_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, tribal_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or tribal_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(tribal_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", tribal_slug, exc)
        ...
        # configured.
        if not has_creds and tribal_slug == "anthropic":
            try:
                ...
                if (tribal_creds and tribal_creds.get("accessToken")) or \
        ...
        if tribal_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(tribal_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(tribal_slug)
                model_ids = _ids if _ids else (curated.get(tribal_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(tribal_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(tribal_slug, []) or curated.get(pid, [])
                if tribal_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(tribal_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": tribal_slug,
            "name": get_label(tribal_slug),
            "is_current": tribal_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "tribal",
        ...
        seen_slugs.add(tribal_slug.lower())
        _record_builtin_endpoint(tribal_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TRIBAL_OVERLAYS (keeps /model in sync
    # with `tribal model`).
    try:
        from tribal_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Tribal's canonical write key (matches
            ...
            # Tribal writes ``models:`` as a dict keyed by model id
            # (see tribal_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Tribal's own writer uses; a hyphen variant is accepted
            ...
            # active model. Tribal's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, tribal_cli/config.py) already consume that dict.
        ...
        for grp_key, grp in groups.items():
            ...
            local_endpoint = (
                "localhost" in api_url_lower
                or "127.0.0.1" in api_url_lower
                or "[::1]" in api_url_lower
            )
            has_explicit_models = bool(grp["models"])
            should_probe = bool(api_url) and (
                bool(api_key) or not has_explicit_models
            ) and not (local_endpoint and has_explicit_models)
            if should_probe:
                try:
                    from tribal_cli.models import fetch_api_models
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`tribal_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from tribal_cli.models import fetch_openrouter_models
