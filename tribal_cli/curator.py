"""CLI subcommand: `tribal curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `tribal curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`tribal curator status` and run `tribal curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`tribal curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.tribal/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`tribal curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.tribal/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from tribal_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.tribal/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.tribal/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by tribal_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="tribal curator")
    register_cli(parser)
