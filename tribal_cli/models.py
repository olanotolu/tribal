...
Add, remove, or reorder entries here — both `tribal setup` and
`tribal` provider-selection will pick up the change automatically.
"""
...
_TRIBAL_USER_AGENT = f"tribal-cli/{_TRIBAL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `tribal model`
    flow without maintaining a separate static list.
    """
    from tribal_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $TRIBAL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $TRIBAL_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Tribal refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Tribal that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Tribal that doesn't ship
...
# this list:  tribal model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `tribal model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `tribal model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``tribal model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``tribal auth add openai-codex`` without
    ``tribal model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    tribal-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from tribal_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — tribal-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from tribal_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from tribal_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``tribal model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from tribal_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from tribal_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Tribal' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``tribal_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Tribal should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``tribal auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.tribal/.env``.
    ...
    """
    try:
        from tribal_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from tribal_cli.auth import read_credential_pool
        from tribal_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Tribal release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Tribal release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from tribal_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Tribal release.
...
#
# Cache strategy:
#   - One JSON file at $TRIBAL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_tribal_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$TRIBAL_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _TRIBAL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Tribal' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _TRIBAL_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from tribal_constants import get_tribal_home
    return get_tribal_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Tribal will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Tribal cannot verify the model name."
