"""tribal claw — OpenClaw migration commands.

Usage:
    tribal claw migrate              # Preview then migrate (always shows preview first)
    tribal claw migrate --dry-run    # Preview only, no changes
    tribal claw migrate --yes        # Skip confirmation prompt
    tribal claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    tribal claw migrate --no-backup  # Skip pre-migration snapshot
    tribal claw cleanup              # Archive leftover OpenClaw directories
    tribal claw cleanup --dry-run    # Preview what would be archived
"""
...
from tribal_cli.config import get_tribal_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Tribal may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Tribal gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Tribal gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'tribal stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_tribal.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_tribal", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: tribal claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Tribal")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'tribal claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Tribal migration."""
    ...
    # migrate-tribal posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: tribal claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {tribal_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.tribal/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: tribal claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Tribal home ─────────
    # Delegates to tribal_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `tribal import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from tribal_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(tribal_home=tribal_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: tribal import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Tribal home."
            )
    ...
    # 'tribal claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: tribal claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: tribal claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  tribal claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  tribal claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  tribal config set OPENROUTER_API_KEY sk-or-v1-...")
