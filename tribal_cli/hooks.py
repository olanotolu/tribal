"""tribal hooks — inspect and manage shell-script hooks.

Usage::

    tribal hooks list
    tribal hooks test <event> [--for-tool X] [--payload-file F]
    tribal hooks revoke <command>
    tribal hooks doctor

Consent records live under ``~/.tribal/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.tribal/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``tribal hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: tribal hooks {list|test|revoke|doctor}")
        print("Run 'tribal hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.tribal/config.yaml.")
        print("See `tribal hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `tribal hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `tribal hooks test` and `tribal hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Tribal wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `tribal hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `tribal hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `tribal hooks doctor`.")
