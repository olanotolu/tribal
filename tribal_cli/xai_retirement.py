...
Pure logic: walks a Tribal config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `tribal doctor` and a future `tribal migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Tribal config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Tribal config and return retirement issues.
