"""
Configuration management for Tribal Agent.

Config files are stored in ~/.tribal/ for easy access:
- ~/.tribal/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.tribal/.env         - API keys and secrets

This module provides:
- tribal config          - Show current configuration
- tribal config edit     - Open config in editor
- tribal config set      - Set a specific value
- tribal config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.tribal/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``tribal logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  tribal config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Tribal makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Tribal itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``tribal update``, the TUI build.
...
#   on every plugin install / ``tribal update``.
...
# * ``TRIBAL_HOME`` / ``TRIBAL_PROFILE`` / ``TRIBAL_CONFIG`` /
#   ``TRIBAL_ENV`` — Tribal runtime location flags. Writing these into
...
#
# IMPORTANT: ``TRIBAL_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (TRIBAL_GEMINI_CLIENT_ID,
# TRIBAL_LANGFUSE_PUBLIC_KEY, TRIBAL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Tribal runtime location — never via dashboard env writer.
    # NOT a TRIBAL_* blanket: integration credentials (TRIBAL_GEMINI_*,
    # TRIBAL_LANGFUSE_*, TRIBAL_SPOTIFY_*, ...) ARE allowed.
    "TRIBAL_HOME", "TRIBAL_PROFILE", "TRIBAL_CONFIG", "TRIBAL_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Tribal runtime location "
            "(TRIBAL_HOME, TRIBAL_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `tribal plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "TRIBAL_LANGFUSE_ENV",
    "TRIBAL_LANGFUSE_RELEASE",
    "TRIBAL_LANGFUSE_SAMPLE_RATE",
    "TRIBAL_LANGFUSE_MAX_CHARS",
    "TRIBAL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("TRIBAL_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_tribal_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Tribal is running in package-manager-managed mode.

    Two signals: the TRIBAL_MANAGED env var (set by the systemd service),
    or a .managed marker file in TRIBAL_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade tribal-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Tribal was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.tribal/.install_method`` file (written by installers)
    2. TRIBAL_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_tribal_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.tribal/.install_method."""
    stamp = get_tribal_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade tribal-agent"
    if method == "docker":
        return "docker pull nousresearch/tribal-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade tribal-agent"
        return "pip install --upgrade tribal-agent"
    return "tribal update"
...
# Long-form text for ``tribal update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# tribal_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Tribal, it doesn't update
...
✗ ``tribal update`` doesn't apply inside the Docker container.

Tribal Agent runs as a published image (nousresearch/tribal-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/tribal-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate tribal-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/tribal-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/tribal-agent/tags
  • Your config and session history live under ``$TRIBAL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``tribal update`` inside Docker.
...
def format_managed_message(action: str = "modify this Tribal installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("TRIBAL_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Tribal installation is managed by NixOS "
            f"(TRIBAL_MANAGED={env_hint}).\n"
            "Edit services.tribal-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Tribal installation is managed by Homebrew "
            f"(TRIBAL_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade tribal-agent"
        )

    return (
        f"Cannot {action}: this Tribal installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Tribal."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from TRIBAL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, tribal_bin
    or None if container mode is not active, we're already inside the
    container, or TRIBAL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("TRIBAL_DEV") == "1":
        return None
    ...
    container_mode_file = get_tribal_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "tribal-agent")
    exec_user = info.get("exec_user", "tribal")
    tribal_bin = info.get("tribal_bin", "/data/current-package/bin/tribal")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "tribal_bin": tribal_bin,
    }
...
# Re-export from tribal_constants — canonical definition lives there.
from tribal_constants import get_tribal_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_tribal_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_tribal_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the tribal group can
    share state with the gateway service.

    The mode can be overridden via the TRIBAL_HOME_MODE environment variable
    (e.g. TRIBAL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse TRIBAL_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("TRIBAL_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Tribal runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("TRIBAL_CONTAINER") or os.environ.get("TRIBAL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set TRIBAL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into TRIBAL_HOME if the user doesn't have one yet."""
...
def ensure_tribal_home():
    """Ensure ~/.tribal directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_tribal_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_tribal_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"TRIBAL_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Tribal-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Tribal auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Tribal sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Tribal
        ...
        # bundled Tribal image, which drops to the `tribal` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Tribal sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Tribal should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``tribal chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.tribal/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: tribal sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Tribal truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``tribal kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``tribal kanban decompose`` and the kanban
        ...
        # ``tribal profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `tribal model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `tribal --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `tribal -c` muscle memory.  Default off so existing
        # users aren't surprised.  TRIBAL_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/tribal`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``TRIBAL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``TRIBAL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # Public URL override (env: ``TRIBAL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/tribal``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.tribal/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.tribal/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Tribal feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Tribal auto-pauses the goal and
    ...
    # always goes to ~/.tribal/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${TRIBAL_SKILL_DIR} and ${TRIBAL_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `tribal curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.tribal/skills/ into
        # ~/.tribal/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `tribal curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for tribal-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Tribal Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (TRIBAL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.tribal/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TRIBAL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `tribal doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``tribal_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Tribal to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via TRIBAL_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `tribal -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `tribal` launches with
        ...
        # decomposition is manual via `tribal kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     tribal-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.tribal/logs/.
    ...
    # update model picker lists without shipping a tribal-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://tribal.dev/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `tribal model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.tribal/.env,
        ...
        # behavior: files must live under the Tribal cache, under
        ...
        # user. Bridged to TRIBAL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Tribal
        # cache (~/.tribal/cache/{documents,images,audio,video,screenshots})
        ...
        # to TRIBAL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # TRIBAL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to TRIBAL_MEDIA_TRUST_RECENT_SECONDS.
        # Only consulted when ``strict`` is true.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.tribal/state.db.
    ...
    "sessions": {
        ...
        # default of ``tribal sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.tribal/sessions/session_{sid}.json`` on every turn
    ...
    # ``tribal update`` behaviour.
    "updates": {
        # Run a full ``tribal backup``-style zip of TRIBAL_HOME before every
        # ``tribal update``.  Backups land in ``<TRIBAL_HOME>/backups/`` and
        # can be restored with ``tribal import <path>``.  Off by default —
        # on large TRIBAL_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<TRIBAL_HOME>/lsp/bin/`` on first use.
    ...
    # `tribal tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.tribal/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.tribal/.env (or your shell) and never in config.yaml.
            ...
            # ~/.tribal/bin/ on first use.  When False you must install
            ...
            # `tribal secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'tribal model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Tribal auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @tribal:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. TRIBAL_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #tribal)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: tribal-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'tribal-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Tribal API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Tribal API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Tribal API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # TRIBAL_TOOL_PROGRESS and TRIBAL_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "TRIBAL_TOOL_PROGRESS": {
    ...
    "TRIBAL_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.tribal.config`` entries, then checks
    ...
    try:
        ...
        # should never break `tribal update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Tribal versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``tribal_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'tribal setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Tribal won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'tribal doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("TRIBAL_HOME", "~/.tribal")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("TRIBAL_TOOL_PROGRESS")
            old_mode = get_env_value("TRIBAL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from TRIBAL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from TRIBAL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("TRIBAL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from TRIBAL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$TRIBAL_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_tribal_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `tribal plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `tribal curator status` has no
    ...
    #   3. Creates `~/.tribal/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_tribal_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_tribal_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: tribal config set <key> <value>")
    ...
    # metadata.tribal.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _tribal_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.tribal/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.tribal/config.yaml.
    ...
    (which change ``TRIBAL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — tribal auth) — OpenAI Codex
#   nous         (OAuth — tribal auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.tribal/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.tribal/.env.
    ...
    interactive menu render (`tribal tools`, `tribal setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `tribal tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Tribal env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.tribal/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.tribal/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.tribal/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.tribal/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Tribal — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Tribal vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.tribal/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Tribal Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  tribal config edit     # Edit config file", Colors.DIM))
    print(color("  tribal config set <key> <value>", Colors.DIM))
    print(color("  tribal setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: tribal config set <key> <value>")
            print()
            print("Examples:")
            print("  tribal config set model anthropic/claude-sonnet-4")
            print("  tribal config set terminal.backend docker")
            print("  tribal config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'tribal config migrate' to add them")
        ...
        print("  tribal config           Show current configuration")
        print("  tribal config edit      Open config in editor")
        print("  tribal config set <key> <value>   Set a config value")
        print("  tribal config check     Check for missing/outdated config")
        print("  tribal config migrate   Update config with new options")
        print("  tribal config path      Show config file path")
        print("  tribal config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``tribal config`` UI so users
