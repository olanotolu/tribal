"""
Baked-in build metadata for Tribal Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``tribal_cli/dump.py`` and ``tribal_cli/banner.py``).  That doesn't work inside
...
To make ``tribal dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$TRIBAL_GIT_SHA``
arg into ``<project_root>/.tribal_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``TRIBAL_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".tribal_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.tribal_build_sha`` if present.  The file is
    written by the Dockerfile's ``TRIBAL_GIT_SHA`` build-arg and contains
