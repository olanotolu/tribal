...
entries via ``tribal mcp catalog`` or the interactive ``tribal mcp picker``,
and install them with ``tribal mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into tribal-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``tribal mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.tribal/.env`` (the
...
from tribal_constants import get_tribal_home, get_optional_mcps_dir
...
from tribal_cli.cli_output import prompt as _prompt_input, prompt_yes_no
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Tribal install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Tribal understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Tribal is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Tribal
        understands. Update Tribal to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_tribal_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.tribal/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.tribal/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from tribal_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``tribal mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `tribal mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `tribal mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `tribal mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `tribal mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `tribal auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`tribal auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Tribal session to load its tools.",
        Colors.GREEN,
    ))
