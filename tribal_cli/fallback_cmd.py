"""
tribal fallback — manage the fallback provider chain.
...
Subcommands:
  tribal fallback [list]   Show the current fallback chain (default when no subcommand)
  tribal fallback add      Pick provider + model via the same picker as `tribal model`,
                           then append the selection to the chain
  tribal fallback remove   Pick an entry to delete from the chain
  tribal fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.tribal/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from tribal_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `tribal model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  tribal fallback add")
        print()
        return
    ...
    print("  Docs: https://tribal.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `tribal model`, then append the selection to the chain."""
    from tribal_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `tribal model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `tribal fallback list` to view, or `tribal fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``tribal fallback [subcommand]``."""
