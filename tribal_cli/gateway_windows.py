...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m tribal_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Tribal_Gateway"
_TASK_DESCRIPTION = "Tribal Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Tribal profile."""
    from tribal_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "tribal_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("TRIBAL_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("TRIBAL_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("TRIBAL_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["TRIBAL_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["TRIBAL_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["TRIBAL_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("TRIBAL_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("TRIBAL_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("TRIBAL_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Tribal_Gateway``
    Named profile X: ``Tribal_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during tribal_cli boot.
    from tribal_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\tribal\\gateway-service\\<task_name>.cmd``
    (or ``<TRIBAL_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Tribal installs stay self-contained).
    """
    _assert_windows()
    from tribal_cli.config import get_tribal_home

    script_dir = Path(get_tribal_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports TRIBAL_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m tribal_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "TRIBAL_HOME={tribal_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "TRIBAL_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports tribal_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "tribal_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    tribal_home = str(Path(get_tribal_home()).resolve())
    profile_arg = _profile_arg(tribal_home)

    content = _build_gateway_cmd_script(python_path, working_dir, tribal_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Tribal builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "tribal_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m tribal_cli.main gateway run``
    ...
    log_dir = Path(get_tribal_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("TRIBAL_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("TRIBAL_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from tribal_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `tribal -p <profile> gateway install` must be safe.
    ...
    from tribal_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"tribal {profile_arg} gateway start" if profile_arg else "tribal gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: tribal gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Tribal gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: tribal gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: tribal gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from tribal_cli.config import get_tribal_home
        print(f"    type {Path(get_tribal_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_tribal_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from tribal_cli.config import get_tribal_home

    tribal_home = Path(get_tribal_home()).resolve()
    print()
    print("Next steps:")
    print("  tribal gateway status                      # Check status")
    print(f"  type {tribal_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Tribal gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: tribal gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: tribal gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: tribal gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from tribal_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
