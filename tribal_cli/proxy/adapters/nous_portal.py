...
Reads the user's Nous OAuth state from ``~/.tribal/auth.json`` through the
...
:func:`tribal_cli.auth.resolve_nous_runtime_credentials`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `tribal auth add nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `tribal auth add nous` to re-authenticate."
                )
    ...
    # to tribal_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
