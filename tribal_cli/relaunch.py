"""
Unified self-relaunch for Tribal CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``tribal sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``tribal`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from tribal_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched tribal."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_tribal_bin() -> Optional[str]:
    """Find the tribal entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("tribal")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m tribal_cli.main``.
    ...
    ``tribal.exe`` on PATH or the ``sys.executable -m tribal_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with tribal.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_tribal_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "tribal_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh tribal invocation.
    ...
    relaunch contract wants: "run tribal again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``tribal.exe`` (a Python console-script shim that wraps
    ``python -m tribal_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "tribal exited, then
    new tribal started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``tribal`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nTribal relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run tribal.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
