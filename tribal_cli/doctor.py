"""
Doctor command for tribal CLI.

Diagnoses issues with Tribal Agent setup.
"""
...
from tribal_cli.config import get_project_root, get_tribal_home, get_env_path
...
_DHH = display_tribal_home()  # user-facing display path (e.g. ~/.tribal or ~/.tribal/profiles/coder)

# Load environment variables from ~/.tribal/.env so API key checks work
_env_path = get_env_path()
load_tribal_dotenv(tribal_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from tribal_cli.colors import Colors, color
from tribal_cli.models import _TRIBAL_USER_AGENT
from tribal_constants import OPENROUTER_MODELS_URL
...
from tribal_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.tribal/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("TRIBAL_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from tribal_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from tribal_cli.auth import get_minimax_oauth_auth_status
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-tribal and dashboard static services are up
    ...
    for static in ("main-tribal", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `tribal profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from tribal_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `tribal`.
    os.environ.setdefault("TRIBAL_INTERACTIVE", "1")

    # Handle `tribal doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.tribal/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Tribal Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`tribal doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.tribal/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'tribal setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'tribal setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'tribal setup' to create one")
                issues.append("Run 'tribal setup' to create .env")
    
    # Check ~/.tribal/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from tribal_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from tribal_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'tribal config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from tribal_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.tribal/.env or run 'tribal setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'tribal setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'tribal config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from tribal_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from tribal_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'tribal setup' to migrate config")
                else:
                    issues.append("Run 'tribal doctor --fix' or 'tribal setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'tribal doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Tribal' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = tribal_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Tribal a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Tribal Agent Persona\n\n"
                "<!-- Edit this file to customize how Tribal communicates. -->\n\n"
                "You are Tribal, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = tribal_home / "memories"
    ...
    state_db_path = tribal_home / "state.db"
    ...
    wal_path = tribal_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'tribal doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "tribal"
        ...
        _cmd_link = _cmd_link_dir / "tribal"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(tribal not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/tribal → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/tribal points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/tribal → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/tribal — run 'tribal doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/tribal exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/tribal not found",
                    "(tribal command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/tribal → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/tribal symlink — run 'tribal doctor --fix'")
    ...
    try:
        from tribal_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `tribal doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'tribal config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``tribal doctor``.
    ...
    # never the legitimate source for `tribal doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'tribal setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = TRIBAL_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: tribal skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = TRIBAL_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: tribal memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: tribal memory setup",
                    "No Honcho API key — run 'tribal memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run tribal memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: tribal memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: tribal memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from tribal_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "tribal -p" in content:
                            _m = _re.search(r"tribal -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'tribal doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
