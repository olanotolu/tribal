"""
Unified tool configuration for Tribal Agent.

`tribal tools` and `tribal setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.tribal/config.yaml under
...
from tribal_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from tribal_cli.nous_account import format_nous_portal_entitlement_message
...
from tribal_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _TRIBAL_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `tribal tools` → Video Generation, which walks
...
# `tribal tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from tribal_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `tribal tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``tribal tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from tribal_cli.plugins import discover_plugins, get_plugin_toolsets
...
from tribal_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Tribal routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. TRIBAL_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("TRIBAL_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``tribal update`` and
      by ``tribal computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `tribal update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Tribal process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_tribal_home()}/tribal-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in tribal-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/tribal-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `tribal update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.tribal/config.yaml")
        ...
        # Run the full `tribal auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.tribal/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from tribal_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: tribal auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `tribal auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: tribal auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: tribal auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `tribal auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: tribal auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: tribal auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "tribal-cli" (which include all _TRIBAL_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[tribal-cli, spotify]`` after enabling Spotify via ``tribal
        ...
        # (e.g. "tribal-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``tribal tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `tribal tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"tribal-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("tribal-")}
    ...
    # shipped as a bundled plugin but user must opt in via `tribal tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `tribal tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by tribal tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (tribal-cli, tribal-telegram, etc.)
    ...
    # Opening `tribal tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `tribal tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `tribal tools` and `tribal setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Tribal Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://tribal.dev/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from tribal_constants import display_tribal_home
    print(color(f"  Tool configuration saved to {display_tribal_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'tribal' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, tribal mcp
