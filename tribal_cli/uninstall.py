"""
Tribal Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.tribal/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Tribal PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing tribal-agent or tribal PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Tribal Agent" comment and following line
                if '# Tribal Agent' in line or '# tribal-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('tribal' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing tribal
                if 'tribal' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the tribal wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "tribal",
        Path("/usr/local/bin/tribal"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains tribal_cli reference)
                content = wrapper.read_text()
                if 'tribal_cli' in content or 'tribal-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``tribal gateway run`` processes
    ...
    try:
        from tribal_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from tribal_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``TRIBAL_HOME`` and ``TRIBAL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\tribal\git\cmd``, ``%LOCALAPPDATA%\tribal\git\bin``,
#      ``%LOCALAPPDATA%\tribal\git\usr\bin``, ``%LOCALAPPDATA%\tribal\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\tribal\git\`` and Node to
#      ``%LOCALAPPDATA%\tribal\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``tribal dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\tribal\gateway-service\`` and sometimes
...
def _tribal_path_markers(tribal_home: Path) -> list[str]:
    """Path-entry substrings that identify Tribal-owned User-PATH entries."""
    root = str(tribal_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare tribal-agent install dir.
    markers = [root + "\\tribal-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if TRIBAL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "tribal".  We don't want to catch
    # unrelated entries like "ctribal-foo" or "ephermeral", so we look for
    # backslash-tribal as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(tribal_home: Path) -> list[str]:
    """Strip Tribal-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _tribal_path_markers(tribal_home)
            kept: list[str] = []
...
def remove_tribal_env_vars_windows() -> list[str]:
    """Delete TRIBAL_HOME and TRIBAL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("TRIBAL_HOME", "TRIBAL_GIT_BASH_PATH"):
                try:
...
def _is_default_tribal_home(tribal_home: Path) -> bool:
    """Return True when ``tribal_home`` points at the default (non-profile) root."""
    try:
        from tribal_constants import get_default_tribal_root
        return tribal_home.resolve() == get_default_tribal_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TRIBAL_HOME directory.

    We shell out to ``tribal -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TRIBAL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m tribal_cli.main` so we don't depend on a `tribal`
    #    wrapper that may be half-removed mid-uninstall.
    tribal_invocation = [_sys.executable, "-m", "tribal_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                tribal_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's TRIBAL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.tribal/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.tribal/ for future reinstall
    """
    project_root = get_project_root()
    tribal_home = get_tribal_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie TRIBAL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_tribal_home(tribal_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Tribal Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {tribal_home / 'config.yaml'}")
    print(f"  Secrets: {tribal_home / '.env'}")
    print(f"  Data:    {tribal_home / 'cron/'}, {tribal_home / 'sessions/'}, {tribal_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TRIBAL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Tribal data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Tribal code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in tribal_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\tribal\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(tribal_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Tribal-owned PATH entries in User environment")

        log_info("Removing TRIBAL_HOME / TRIBAL_GIT_BASH_PATH User env vars...")
        removed_env = remove_tribal_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Tribal-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing tribal command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.tribal/, just remove the tribal-agent subdir
            if tribal_home in project_root.parents or project_root.parent == tribal_home:
    ...
    #     under TRIBAL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(tribal_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(tribal_home)
    ...
    # 5. Optionally remove ~/.tribal/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TRIBAL_HOME dirs live under
        ...
        try:
            if tribal_home.exists():
                shutil.rmtree(tribal_home)
                log_success(f"Removed {tribal_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {tribal_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {tribal_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Tribal/tribal/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Tribal/tribal/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Tribal Agent! ⚕")
    print()
