"""
Tribal Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m tribal_cli.main web          # Start on http://127.0.0.1:9119
    python -m tribal_cli.main web --port 8080
"""
...
try:
    ...
    # running `tribal dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["TRIBAL_WEB_DIST"]) if "TRIBAL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Tribal Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Tribal-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``tribal dashboard --tui``
# or TRIBAL_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from tribal_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``tribal status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from tribal_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.tribal/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_tribal_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "tribal-update": "tribal-update.log",
}
...
def _spawn_tribal_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``tribal <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``tribal_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "tribal_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "TRIBAL_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``tribal gateway restart`` in the background."""
    try:
        proc = _spawn_tribal_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/tribal/update")
async def update_tribal():
    """Kick off ``tribal update`` in the background."""
    try:
        proc = _spawn_tribal_action(["update"], "tribal-update")
    except Exception as exc:
        _log.exception("Failed to spawn tribal update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Tribal supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in tribal_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.tribal/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``tribal auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Tribal resolves Anthropic creds in this order at runtime:
    1. ``~/.tribal/.anthropic_oauth.json`` — Tribal-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_tribal_oauth_credentials,
            read_claude_code_credentials,
            _TRIBAL_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_tribal_oauth_credentials = None  # type: ignore
        _TRIBAL_OAUTH_FILE = None  # type: ignore

    tribal_creds = None
    if read_tribal_oauth_credentials:
        try:
            tribal_creds = read_tribal_oauth_credentials()
        except Exception:
            tribal_creds = None
    if tribal_creds and tribal_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "tribal_pkce",
            "source_label": f"Tribal PKCE ({_TRIBAL_OAUTH_FILE})",
            "token_preview": _truncate_token(tribal_creds.get("accessToken")),
            "expires_at": tribal_creds.get("expiresAt"),
            "has_refresh_token": bool(tribal_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Tribal even
    when they also have a separate Tribal-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "tribal auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "tribal auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "tribal auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "tribal auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "tribal auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from tribal_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("tribal_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Tribal-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _TRIBAL_OAUTH_FILE
            if _TRIBAL_OAUTH_FILE.exists():
                _TRIBAL_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from tribal_cli.auth import clear_provider_auth
...
#          → persists to ~/.tribal/.anthropic_oauth.json AND credential pool
...
# Guarded so tribal web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Tribal file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``tribal auth add anthropic``.
    """
    from agent.anthropic_adapter import _TRIBAL_OAUTH_FILE
    payload = {
    ...
    _TRIBAL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _TRIBAL_OAUTH_FILE.with_name(
        f"{_TRIBAL_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _TRIBAL_OAUTH_FILE)
        try:
            _TRIBAL_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "tribal-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("TRIBAL_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from tribal_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from tribal_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_tribal_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from tribal_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from tribal_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, TRIBAL_HOME)."""
    from tribal_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["tribal_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process TRIBAL_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_tribal_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "tribal setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from tribal_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from tribal_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``tribal --tui`` binary the CLI uses, behind
...
try:
    from tribal_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from tribal_cli.dashboard_auth.audit import AuditEvent, audit_log
        from tribal_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``tribal --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``TRIBAL_TUI_RESUME`` env var —
    matching what ``tribal_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``TRIBAL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from tribal_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("TRIBAL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("TRIBAL_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["TRIBAL_TUI_RESUME"] = resume

    if sidecar_url:
        env["TRIBAL_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from tribal_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Tribal inside WSL2 to use the dashboard's /chat "
...
# TRIBAL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`tribal_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from tribal_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/tribal/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /tribal`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__TRIBAL_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/tribal``)
        ...
        ``__TRIBAL_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__TRIBAL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__TRIBAL_BASE_PATH__="{prefix}";'
                f"window.__TRIBAL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__TRIBAL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__TRIBAL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__TRIBAL_BASE_PATH__="{prefix}";'
                f"window.__TRIBAL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/tribal`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Tribal backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Tribal Teal",         "description": "Classic dark teal — the canonical Tribal look"},
    {"name": "default-large", "label": "Tribal Teal (Large)", "description": "Tribal Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.tribal/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.tribal/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_tribal_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.tribal/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as tribal_cli.plugins):
    1. User plugins:    ~/.tribal/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.tribal/plugins/  (only if TRIBAL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from tribal_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_tribal_home() / "plugins", "user"),
    ...
    # ``tribal_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("TRIBAL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".tribal" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from tribal_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_tribal_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"tribal auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from tribal_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from tribal_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from tribal_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from tribal_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.tribal/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.tribal/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"tribal_dashboard_plugin_{plugin['name']}"
...
from tribal_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from tribal_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing TRIBAL_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Tribal Agent exposes a
    ...
    print(f"  Tribal Web UI → http://{host}:{port}")
