...
``$TRIBAL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-tribal-setup
(the stage2 hook) has chowned the volume and seeded $TRIBAL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$TRIBAL_HOME``,
    not under ``profiles/``). The dispatcher in ``tribal_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``tribal gateway start`` (no ``-p``) targets. Without it,
    bare ``tribal gateway start`` inside the container would land on
    ...
    ``$TRIBAL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        tribal_home: The container's TRIBAL_HOME (typically /opt/data).
            Profiles live under ``<tribal_home>/profiles/<name>/``;
            the default profile lives at ``<tribal_home>`` itself.
    ...
    # ``tribal gateway start`` (no ``-p``) has somewhere to land;
    # auto-up only when the prior state was "running" (same rule as
    # named profiles).
    default_prior_state = _read_prior_state(tribal_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(tribal_home)
    ...
    profiles_root = tribal_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `tribal profile create` (config.yaml
            # is not — that comes later via `tribal setup`). Use it as the
            ...
            # via ``tribal -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(tribal_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `tribal -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit tribal
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the tribal user) won't hit EACCES. See
...
def _write_reconcile_log(
    tribal_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $TRIBAL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    tribal_home = Path(os.environ.get("TRIBAL_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        tribal_home=tribal_home, scandir=scandir,
    )
    for a in actions:
        print(
