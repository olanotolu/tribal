"""
Multi-provider authentication system for Tribal Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.tribal/auth.json with cross-process file locking.
...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when TRIBAL_AGENT_USE_LEGACY_SESSION_KEYS is set for
...
from tribal_cli.config import get_tribal_home, get_config_path, read_raw_config
from tribal_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "tribal-cli"
...
NOUS_LEGACY_SESSION_KEYS_ENV = "TRIBAL_AGENT_USE_LEGACY_SESSION_KEYS"
...
SPOTIFY_DOCS_URL = "https://tribal.dev/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://tribal.dev/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://tribal.dev/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from tribal_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.tribal/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``tribal auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `tribal model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("TRIBAL_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.tribal/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_tribal_home() / "auth.json"
    # Seat belt: if pytest is running and TRIBAL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch TRIBAL_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".tribal" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set TRIBAL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom TRIBAL_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_tribal_root()
    except Exception:
        return None
    profile_home = get_tribal_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.tribal/auth.json`` even when TRIBAL_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_tribal_root()`` for a profile-shaped TRIBAL_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".tribal" / "auth.json"
            try:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``tribal auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``tribal auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Tribal.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `tribal logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from tribal_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'tribal doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'tribal model' for available providers, or run 'tribal doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'tribal model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.tribal/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("TRIBAL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `tribal model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.tribal/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `tribal auth list` / `tribal status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.tribal/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("TRIBAL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set TRIBAL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("TRIBAL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("TRIBAL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("TRIBAL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Tribal times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``TRIBAL_LOG_LEVEL=INFO`` (or check
            # ``~/.tribal/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>tribal auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `tribal auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `tribal auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `tribal auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `tribal auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.tribal/.env, and return it.
    ...
    print("       App name:     anything (e.g. tribal-agent)")
    ...
    # Persist so subsequent `tribal auth spotify` runs skip the wizard.
    save_env_value("TRIBAL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("TRIBAL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved TRIBAL_SPOTIFY_CLIENT_ID to ~/.tribal/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "TRIBAL_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Tribal:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Tribal is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.tribal/auth.json (not ~/.codex/)
#
# Tribal maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Tribal auth store (~/.tribal/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `tribal auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `tribal auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `tribal auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `tribal auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``tribal setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``tribal auth add openai-codex``
      ...
      ``tribal auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Tribal auth store (~/.tribal/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Tribal auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run tribal auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `tribal auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Tribal auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Tribal's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            base_url = (
                os.getenv("TRIBAL_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("TRIBAL_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Tribal processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.tribal/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `tribal model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `tribal model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `tribal model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `tribal model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.tribal/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `tribal model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``TRIBAL_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `tribal model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Tribal (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `tribal model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``tribal model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("TRIBAL_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("TRIBAL_XAI_BASE_URL", "").strip().rstrip("/")
...
# so a new `tribal --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${TRIBAL_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<tribal-root>/shared/nous_auth.json`` where ``<tribal-root>`` is what
# ``get_default_tribal_root()`` returns — ``~/.tribal`` on Linux/macOS,
# ``%LOCALAPPDATA%\tribal`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's TRIBAL_HOME so named profiles (which
# typically live under ``<tribal-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``TRIBAL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<tribal-root>/shared/``, where ``<tribal-root>`` is what
    :func:`tribal_constants.get_default_tribal_root` returns — so
    Linux/macOS classic installs land at ``~/.tribal/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\tribal\\shared\\``, and
    Docker / custom ``TRIBAL_HOME`` deployments at
    ``<TRIBAL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("TRIBAL_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from tribal_constants import get_default_tribal_root
    return get_default_tribal_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Tribal root, refuse rather than silently corrupt cross-profile
    # state. Tests must set TRIBAL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from tribal_constants import get_default_tribal_root
        real_home_shared = (
            get_default_tribal_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set TRIBAL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No TRIBAL_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Tribal's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Tribal's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Tribal install sharing "
            "~/.tribal/auth.json) called POST /api/oauth/token with Tribal's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Tribal may call the "
            "refresh endpoint. For health checks, use `tribal auth status` "
            "instead.\n"
            "Re-authenticate with: tribal auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Tribal models — they're not reliable for agentic tool-calling
            if "tribal" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Tribal is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("TRIBAL_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "tribal-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``tribal auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``tribal auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `tribal auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Tribal is not logged into Nous Portal.",
...
# ~350ms even on the failure path, and read-only UI surfaces (`tribal tools`, status panels,
# subscription-feature checks) call it many times per render — `tribal tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `tribal auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `tribal auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `tribal auth` and
    # `tribal model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("TRIBAL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("TRIBAL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``tribal doctor`` runs the live
    ...
    try:
        from tribal_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Tribal' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `tribal doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set TRIBAL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `tribal logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from tribal_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from tribal_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'tribal model' or 'tribal setup' instead."""
    print("The 'tribal login' command has been removed.")
    print("Use 'tribal auth' to manage credentials,")
    print("'tribal model' to select a provider, or 'tribal setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.tribal/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Tribal-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Tribal auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Tribal will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("TRIBAL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Tribal will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Tribal gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Tribal creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Tribal auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Tribal auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Tribal creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=tribal-agent` lets xAI attribute Tribal-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Tribal — please report at "
            "https://github.com/Tribal/tribal/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Tribal with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Tribal auth store (~/.tribal/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Tribal login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `tribal model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``tribal status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Tribal login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `tribal model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Tribal builds still see
        ...
        if not selected_model:
            ...
            print("  Run `tribal model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Tribal will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `tribal model` or configure an API key to use Tribal.")
        else:
