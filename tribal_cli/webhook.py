"""tribal webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.tribal/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _tribal_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: tribal gateway run
"""
...
def webhook_command(args):
    """Entry point for 'tribal webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: tribal webhook {subscribe|list|remove|test}")
        print("Run 'tribal webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (tribal gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: tribal webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from tribal webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (tribal gateway run)")
