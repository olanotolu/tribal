#!/usr/bin/env python3
"""
Tribal CLI - Main entry point.

Usage:
    tribal                     # Interactive chat (default)
    tribal chat                # Interactive chat
    tribal gateway             # Run gateway in foreground
    tribal gateway start       # Start gateway as service
    tribal gateway stop        # Stop gateway service
    tribal gateway status      # Show gateway status
    tribal gateway install     # Install gateway service
    tribal gateway uninstall   # Uninstall gateway service
    tribal setup               # Interactive setup wizard
    tribal logout              # Clear stored authentication
    tribal status              # Show status of all components
    tribal cron                # Manage cron jobs
    tribal cron list           # List cron jobs
    tribal cron status         # Check if cron scheduler is running
    tribal doctor              # Check configuration and dependencies
    tribal honcho setup                    # Configure Honcho AI memory integration
    tribal honcho status                   # Show Honcho config and connection status
    tribal honcho sessions                 # List directory → session name mappings
    tribal honcho map <name>               # Map current directory to a session name
    tribal honcho peer                     # Show peer names and dialectic settings
    tribal honcho peer --user NAME         # Set user peer name
    tribal honcho peer --ai NAME           # Set AI peer name
    tribal honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    tribal honcho mode                     # Show current memory mode
    tribal honcho mode [hybrid|honcho|local]  # Set memory mode
    tribal honcho tokens                   # Show token budget settings
    tribal honcho tokens --context N       # Set session.context() token cap
    tribal honcho tokens --dialectic N     # Set dialectic result char cap
    tribal honcho identity                 # Show AI peer identity representation
    tribal honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    tribal honcho migrate                  # Step-by-step migration guide: OpenClaw native → Tribal + Honcho
    tribal version             Show version
    tribal update              Update to latest version
    tribal uninstall           Uninstall Tribal Agent
    tribal acp                 Run as an ACP server for editor integration
    tribal sessions browse     Interactive session picker with search

    tribal claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: tribal_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``tribal_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``tribal update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``tribal_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, tribal crashes on import and the user can't run
# ``tribal update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# `entry.tsx`; this is just the earlier cousin. ``TRIBAL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("TRIBAL_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("TRIBAL_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`tribal --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from tribal_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Tribal Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``tribal --version`` before config/logging imports on Termux."""
    if os.environ.get("TRIBAL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to TRIBAL_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (tribal tools, tribal setup, tribal model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'tribal {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any tribal module import.
#
# Many modules cache TRIBAL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("TRIBAL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.tribal/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set TRIBAL_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors tribal_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If TRIBAL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.tribal/profiles/coder or
    # /opt/data/profiles/coder).  If TRIBAL_HOME points to the tribal root
    # instead (e.g. systemd hardcodes TRIBAL_HOME=/root/.tribal), we must
    # still read active_profile — the user may have switched profiles via
    # `tribal profile use` and the gateway should honour that choice.
    # See issue #22502.
    tribal_home_env = os.environ.get("TRIBAL_HOME", "")
    if profile_name is None and tribal_home_env:
        if Path(tribal_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the tribal root
    if profile_name is None:
        try:
            from tribal_constants import get_default_tribal_root

            active_path = get_default_tribal_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set TRIBAL_HOME
    if profile_name is not None:
        try:
            from tribal_cli.profiles import resolve_profile_env

            tribal_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent tribal from starting
            print(
...
# Load .env from ~/.tribal/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from tribal_cli.config import get_tribal_home
from tribal_cli.env_loader import load_tribal_dotenv

load_tribal_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → TRIBAL_REDACT_SECRETS env
# var BEFORE tribal_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_tribal_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "TRIBAL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["TRIBAL_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `tribal` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from tribal_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from tribal_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if tribal_constants not importable yet
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_tribal_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("TRIBAL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("TRIBAL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from tribal_cli.config import get_env_path, get_tribal_home, load_config
    from tribal_cli.auth import get_auth_status

    # Determine whether Tribal itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_tribal_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_tribal_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Tribal has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Tribal to use their tokens.
    if _has_tribal_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, tribal_bin
        cli_args: the original CLI arguments (everything after 'tribal')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    tribal_bin = container_info["tribal_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo tribal {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo tribal {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  tribal --tui --resume {target}")
    if title:
        print(f'  tribal --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @tribal/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@tribal" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/tribal-ink/package.json",
    "packages/tribal-ink/package-lock.json",
    "packages/tribal-ink/index.js",
    "packages/tribal-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``TRIBAL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("TRIBAL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``TRIBAL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("TRIBAL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "TRIBAL_HOME": tribal_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(tribal_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(tribal_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if tribal_cli_dir is None:
        tribal_cli_dir = Path(__file__).parent
    bundled = tribal_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (TRIBAL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("TRIBAL_NODE")
    ...
    ext_dir = os.environ.get("TRIBAL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with TRIBAL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset TRIBAL_TUI_DIR (e.g. `unset TRIBAL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("TRIBAL_QUIET"):
    ...
    if tui_dev:
        # Keep the local @tribal/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @tribal/ink resolves through
        # packages/tribal-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "tribal-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from tribal_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="tribal-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["TRIBAL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["TRIBAL_PYTHON_SRC_ROOT"] = os.environ.get(
        "TRIBAL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("TRIBAL_PYTHON", sys.executable)
    env.setdefault("TRIBAL_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["TRIBAL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["TRIBAL_MODEL"] = model
        env["TRIBAL_INFERENCE_MODEL"] = model
    if provider:
        env["TRIBAL_TUI_PROVIDER"] = provider
        env["TRIBAL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["TRIBAL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["TRIBAL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["TRIBAL_TUI_SKILLS"] = value
    if query:
        env["TRIBAL_TUI_QUERY"] = query
    if image:
        env["TRIBAL_TUI_IMAGE"] = image
    if checkpoints:
        env["TRIBAL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["TRIBAL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["TRIBAL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["TRIBAL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["TRIBAL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["TRIBAL_ACCEPT_HOOKS"] = "1"
    ...
    # TRIBAL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `tribal --tui` try to
    ...
    # still set TRIBAL_TUI_RESUME themselves.
    env.pop("TRIBAL_TUI_RESUME", None)
    if resume_session_id:
        env["TRIBAL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `tribal update` so
    ...
    if code == 42:
        from tribal_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``TRIBAL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``tribal kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``tribal kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("TRIBAL_KANBAN_BOARD"):
        return
    try:
        from tribal_cli.kanban_db import get_current_board

        os.environ["TRIBAL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("TRIBAL_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'tribal sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from tribal_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from tribal_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'tribal doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Tribal isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  tribal setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'tribal setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.tribal/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["TRIBAL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["TRIBAL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["TRIBAL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from tribal_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from tribal_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Tribal?")
        print()
    ...
    # has no creds.json.  Every subsequent `tribal gateway` then paid a 30s
    ...
    session_dir = get_tribal_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: tribal gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `tribal gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  tribal gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Tribal Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  tribal gateway")
        ...
        print()
        print("  Or install as a service: tribal gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'tribal whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from tribal_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from tribal_cli.config import stamp_install_method
    from tribal_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Tribal post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``tribal model``) and the setup wizard
    ...
    from tribal_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("TRIBAL_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'tribal model' for "
                "available providers, or run 'tribal doctor' to diagnose config "
                "issues."
            )
    ...
    from tribal_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.tribal/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.tribal/.env if the active provider is not 'custom'.
    ...
    """
    from tribal_cli.config import get_env_value, save_env_value, load_config
...
#
# Tribal uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `tribal model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `tribal model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`tribal_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Tribal only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``tribal model`` flow, then route aux tasks to them here.
    """
    from tribal_cli.config import load_config
    from tribal_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from tribal_cli.auth import _prompt_model_selection
    from tribal_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from tribal_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from tribal_constants import OPENROUTER_BASE_URL
    from tribal_cli.auth import (
        ProviderConfig,
    ...
    # in-flow (K/R/C) instead of having to edit ~/.tribal/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `tribal setup` from scratch. OpenRouter
    ...
    from tribal_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from tribal_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from tribal_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from tribal_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `tribal auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from tribal_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``tribal model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``tribal model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``tribal auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from tribal_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from tribal_cli.auth import _save_model_choice, deactivate_provider
    from tribal_cli.config import get_env_value, load_config, save_config
    from tribal_cli.secret_prompt import masked_secret_prompt
    ...
    from tribal_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `tribal model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from tribal_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Tribal URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from tribal_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from tribal_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from tribal_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Tribal will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Tribal will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from tribal_cli.curses_ui import flush_stdin
...
# downstream call sites read `tribal_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `tribal` invocation — including
# fast paths like `tribal --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from tribal_cli.config import save_env_value, load_config, save_config
    from tribal_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from tribal_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from tribal_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from tribal_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Tribal turns to `copilot --acp`.")
    print("  Tribal currently starts its own ACP subprocess for each request.")
    print("  Tribal uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set TRIBAL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``tribal setup`` / ``tribal model``.
    ...
    recover from a malformed paste without editing ``~/.tribal/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from tribal_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `tribal setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from tribal_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so tribal knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Tribal typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Tribal, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from tribal_cli.models import fetch_lmstudio_models
        ...
        from tribal_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from tribal_constants import display_tribal_home as _dhh_fn

            print(
                f"    Tribal will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from tribal_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'tribal model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Tribal CLI with a provider."""
    from tribal_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from tribal_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from tribal_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from tribal_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from tribal_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from tribal_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from tribal_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``tribal slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: tribal slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `tribal slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from tribal_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from tribal_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from tribal_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from tribal_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `tribal security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from tribal_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from tribal_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from tribal_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from tribal_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Tribal home directory to a zip file."""
    if getattr(args, "quick", False):
        from tribal_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from tribal_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Tribal backup from a zip file."""
    from tribal_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from tribal_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Tribal Agent."""
    _require_tty("uninstall")
    from tribal_cli.uninstall import run_uninstall
...
# Critical files that every ``tribal`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``tribal update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "tribal_cli/main.py",
    "tribal_cli/config.py",
    "tribal_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``tribal`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="tribal-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``tribal update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``tribal_cli/web_dist/`` (per vite.config.ts
    outDir: "../tribal_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "tribal_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``tribal`` launcher present but ``tribal_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``tribal update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``tribal web``).
    ...
    # build path usable on a stock `py -m tribal_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (tribal web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``tribal dashboard`` processes other than ourselves.

    ``tribal dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``tribal update`` replaces files on disk, the running
    ...
    patterns = [
        "tribal dashboard",
        "tribal_cli.main dashboard",
        "tribal_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "tribal.*dashboard"`) keeps us consistent
            # with `tribal_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `tribal update`.
    ...
    print("  Preview now:  tribal curator run --dry-run")
    print("  Pause it:     tribal curator pause")
    print(
        "  Docs:         https://tribal.dev/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``tribal update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``tribal update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: tribal curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``tribal dashboard`` processes.

    Called at the end of ``tribal update`` (default ``reason``) and also
    from ``tribal dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    tribal dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Tribal Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `tribal update "
            f"--branch {branch}`, or update against main with `tribal update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/Tribal/tribal/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="tribal-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"tribal-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for tribal-agent itself
        ...
        # GitHub ZIPs extract to tribal-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"tribal-agent-{branch}")
    ...
    # optional — a failure here only affects ``tribal dashboard``. Make
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "tribal-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Tribal behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes tribal completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Tribal couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Tribal couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Tribal behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `tribal update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Tribal/tribal.git",
    "git@github.com:Tribal/tribal.git",
    "https://github.com/Tribal/tribal",
    "git@github.com:Tribal/tribal",
}
OFFICIAL_REPO_URL = "https://github.com/Tribal/tribal.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_tribal_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_tribal_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Tribal repository.")
        print("  This means you may miss updates from Tribal/tribal.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Tribal/tribal.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Tribal/tribal.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``tribal update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from tribal_constants import get_default_tribal_root

    default_home = get_default_tribal_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``tribal update`` is still progressing even if pip/uv itself is silent.
    """
...
def _tribal_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "tribal.exe",
        scripts_dir / "tribal-gateway.exe",
    ]


def _detect_concurrent_tribal_instances(
    ...
    The Tribal Desktop Electron app spawns ``tribal.EXE`` as a backend child,
    so during ``tribal update`` the user-invoked process and the desktop's
    ...
    shims (``tribal.exe`` / ``tribal-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``tribal update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``tribal.exe``) is a separate process from the Python
    ...
    # entire parent chain. On Windows the setuptools-generated tribal.exe
    ...
    # walking the parent chain, every ``tribal update`` reports its own
    ...
    for shim in _tribal_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "tribal.exe"
    lines = ["✗ Another tribal.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Tribal Desktop, exit any open `tribal` REPLs, and")
    lines.append("  stop the gateway (`tribal gateway stop`) before retrying.")
    lines.append("  Override with `tribal update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_tribal_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``tribal.exe``.
    ...
    when ``tribal update`` runs, ``tribal.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``tribal.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next tribal invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Tribal Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Tribal Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _tribal_exe_shims(scripts_dir):
        ...
        print(
            "    Close Tribal Desktop, exit other `tribal` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `tribal update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_tribal_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``tribal.exe.old.*`` left by prior updates.

    Called early on every tribal invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``tribal update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `tribal update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``tribal.exe`` / ``tribal-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_tribal_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_tribal_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from tribal_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.tribal/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``tribal update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.tribal/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``tribal update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``tribal update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.tribal/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``tribal update --gateway``) the update is already
    ...
    try:
        ...
        # tribal_cli.config.get_tribal_home to simulate setup failure.
        from tribal_cli.config import get_tribal_home as _get_tribal_home

        logs_dir = _get_tribal_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``tribal update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from tribal_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``tribal update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from tribal_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from tribal_cli.config import recommended_update_command
        from tribal_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``tribal update`` without requiring a reinstall.  The
    ...
    ``tribal`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/tribal.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v tribal'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/tribal, code at /usr/local/lib/tribal-agent).
    fhs_link = Path("/usr/local/bin/tribal")
    ...
    path_comment = (
        "# Tribal Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of TRIBAL_HOME before running the update.
    ...
    TRIBAL_HOME directories.  The ``--backup`` flag on ``tribal update``
    ...
    try:
        from tribal_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_tribal_home so the user sees ~/.tribal/...
    try:
        from tribal_constants import get_tribal_home, display_tribal_home

        home = get_tribal_home()
        try:
            display_path = f"{display_tribal_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  tribal import {out_path}")
...
def cmd_update(args):
    """Update Tribal Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Tribal Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `tribal update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Tribal via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "tribal-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "tribal-agent"]
    ...
    print("✓ Update complete! Restart tribal to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Tribal Agent...")
    print()

    # On Windows, abort early if another tribal.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_tribal_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in tribal_cli/config.py bricked
        # every user who ran ``tribal update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``tribal
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``tribal update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_tribal_home added to tribal_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload tribal_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_tribal_home() added since the last release.
        try:
            import importlib
            import tribal_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit TRIBAL_HOME so
        # it is not affected by sync_skills()'s module-level TRIBAL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's TRIBAL_HOME env var points at the default or a named profile.
        try:
            from tribal_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: tribal config migrate")
            else:
                print()
                print("Skipped. Run 'tribal config migrate' later to configure.")
        else:
        ...
        # check `tribal curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``tribal update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from tribal_cli.tools_config import install_cua_driver
        ...
        # When running as ``tribal update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_tribal_home() / ".update_exit_code"
            try:
        ...
        try:
            from tribal_cli.gateway import (
                is_macos,
            ...
            # Discover all tribal-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. tribal-gateway.service or tribal-gateway-coder.service
                            ...
                            # path in `tribal gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: tribal gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: tribal -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Tribal gateway unit files are still installed.
        # When both tribal.service (from a pre-rename install) and the
        # current tribal-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `tribal update` surfaces the issue until the user migrates.
        try:
            from tribal_cli.gateway import (
                has_legacy_tribal_units,
                _find_legacy_tribal_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_tribal_units():
                print()
                print("⚠ Legacy Tribal gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_tribal_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (tribal.service) fight the current")
                print("  tribal-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    tribal gateway migrate-legacy")
                print()
        ...
        print("  tribal model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``tribal -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `tribal profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_tribal_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → tribal -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.tribal)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  tribal profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     tribal -p {name} chat")
                else:
        ...
        from tribal_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from tribal_constants import get_tribal_home as _hh
        ...
        from tribal_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `tribal profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from tribal_cli.profiles import profile_exists
        ...
        from tribal_cli.profiles import rename_profile
        ...
        from tribal_cli.profiles import export_profile
        ...
        from tribal_cli.profiles import import_profile
        ...
        import tempfile
        from tribal_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tribal_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  tribal -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      tribal -p {plan.manifest.name} chat")
        ...
        from tribal_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `tribal profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"tribal -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from tribal_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("tribal_requires"):
            print(f"Requires:     Tribal {data['tribal_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from tribal_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.tribal_requires:
        print(f"  Requires: Tribal {mf.tribal_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``tribal dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``tribal dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No tribal dashboard processes running.")
        return 0

    print(f"{len(pids)} tribal dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No tribal dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `tribal update`.
    ...
    if "TRIBAL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["TRIBAL_WEB_DIST"])
            if "TRIBAL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "tribal_cli" / "web_dist"
        )
    ...
    from tribal_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("TRIBAL_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from tribal_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Tribal log files."""
    from tribal_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from tribal_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``tribal -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``tribal_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``tribal -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``tribal`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("TRIBAL_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from tribal_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from tribal_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["TRIBAL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["TRIBAL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["TRIBAL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `tribal --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("TRIBAL_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("TRIBAL_TUI") == "1"):
        return False
...
def main():
    """Main entry point for tribal CLI."""
    ...
    # Sweep stale ``tribal.exe.old.*`` quarantine files left by previous
    # ``tribal update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_tribal_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: tribal-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from tribal_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://tribal.dev/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `tribal model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.tribal/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://tribal.dev/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from tribal_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from tribal_cli.migrate import cmd_migrate, cmd_migrate_xai

    migrate_parser = subparsers.add_parser(
        "migrate",
        help="Migrate legacy homes, retired models, or deprecated settings",
        description=(
            "Copy legacy agent homes into Tribal or diagnose and rewrite the "
            "active config.yaml to replace retired models or deprecated settings."
        ),
    )
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if TRIBAL_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy tribal.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Tribal gateway unit files "
            "(e.g. tribal.service) left over from older installs. Profile "
            "units (tribal-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `tribal proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Tribal Agent with an interactive wizard. "
        "Run a specific section: tribal setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'tribal setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Tribal.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $TRIBAL_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Tribal")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from tribal_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Tribal CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: tribal-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Tribal with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set TRIBAL_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Tribal Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.tribal/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Tribal profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.tribal/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Tribal profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from tribal_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from tribal_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.tribal/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.tribal/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Tribal Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `tribal "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Tribal "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.tribal/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Tribal Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Tribal setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Tribal Agent. Use 'tribal debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    tribal debug share              Upload debug report and print URL
    tribal debug share --lines 500  Include more log lines
    tribal debug share --expire 30  Keep paste for 30 days
    tribal debug share --local      Print report locally (no upload)
    tribal debug share --no-redact  Disable upload-time secret redaction
    tribal debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'tribal debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Tribal home directory to a zip file",
        description="Create a zip archive of your entire Tribal configuration, "
        "skills, sessions, and data (excludes the tribal-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/tribal-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.tribal/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo tribal uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from tribal_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Tribal backup from a zip file",
        description="Extract a previously created Tribal backup into your "
        "Tribal home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Tribal Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from tribal_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.tribal/skills/.bundled_manifest) "
            "so future 'tribal update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from tribal_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from tribal_cli.skills_hub import skills_command
    ...
    from tribal_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/tribal-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `tribal plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from tribal_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``tribal --help``, ``tribal version``, ``tribal logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from tribal_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from tribal_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_tribal_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_tribal_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_tribal_home()}/memories/\n")
        else:
            from tribal_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'tribal tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # tribal tools list [--platform cli]
    ...
    # tribal tools disable <name...> [--platform cli]
    ...
    # tribal tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from tribal_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from tribal_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `tribal computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `tribal tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: tribal computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: tribal computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Tribal as an MCP server",
        description=(
            "Manage MCP server connections and run Tribal as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'tribal mcp add' to connect to a new server, or\n"
            "'tribal mcp serve' to expose Tribal conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Tribal as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `tribal mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `tribal mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `tribal mcp install n8n`)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch tribal --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Tribal",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Tribal",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.tribal/ (by default a "
        "single restore-point archive is written to ~/.tribal/backups/ "
        "before apply; restorable with 'tribal import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from tribal_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Tribal Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'tribal config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, tribal will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another tribal.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Tribal Agent",
        description="Remove Tribal Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Tribal Agent as an ACP (Agent Client Protocol) server",
        description="Start Tribal Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Tribal ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    def cmd_acp(args):
        """Launch Tribal Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Tribal instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `tribal update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `tribal profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Tribal profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Tribal Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `tribal --tui` via PTY/WebSocket). "
            "Alternatively set TRIBAL_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `tribal dashboard` cmdlines and SIGTERM them directly — the same
    # path `tribal update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running tribal dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running tribal dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Tribal log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    tribal logs                    Show last 50 lines of agent.log
    tribal logs -f                 Follow agent.log in real time
    tribal logs errors             Show last 50 lines of errors.log
    tribal logs gateway -n 100     Show last 100 lines of gateway.log
    tribal logs --level WARNING    Only show WARNING and above
    tribal logs --session abc123   Filter by session ID
    tribal logs --component tools  Only show tool-related lines
    tribal logs --since 1h         Lines from the last hour
    tribal logs --since 30m -f     Follow, starting from 30 min ago
    tribal logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``tribal -c Pokemon Agent Dev`` → ``tribal -c 'Pokemon Agent Dev'``
    ...
    from tribal_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'tribal -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (tribal hooks list, cron
