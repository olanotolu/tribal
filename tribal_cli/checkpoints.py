"""`tribal checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.tribal/checkpoints/``.  Actions:

    tribal checkpoints               # same as `status`
    tribal checkpoints status        # total size, project count, breakdown
    tribal checkpoints list          # per-project checkpoint counts + workdir
    tribal checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    tribal checkpoints clear [-f]    # nuke the entire base (asks first)
    tribal checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    tribal checkpoints
    tribal checkpoints prune --retention-days 3 --max-size-mb 200
    tribal checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: tribal checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``tribal checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `tribal checkpoints` → status
