...
# lookup, and tribal-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and tribal-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when TRIBAL_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("TRIBAL_VOICE_DEBUG", "").strip() != "1":
        return
    try:
