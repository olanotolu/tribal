"""Shell completion script generation for tribal CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_tribal_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Tribal Agent bash completion
# Add to ~/.bashrc:
#   eval "$(tribal completion bash)"

_tribal_profiles() {{
    local profiles_dir="$HOME/.tribal/profiles"
...
_tribal_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_tribal_profiles)" -- "$cur"))
        return
    fi
...
complete -F _tribal_completion tribal
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef tribal
# Tribal Agent zsh completion
# Add to ~/.zshrc:
...
_tribal_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.tribal/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.tribal/profiles 2>/dev/null)}}")
    fi
...
_tribal() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_tribal_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'tribal command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Tribal Agent fish completion",
        "# Add to your config:",
        "#   tribal completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __tribal_profiles",
        "    echo default",
        "    if test -d $HOME/.tribal/profiles",
        "        ls $HOME/.tribal/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c tribal -f -s p -l profile"
        " -d 'Profile name' -xa '(__tribal_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__tribal_profiles)' -d 'Profile name'"
                )
