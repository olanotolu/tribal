"""CLI subcommand: ``tribal send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"tribal send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"tribal send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"tribal send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"tribal send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"tribal send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `tribal gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.tribal/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_tribal_env() -> None:
    """Populate ``os.environ`` from ``~/.tribal/.env`` AND bridge top-level
    ...
    startup that ``tribal send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.tribal/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.tribal/config.yaml`` into
       ...
       via ``tribal config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``tribal send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.tribal/.env and ~/.tribal/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "tribal send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  tribal send --to telegram \"hello\"\n"
            "  tribal send --to discord:#ops --file report.md\n"
            "  tribal send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "tribal send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `tribal send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Tribal "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.tribal/.env + ~/.tribal/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `tribal send --list telegram`.",
    )
