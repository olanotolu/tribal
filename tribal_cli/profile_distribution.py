"""Profile distributions — shareable, packaged Tribal profiles via git.

A distribution is a Tribal profile published as a git repository (or
...
* ``tribal profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``tribal skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``tribal profile``, not a parallel tree):

    tribal profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "tribal-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            tribal_requires=str(data.get("tribal_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.tribal_requires:
            out["tribal_requires"] = self.tribal_requires
        if self.author:
...
def check_tribal_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Tribal {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``tribal profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Tribal profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Tribal distribution."
        )

    # Version check up-front so we fail fast
    check_tribal_requires(manifest.tribal_requires, tribal_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.tribal).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="tribal_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `tribal profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`tribal profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="tribal_dist_update_") as tmp:
        plan = plan_install(
