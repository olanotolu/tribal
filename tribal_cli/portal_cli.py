"""``tribal portal`` — small CLI surface for Nous Portal users.
...
already in ``tribal auth`` or ``tribal tools``. It's a discovery + status
...
DOCS_URL = "https://tribal.dev/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   tribal auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `tribal model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `tribal auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `tribal portal <subcommand>`."""
    ...
    print("Run `tribal portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `tribal portal` on the given argparse subparsers object."""
