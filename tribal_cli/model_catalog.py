...
The Tribal docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.tribal/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_tribal_home() / "cache" / "model_catalog.json"
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``tribal model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
