"""MCP picker — interactive `tribal mcp picker` (also the default `tribal mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``tribal mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `tribal plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from tribal_cli.cli_output import prompt_yes_no
from tribal_cli.curses_ui import curses_single_select
from tribal_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Tribal session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from tribal_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `tribal mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: tribal mcp install <name>    Picker: tribal mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Tribal is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Tribal — run `tribal update` "
...
def show_catalog() -> None:
    """`tribal mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`tribal mcp picker` (and default `tribal mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`tribal mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from tribal_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `tribal mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
