"""Migrate Tribal' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Tribal' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Tribal mcp_servers.<n>.command/args/env  → codex stdio transport
  Tribal mcp_servers.<n>.url/headers       → codex streamable_http transport
  Tribal mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Tribal mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Tribal-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Tribal' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Tribal config.")
...
# Tribal keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Tribal' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, tribal_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Tribal MCP server config to the codex inline-table dict
    ...
    if not isinstance(tribal_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(tribal_cfg.get("command"))
    has_url = bool(tribal_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (TRIBAL_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Tribal)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Tribal' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Tribal' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Tribal
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="tribal-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a TRIBAL_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed tribal-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.tribal`` symlink that happens to live under ``/private/var/folders``
...
def _build_tribal_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Tribal' own
    ...
    so a tribal installed under /opt/, /usr/local/, or a venv all work.
    TRIBAL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # TRIBAL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_tribal_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever TRIBAL_HOME its launcher sets
    ...
    # — that would override the launcher's TRIBAL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("TRIBAL_HOME", tmp_path) would
    ...
    tribal_home = os.environ.get("TRIBAL_HOME") or ""
    if tribal_home and _looks_like_test_tempdir(tribal_home):
        tribal_home = ""
    if tribal_home:
        env["TRIBAL_HOME"] = tribal_home
    # PYTHONPATH passes through so a worktree-launched tribal finds the
    ...
    env["TRIBAL_QUIET"] = "1"
    env["TRIBAL_REDACT_SECRETS"] = env.get("TRIBAL_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_tribal_tools: bool = True,
) -> MigrationReport:
    """Translate Tribal mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        tribal_config: full ~/.tribal/config.yaml dict
        ...
        expose_tribal_tools: when True (default), register Tribal' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Tribal for tools
    ...
    tribal_servers = (tribal_config or {}).get("mcp_servers") or {}
    if not isinstance(tribal_servers, dict):
        report.errors.append(
            "mcp_servers in Tribal config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in tribal_servers.items():
    ...
    # Inject Tribal' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Tribal for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/tribal_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_tribal_tools:
        translated["tribal-tools"] = _build_tribal_tools_mcp_entry()
        if "tribal-tools" not in report.migrated:
            report.migrated.append("tribal-tools")
