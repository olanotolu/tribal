"""
Dump command for tribal CLI.

Outputs a compact, plain-text summary of the user's Tribal setup
...
from tribal_cli.config import get_tribal_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.tribal_build_sha`` by the Dockerfile's
    ``TRIBAL_GIT_SHA`` build-arg (see ``tribal_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``tribal dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from tribal_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(tribal_home: Path) -> int:
...
def _cron_summary(tribal_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = tribal_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"tribal_home:      {display_tribal_home()}")
    ...
    toolsets = config.get("toolsets", ["tribal-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(tribal_home)}")
    lines.append(f"  skills:             {_count_skills(tribal_home)}")
