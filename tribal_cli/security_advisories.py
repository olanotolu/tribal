"""
Security advisory checker for Tribal Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``tribal doctor --ack <id>``; the ack is persisted to
...
1. ``tribal doctor`` (and ``tribal doctor --ack <id>``)
...
run in environments where the rest of Tribal failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including tribal when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.tribal/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: tribal doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Tribal requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'tribal doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.tribal/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_tribal_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``tribal doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `tribal doctor` on the gateway host for details.")
