...
the existing module-level functions in tribal_cli.gateway and
tribal_cli.gateway_windows directly. This protocol is a thin facade
...
``tribal gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``tribal gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged tribal
    ...
    detection always fails. Since every Tribal runtime call inside the
    container drops to tribal via ``s6-setuidgid``, that silent failure
...
# in ``tribal_cli.gateway`` (systemd/launchd) and ``tribal_cli.gateway_windows``
...
# active (set via the ``tribal -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in tribal_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from tribal_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from tribal_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from tribal_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from tribal_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in tribal_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from tribal_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from tribal_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from tribal_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from tribal_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``tribal_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from tribal_cli import gateway_windows
        from tribal_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `tribal profile create`
# runs inside the container (Phase 4). Static services (main-tribal, dashboard)
...
# UID/GID of the in-image ``tribal`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the tribal user.
    ...
    (the ``tribal`` user — UID 10000 — running every Tribal runtime
    ...
    is dispatched as the tribal user.
    ...
    with tribal ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           tribal:tribal, 0755 (parent must already exist)
    ``svc_dir/event/``                     tribal:tribal, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 tribal:tribal, 0755
    ``svc_dir/supervise/event/``           tribal:tribal, 03730
    ``svc_dir/supervise/control``          tribal:tribal, 0660    (FIFO)
    ...
    slot's supervise/ was tribal-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _TRIBAL_UID, _TRIBAL_GID)
        except PermissionError:
            # Running as the tribal user already — directory is tribal-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _TRIBAL_UID, _TRIBAL_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is tribal-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _TRIBAL_UID, _TRIBAL_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``tribal -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`tribal profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-tribal, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources TRIBAL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e TRIBAL_HOME=/data/tribal`` is honored at run
          ...
          4. Drops to the tribal user and exec's
             ``tribal -p <profile> gateway run`` (or just ``tribal
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``tribal gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root TRIBAL_HOME profile" (the implicit profile that exists at
        the top of $TRIBAL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``tribal_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$TRIBAL_HOME/profiles/default/`` — a
        ...
        ``tribal_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export TRIBAL_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            lines.append("exec s6-setuidgid tribal tribal gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid tribal tribal -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${TRIBAL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the TRIBAL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e TRIBAL_HOME=/data/tribal``
        gets its logs under /data/tribal/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``tribal logs`` reads and what
        ...
        return (
            ...
            f': "${{TRIBAL_HOME:=/opt/data}}"\n'
            f'log_dir="$TRIBAL_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R tribal:tribal "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid tribal s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with tribal ownership
            ...
            # the tribal user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is tribal-owned (see ``_seed_supervise_skeleton``), and on
