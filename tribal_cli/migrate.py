"""CLI handlers for ``tribal migrate ...``."""
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``tribal migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: tribal migrate {xai} [options]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `tribal migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `tribal doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
