...
configured for "cli" in `tribal tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (TRIBAL_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `tribal chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"tribal -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "tribal -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"tribal -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "tribal -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "tribal -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to TRIBAL_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("TRIBAL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "tribal -z: --provider requires --model (or TRIBAL_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``tribal -z`` / oneshot mode.

    Oneshot bypasses ``TribalCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when tribal is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from tribal_cli.config import load_config
    from tribal_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("TRIBAL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from tribal_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("TRIBAL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                TRIBAL_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via TRIBAL_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via TRIBAL_YOLO_MODE=1
