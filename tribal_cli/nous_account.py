...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Tribal could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `tribal model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Tribal cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`tribal model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `tribal model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Tribal could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `tribal model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Tribal could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `tribal model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `tribal model` to refresh Tribal."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from tribal_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from tribal_cli.auth import _decode_jwt_claims
    except Exception:
        return None
