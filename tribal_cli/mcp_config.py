"""
MCP Server Management CLI — ``tribal mcp`` subcommand.

Implements ``tribal mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.tribal/config.yaml under the ``mcp_servers`` key.
"""
...
from tribal_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_tribal_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from tribal_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # tribal_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  tribal mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  tribal mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  tribal mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_tribal_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: tribal mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_tribal_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `tribal mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  tribal mcp add <name> --url <endpoint>')
        _info('  tribal mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `tribal mcp remove` + `tribal mcp add` to reconfigure auth.")
        return
...
# ─── tribal mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'tribal mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``tribal mcp`` subcommands."""
    ...
    if action == "picker":
        from tribal_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from tribal_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from tribal_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `tribal plugin`.
        from tribal_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("tribal mcp                                    Open the catalog picker (default)")
        _info("tribal mcp catalog                            List Nous-approved MCPs")
        _info("tribal mcp install <name>                     Install a catalog MCP")
        _info("tribal mcp serve                              Run as MCP server")
        _info("tribal mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("tribal mcp add <name> --command <cmd>         Add a stdio server")
        _info("tribal mcp add <name> --preset <preset>       Add from a known preset")
        _info("tribal mcp remove <name>                      Remove a server")
        _info("tribal mcp list                               List configured servers")
        _info("tribal mcp test <name>                        Test connection")
        _info("tribal mcp configure <name>                   Toggle tools")
        _info("tribal mcp login <name>                       Re-authenticate OAuth")
        print()
