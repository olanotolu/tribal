"""Implementation of the ``tribal bundles`` CLI subcommand.

Mirrors the structure of ``tribal_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `tribal bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  tribal bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``tribal bundles`` argparse tree.

    Called from ``tribal_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `tribal bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``tribal bundles <subcommand>`` to the right handler."""
