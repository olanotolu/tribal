"""``tribal logs`` — view and filter Tribal log files.
...
Usage examples::
    ...
    tribal logs                    # last 50 lines of agent.log
    tribal logs -f                 # follow agent.log in real time
    tribal logs errors             # last 50 lines of errors.log
    tribal logs gateway -n 100    # last 100 lines of gateway.log
    tribal logs --level WARNING    # only WARNING+ lines
    tribal logs --session abc123   # filter by session ID substring
    tribal logs --component tools  # only tool-related lines
    tribal logs --since 1h         # lines from the last hour
    tribal logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_tribal_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Tribal runs — try 'tribal chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_tribal_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_tribal_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_tribal_home()}/logs/")
        return

    print(f"Log files in {display_tribal_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'tribal chat' to generate logs)")
