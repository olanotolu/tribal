"""
Tribal Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with tribal-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.tribal/plugins/<name>/``
3. **Project plugins** – ``./.tribal/plugins/<name>/`` (opt-in via
   ``TRIBAL_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``tribal_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``TRIBAL_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("TRIBAL_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``TRIBAL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.tribal/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("TRIBAL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When TRIBAL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Tribal loggers.
    """
    ...
    logger.debug(
        "TRIBAL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "tribal_agent.plugins"

_NS_PARENT = "tribal_plugins"
...
class PluginManifest:
    ...
    #              in ~/.tribal/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``tribal plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``tribal honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``tribal <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`tribal_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``tribal model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: tribal_cli.main imports plugins indirectly
        from tribal_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.tribal/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to tribal_cli/. Two layouts are
        ...
        # 2. User plugins (~/.tribal/plugins/)
        user_dir = get_tribal_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.tribal/plugins/)
        if _env_enabled("TRIBAL_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".tribal" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set TRIBAL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with tribal and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Tribal ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `tribal plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``tribal_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``tribal_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="tribal-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``tribal tools`` TUI so plugin-provided toolsets appear
