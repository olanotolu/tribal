"""``tribal slack ...`` CLI subcommands.

Today only ``tribal slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ tribal slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Tribal. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Tribal deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from tribal_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Tribal agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Tribal in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``tribal_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$TRIBAL_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Tribal")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Tribal"
    description = getattr(args, "description", None) or "Your Tribal agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("TRIBAL_HOME") or str(Path.home() / ".tribal")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Tribal app\n"
