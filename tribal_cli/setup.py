"""
Interactive setup wizard for Tribal Agent.
...
from tribal_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://tribal.dev/docs"
...
# display_tribal_home imported lazily at call sites (stale-module safety during tribal update)
...
from tribal_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from tribal_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Tribal Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Tribal using environment variables or config commands:")
    print_info("  tribal config set model.provider custom")
    print_info("  tribal config set model.base_url http://localhost:8080/v1")
    print_info("  tribal config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'tribal setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from tribal_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from tribal_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'tribal setup')")


def _print_setup_summary(config: dict, tribal_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'tribal setup' to configure"))
    ...
    # Video generation — opt-in via `tribal tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from tribal_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'tribal setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'tribal setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'tribal setup terminal'"))
    ...
    # Spotify (OAuth via tribal auth spotify — check auth.json, not env vars)
    try:
        from tribal_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'tribal setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {tribal_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('tribal setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('tribal setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('tribal setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('tribal setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('tribal setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('tribal config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('tribal config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('tribal config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('tribal', Colors.GREEN)}              Start chatting")
    print(f"   {color('tribal gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('tribal doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `tribal tools` and `tribal setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``tribal model``)
    ...
    provider added to ``tribal model`` is automatically available here.
    ...
    # Delegate to the shared tribal model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from tribal_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: tribal model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from tribal_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Tribal can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Tribal can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'tribal setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``tribal model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from tribal_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.tribal/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Tribal auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via tribal setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'tribal setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Tribal runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into TRIBAL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("TRIBAL_MAX_ITERATIONS")
    ...
    print_info("  Run `tribal setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # TRIBAL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Tribal delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `tribal update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://tribal.dev/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under TRIBAL_HOME, and print
    ...
    """
    try:
        from tribal_cli.slack_cli import _build_full_manifest
        from tribal_constants import get_tribal_home

        manifest = _build_full_manifest(
            bot_name="Tribal",
            bot_description="Your Tribal agent on Slack",
        )
        target = Path(get_tribal_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `tribal slack manifest --write` anytime to refresh after "
            "Tribal adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Tribal delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Tribal to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from tribal_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://tribal.dev/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://tribal.dev/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  tribal config edit")
    print_info("   Open config in your editor:  tribal config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from tribal_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Tribal from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'tribal setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     tribal config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_tribal_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  tribal gateway")
        else:
            from tribal_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   tribal gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   tribal gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `tribal setup tools` and `tribal tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``tribal_cli.auth`` — lists every supported
    ...
    """
    try:
        from tribal_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from tribal_cli.config
    ...
    if section_key == "model":
        ...
        from tribal_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_tribal.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_tribal migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_tribal", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Tribal.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Tribal to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Tribal at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Tribal at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Tribal at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Tribal at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Tribal equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Tribal config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Tribal.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Tribal's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(tribal_home: Path) -> bool:
    ...
    print_info("Tribal can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: tribal claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=tribal_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: tribal claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Tribal configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Tribal config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Tribal (use tribal claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``tribal setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``tribal setup --portal``)
    that gets a brand-new user from zero to a fully working Tribal session
    ...
    from tribal_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Tribal Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `tribal setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `tribal auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `tribal auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `tribal model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `tribal portal status` to inspect routing.")
    print_info("  Run `tribal` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      tribal setup           — full or quick (auto-detected)
      tribal setup model     — just model/provider
      tribal setup tts       — just text-to-speech
      tribal setup terminal  — just terminal backend
      tribal setup gateway   — just messaging platforms
      tribal setup tools     — just tool configuration
      tribal setup agent     — just agent settings
    """
    from tribal_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Tribal Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Tribal Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Tribal Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, tribal_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Tribal configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'tribal setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(tribal_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Tribal?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, tribal_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {tribal_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'tribal config edit'")
    ...
    _print_setup_summary(config, tribal_home)


def _run_first_time_quick_setup(config: dict, tribal_home, is_existing: bool):
    ...
    the user can customize later via ``tribal setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'tribal setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    tribal setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  tribal setup gateway")
    print()

    _print_setup_summary(config, tribal_home)


def _run_quick_setup(config: dict, tribal_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'tribal setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Tribal to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'tribal setup gateway'.")
