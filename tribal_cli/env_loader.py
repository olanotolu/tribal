"""Helpers for loading Tribal .env files consistently across entrypoints."""
...
# load_tribal_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_tribal_dotenv().
# Used by setup / `tribal model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# TRIBAL_HOME paths we've already pulled external secrets for during this
# process.  ``load_tribal_dotenv()`` is called at module-import time from
# several hot modules (cli.py, tribal_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_tribal_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which TRIBAL_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `tribal setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``tribal_cli.config._sanitize_env_lines`` which
    already knows all valid Tribal env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from tribal_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_tribal_dotenv(
    *,
    tribal_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Tribal environment files with user config taking precedence.

    Behavior:
    - `~/.tribal/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(tribal_home or os.getenv("TRIBAL_HOME", Path.home() / ".tribal"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Tribal reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_tribal_dotenv()`` runs at import
    time from several hot modules (cli.py, tribal_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``tribal secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `tribal model`
