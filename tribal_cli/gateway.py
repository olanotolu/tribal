"""
Gateway subcommand for tribal CLI.

Handles: tribal gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_tribal_home is imported lazily at call sites to avoid ImportError
# when tribal_constants is cached from a pre-update version during `tribal update`.
from tribal_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from tribal_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "tribal-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "tribal CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``tribal gateway status`` from falsely counting the ``tribal`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``tribal gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "tribal_cli.main gateway",
        "tribal_cli.main --profile",
        "tribal_cli.main -p",
        "tribal_cli/main.py gateway",
        "tribal_cli/main.py --profile",
        "tribal_cli/main.py -p",
        "tribal gateway",
        "gateway/run.py",
    ]
    current_home = str(get_tribal_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"TRIBAL_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. TRIBAL_HOME
        ...
        # TRIBAL_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "TRIBAL_HOME=" in command and f"TRIBAL_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``tribal update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Tribal profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Tribal profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "tribal_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``tribal update`` users
    # with no running gateway until they re-invoke ``tribal gateway``
...
def _sync_tribal_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``TRIBAL_HOME``.

    Under ``sudo``, ``TRIBAL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_tribal_home` falls back to ``/root/.tribal`` — the wrong
    profile. The unit file pins ``TRIBAL_HOME`` for the actual gateway
    ...
    unit_home = env.get("TRIBAL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("TRIBAL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["TRIBAL_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}tribal gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}tribal gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from tribal_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from tribal_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `tribal gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``tribal gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from tribal_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (TRIBAL_HOME-scoped).
...
from tribal_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Tribal Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``tribal gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``TRIBAL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("TRIBAL_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "tribal-gateway"
SERVICE_DESCRIPTION = "Tribal Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current TRIBAL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (TRIBAL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from tribal_constants import get_default_tribal_root
    home = get_tribal_home().resolve()
    default = get_default_tribal_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary TRIBAL_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(tribal_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when TRIBAL_HOME is a named profile.

    For ``~/.tribal/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        tribal_home: Optional explicit TRIBAL_HOME path. Defaults to the current
            ``get_tribal_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from tribal_constants import get_default_tribal_root
    home = Path(tribal_home or str(get_tribal_home())).resolve()
    default = get_default_tribal_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this TRIBAL_HOME.

    Default ``~/.tribal`` returns ``tribal-gateway`` (backward compatible).
    Profile ``~/.tribal/profiles/coder`` returns ``tribal-gateway-coder``.
    Any other TRIBAL_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Tribal installs that predate the
# tribal-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (tribal-gateway-*.service) and unrelated third-party
# "tribal" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("tribal.service",)
...
def _find_legacy_tribal_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Tribal gateway units.

    Detects unit files installed by older Tribal versions that used a
    different service name (e.g. ``tribal.service`` before the rename to
    ``tribal-gateway.service``). When both a legacy unit and the current
    ``tribal-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``tribal-gateway-coder.service`` and unrelated third-party
      ``tribal-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``tribal.service`` running an unrelated
...
def has_legacy_tribal_units() -> bool:
    """Return True when any legacy Tribal gateway unit files exist."""
    return bool(_find_legacy_tribal_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Tribal gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_tribal_units()
    if not legacy:
        return
    print_warning("Legacy Tribal gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current tribal-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    tribal gateway migrate-legacy")


def remove_legacy_tribal_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Tribal gateway unit files.

    Iterates over whatever ``_find_legacy_tribal_units()`` returns — which is
    ...
    legacy = _find_legacy_tribal_units()
    if not legacy:
        print("No legacy Tribal gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Tribal gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: tribal gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo tribal gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    tribal gateway uninstall")
    print_info("    sudo tribal gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Tribal can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo tribal gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo tribal gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo tribal gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Tribal often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.tribal`` → ``ai.tribal.gateway.plist`` (backward compatible).
    Profile ``~/.tribal/profiles/coder`` → ``ai.tribal.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.tribal.gateway-{suffix}" if suffix else "ai.tribal.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.tribal/tribal-agent  -> /home/alice/.tribal/tribal-agent
      /opt/tribal                 -> /opt/tribal  (kept as-is)
...
def _tribal_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current TRIBAL_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_tribal_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.tribal                    → /home/alice/.tribal
      /root/.tribal/profiles/coder     → /home/alice/.tribal/profiles/coder
      /opt/custom-tribal               → /opt/custom-tribal  (kept as-is)
    """
    current_tribal = get_tribal_home().resolve()
    current_default = (Path.home() / ".tribal").resolve()
    target_default = Path(target_home_dir) / ".tribal"

    # Default ~/.tribal → remap to target user's default
    if current_tribal == current_default:
        return str(target_default)

    # Profile or subdir of ~/.tribal → preserve the relative structure
    try:
        relative = current_tribal.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.tribal) — keep as-is
        return str(current_tribal)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    tribal_node = tribal_home / "node" / "bin"
    if _is_dir(tribal_node):
        candidates.append(str(tribal_node))
    tribal_nm = tribal_home / "node_modules" / ".bin"
    if _is_dir(tribal_nm):
        candidates.append(str(tribal_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        tribal_home = _tribal_home_for_target_user(home_dir)
        profile_arg = _profile_arg(tribal_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m tribal_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="TRIBAL_HOME={tribal_home}"
Restart=always
RestartSec=5
...
"""
    ...
    tribal_home = str(get_tribal_home().resolve())
    profile_arg = _profile_arg(tribal_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only TRIBAL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp TRIBAL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../tribal_test`` path into
    # ``Environment="TRIBAL_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Tribal install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo tribal gateway uninstall --system")
    print_info("         tribal gateway install")
    print_info("         tribal gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (tribal.service from pre-rename installs)
    # before installing the new tribal-gateway.service. If both remain, they
    ...
    if has_legacy_tribal_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_tribal_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}tribal gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}tribal gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}tribal gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_tribal_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `tribal gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `tribal gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}tribal gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}tribal gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}tribal gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}tribal gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.tribal.gateway-{suffix}" if suffix else "ai.tribal.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Tribal install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  tribal gateway status             # Check status")
...
def launchd_stop():
    ...
    # `tribal gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Tribal install")
    else:
        print("⚠ Service definition is stale relative to the current Tribal install")
        print("  Run: tribal gateway start")
    ...
    if deep:
        log_file = get_tribal_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("TRIBAL_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Tribal gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'tribal' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/tribal/docker/entrypoint.sh before the Tribal command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$TRIBAL_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set TRIBAL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `tribal gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set TRIBAL_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `tribal gateway restart` which already
    ...
    # `tribal gateway start/restart` — leaving the gateway vulnerable to
    ...
    # TRIBAL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("TRIBAL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from tribal_constants import get_tribal_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Tribal will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @tribal:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. tribal) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Tribal delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Tribal agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Tribal will use (e.g., tribal@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Tribal connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: tribal pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Tribal will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``tribal setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run tribal under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.tribal/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_tribal_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'tribal pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: tribal pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'tribal gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from tribal_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from tribal_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from tribal_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Tribal will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Tribal will store the returned account_id/token in ~/.tribal/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `tribal gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_tribal_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `tribal pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `tribal pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"TribalAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``tribal_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from tribal_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.tribal/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.tribal/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: tribal gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: tribal gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: tribal gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo tribal gateway install --system")
                    print_info("  Or run in foreground:  tribal gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: tribal gateway run")
                print_info("  For persistence:   tmux new -s tribal 'tribal gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from tribal_constants import display_tribal_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: tribal gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup tribal gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: tribal gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'tribal gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`tribal_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from tribal_cli.service_manager import (
    ...
    if profile is None:
        ...
        # TRIBAL_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``tribal gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``tribal gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``tribal gateway run`` are unaffected.
      2. ``TRIBAL_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``tribal gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``TRIBAL_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("TRIBAL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("TRIBAL_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${TRIBAL_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if TRIBAL_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or TRIBAL_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    # gateway flaps. `sleep infinity` matches the static main-tribal
    # service's pattern (see docker/s6-rc.d/main-tribal/run): the CMD
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: tribal gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: tribal gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s tribal 'tribal gateway run'")
                print()
            ...
            print()
            print("  tribal gateway run                              # direct foreground")
            print("  tmux new -s tribal 'tribal gateway run'         # persistent via tmux")
            print("  nohup tribal gateway run > ~/.tribal/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  tribal profile create <name>     # creates the s6 service slot")
                print("  tribal -p <name> gateway start   # bring it up via s6")
                print("  tribal status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: tribal gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: tribal gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: tribal gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  tribal profile delete <name>     # tears down the s6 service slot")
                print("  tribal -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `tribal -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: tribal gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: tribal gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  tribal gateway install")
                else:
                    print("To install as a service:")
                    print("  tribal gateway install")
                    print("  sudo tribal gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  tribal gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup tribal gateway run > ~/.tribal/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s tribal 'tribal gateway run'         # persistent via tmux")
                    print("  nohup tribal gateway run > ~/.tribal/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  tribal gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  tribal gateway install  # Install as user service")
                    print("  sudo tribal gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Tribal gateway unit files from
        # pre-rename installs (e.g. tribal.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_tribal_units(interactive=not yes, dry_run=dry_run)
