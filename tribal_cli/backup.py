"""
Backup and import commands for tribal CLI.

`tribal backup` creates a zip archive of the entire ~/.tribal/ directory
(excluding the tribal-agent repo and transient files).

`tribal import` restores from a backup zip, overlaying onto the current
TRIBAL_HOME root.
"""
...
from tribal_constants import get_default_tribal_root, get_tribal_home, display_tribal_home
...
_EXCLUDED_DIRS = {
    "tribal-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to tribal root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside TRIBAL_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Tribal home directory."""
    tribal_root = get_default_tribal_root()

    if not tribal_root.is_dir():
        print(f"Error: Tribal home directory not found at {tribal_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"tribal-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"tribal-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_tribal_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(tribal_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(tribal_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(tribal_root)
    ...
    print(f"\nRestore with: tribal import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Tribal backup.
    ...
    # Look for telltale files that a tribal home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Tribal backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.tribal/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a tribal dir name
        if prefix in {".tribal", "tribal"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_tribal_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (tribal_root / "config.yaml").exists()
        has_env = (tribal_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Tribal configuration.")
        ...
        for member in members:
            ...
            target = tribal_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(tribal_root.resolve())
            except ValueError:
        ...
        profiles_dir = tribal_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # tribal_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: tribal profile list  (after installing tribal)")

        # Guidance
        print()
        if not (tribal_root / "tribal-agent").is_dir():
            print("Note: The tribal-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: tribal update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  tribal -p {pname} gateway install")

        print("Done. Your Tribal configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and tribal backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TRIBAL_HOME).
...
# — `tribal update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(tribal_home: Optional[Path] = None) -> Path:
    home = tribal_home or get_tribal_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    tribal_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(tribal_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(tribal_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for tribal backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_tribal_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, tribal_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``tribal_root`` to ``out_path``.
...
def _pre_update_backup_dir(tribal_home: Optional[Path] = None) -> Path:
    home = tribal_home or get_tribal_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TRIBAL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TRIBAL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``tribal update``) should continue even if the backup fails.
    """
    tribal_root = tribal_home or get_default_tribal_root()
    if not tribal_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(tribal_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, tribal_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `tribal claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TRIBAL_HOME under ``backups/`` before a
    ``tribal claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``tribal import <archive>``.  Writes to
    ``<TRIBAL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `tribal import` and the
