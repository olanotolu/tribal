...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_tribal_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:tribal_pkce",
    ...
    if provider == "nous":
        ...
        # <tribal-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `tribal --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `tribal auth
    ...
    raise SystemExit(f"`tribal auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Tribal reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `tribal auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `tribal auth` is called bare."""
