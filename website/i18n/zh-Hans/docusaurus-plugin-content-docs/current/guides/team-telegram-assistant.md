...
本教程将引导你搭建一个由 Tribal Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，你的团队将拥有一个共享 AI 助手，可以向它发消息寻求代码、研究、系统管理等方面的帮助——并通过按用户授权保障安全。
...
- **已在服务器或 VPS 上安装 Tribal Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/getting-started/installation)。
- **一个 Telegram 账号**（机器人所有者）
- **已配置 LLM 提供商**——至少在 `~/.tribal/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥

:::tip
一台 $5/月的 VPS 足以运行 gateway（网关）。Tribal 本身很轻量——花钱的是 LLM API 调用，而那些调用发生在远端。
:::
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   - **显示名称**——用户看到的名字（例如 `Team Tribal Assistant`）
   - **用户名**——必须以 `bot` 结尾（例如 `myteam_tribal_bot`）
...
sudo tribal gateway install --system   # 仅 Linux：开机启动的系统服务
```
...
:::tip macOS PATH
launchd plist 在安装时捕获你的 Shell PATH，以便 gateway 子进程能找到 Node.js 和 ffmpeg 等工具。如果之后安装了新工具，请重新运行 `tribal gateway install` 以更新 plist。
:::
...
控制机器人在使用工具时显示的详细程度。在 `~/.tribal/config.yaml` 中：
...
通过编辑 `~/.tribal/SOUL.md` 自定义机器人的沟通方式：

完整指南请参阅[在 Tribal 中使用 SOUL.md](/guides/use-soul-with-tribal)。
...
```bash
# 通过 CLI
tribal cron list          # 查看所有定时任务
tribal cron status        # 检查调度器是否运行
...
| 内容 | 位置 |
|------|----------|
| Gateway 日志 | `journalctl --user -u tribal-gateway`（Linux）或 `~/.tribal/logs/gateway.log`（macOS） |
| Cron 任务输出 | `~/.tribal/cron/output/{job_id}/{timestamp}.md` |
| Cron 任务定义 | `~/.tribal/cron/jobs.json` |
| 配对数据 | `~/.tribal/pairing/` |
| 会话历史 | `~/.tribal/sessions/` |
