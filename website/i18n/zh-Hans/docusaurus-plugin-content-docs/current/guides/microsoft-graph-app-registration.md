...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.tribal/.env`。
...
4. 填写以下内容：
   - **Name：**`Tribal Teams Meeting Pipeline`（或任何你能识别的名称）。
...
3. **Description：**`tribal-graph-secret`。**Expires：**根据你的轮换策略选择合适的值（通常为 6-24 个月）。
4. 点击 **Add**。
...
```powershell
# Create a policy scoped to the Tribal app
New-CsApplicationAccessPolicy `
  -Identity "Tribal-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Tribal meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Tribal-Meeting-Pipeline-Policy" `
...
Tribal 内置了 Graph 身份验证冒烟测试。在 Tribal 安装目录下执行：
...
2. 用新值更新 `~/.tribal/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
3. 重启 gateway 以使新密钥生效：`tribal gateway restart`。
4. 使用上述冒烟测试进行验证。
5. 在 Azure 门户中删除旧密钥。
