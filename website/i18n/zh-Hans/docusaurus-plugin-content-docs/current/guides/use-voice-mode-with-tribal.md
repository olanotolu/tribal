---
sidebar_position: 8
title: "在 Tribal 中使用语音模式"
description: "在 CLI、Telegram、Discord 及 Discord 语音频道中设置和使用 Tribal 语音模式的实用指南"
---
...
- 希望 Tribal 加入 Discord 语音频道进行实时对话
- 边走动边快速记录想法、调试问题或来回交流，而不是打字

## 选择你的语音模式方案

Tribal 中实际上有三种不同的语音体验。
...
如果你在设置向导中选择了 NeuTTS，Tribal 会检查 `neutts` 是否已安装。如果缺失，向导会告知你 NeuTTS 需要 Python 包 `neutts` 和系统包 `espeak-ng`，并提供自动安装，使用平台包管理器安装 `espeak-ng`，然后运行：
...
如果打字不方便，语音模式是保持完整 Tribal 工作流的最快方式之一。

## 调整 CLI 行为

### 静音阈值

如果 Tribal 开始/停止过于激进，调整：
...
Tribal 加入 Discord 语音频道（VC），监听用户语音，转录后运行正常的 agent 流水线，并将回复朗读回频道。
