...
本页介绍如何为 Tribal Agent 配置推理提供商——从 OpenRouter、Anthropic 等云端 API，到 Ollama、vLLM 等自托管端点，再到高级路由与故障转移配置。使用 Tribal 至少需要配置一个提供商。

## 推理提供商

你需要至少一种方式连接到 LLM。使用 `tribal model` 交互式切换提供商和模型，或直接配置：

| 提供商 | 配置方式 |
|----------|-------|
| **Nous Portal** | `tribal model`（OAuth，订阅制） |
| **OpenAI Codex** | `tribal model`（ChatGPT OAuth，使用 Codex 模型） |
| **GitHub Copilot** | `tribal model`（OAuth 设备码流程，`COPILOT_GITHUB_TOKEN`、`GH_TOKEN` 或 `gh auth token`） |
| **GitHub Copilot ACP** | `tribal model`（在本地生成 `copilot --acp --stdio` 子进程） |
| **Anthropic** | `tribal model`（Claude Max + 额外用量积分，通过 OAuth；也支持 Anthropic API key 或手动 setup-token——见下方说明） |
| **OpenRouter** | `~/.tribal/.env` 中的 `OPENROUTER_API_KEY` |
| **NovitaAI** | `~/.tribal/.env` 中的 `NOVITA_API_KEY`（provider: `novita`，200+ 模型，Model API、Agent Sandbox、GPU Cloud） |
| **z.ai / GLM** | `~/.tribal/.env` 中的 `GLM_API_KEY`（provider: `zai`） |
| **Kimi / Moonshot** | `~/.tribal/.env` 中的 `KIMI_API_KEY`（provider: `kimi-coding`） |
| **Kimi / Moonshot（中国）** | `~/.tribal/.env` 中的 `KIMI_CN_API_KEY`（provider: `kimi-coding-cn`；别名：`kimi-cn`、`moonshot-cn`） |
| **Arcee AI** | `~/.tribal/.env` 中的 `ARCEEAI_API_KEY`（provider: `arcee`；别名：`arcee-ai`、`arceeai`） |
| **GMI Cloud** | `~/.tribal/.env` 中的 `GMI_API_KEY`（provider: `gmi`；别名：`gmi-cloud`、`gmicloud`） |
| **MiniMax** | `~/.tribal/.env` 中的 `MINIMAX_API_KEY`（provider: `minimax`） |
| **MiniMax 中国** | `~/.tribal/.env` 中的 `MINIMAX_CN_API_KEY`（provider: `minimax-cn`） |
| **xAI（Grok）— Responses API** | `~/.tribal/.env` 中的 `XAI_API_KEY`（provider: `xai`） |
| **xAI Grok OAuth（SuperGrok）** | `tribal model` → "xAI Grok OAuth (SuperGrok / Premium+)"——浏览器登录，无需 API key。参见[指南](../guides/xai-grok-oauth.md) |
| **Qwen Cloud（阿里 DashScope）** | `~/.tribal/.env` 中的 `DASHSCOPE_API_KEY`（provider: `alibaba`） |
| **阿里云（Coding Plan）** | `DASHSCOPE_API_KEY`（provider: `alibaba-coding-plan`，别名：`alibaba_coding`）——独立计费 SKU，不同端点 |
| **Kilo Code** | `~/.tribal/.env` 中的 `KILOCODE_API_KEY`（provider: `kilocode`） |
| **小米 MiMo** | `~/.tribal/.env` 中的 `XIAOMI_API_KEY`（provider: `xiaomi`，别名：`mimo`、`xiaomi-mimo`） |
| **腾讯 TokenHub** | `~/.tribal/.env` 中的 `TOKENHUB_API_KEY`（provider: `tencent-tokenhub`，别名：`tencent`、`tokenhub`、`tencentmaas`） |
| **OpenCode Zen** | `~/.tribal/.env` 中的 `OPENCODE_ZEN_API_KEY`（provider: `opencode-zen`） |
| **OpenCode Go** | `~/.tribal/.env` 中的 `OPENCODE_GO_API_KEY`（provider: `opencode-go`） |
| **DeepSeek** | `~/.tribal/.env` 中的 `DEEPSEEK_API_KEY`（provider: `deepseek`） |
| **Hugging Face** | `~/.tribal/.env` 中的 `HF_TOKEN`（provider: `huggingface`，别名：`hf`） |
| **Google / Gemini** | `~/.tribal/.env` 中的 `GOOGLE_API_KEY`（或 `GEMINI_API_KEY`）（provider: `gemini`） |
| **Google Gemini（OAuth）** | `tribal model` → "Google Gemini (OAuth)"（provider: `google-gemini-cli`，支持免费层，浏览器 PKCE 登录） |
| **LM Studio** | `tribal model` → "LM Studio"（provider: `lmstudio`，可选 `LM_API_KEY`） |
| **自定义端点** | `tribal model` → 选择"Custom endpoint"（保存在 `config.yaml`） |
...
[Nous Portal](https://portal.nousresearch.com) 是 Nous Research 的统一订阅网关，也是**运行 Tribal Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://chat.nousresearch.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。

```bash
tribal setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
tribal model              # 已有安装——从列表中选择"Nous Portal"
tribal portal status      # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.nousresearch.com/manage-subscription](https://portal.nousresearch.com/manage-subscription) 购买。

**完整详情：** 参见专属的 [Nous Portal 集成页面](/integrations/nous-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Nous Portal 运行 Tribal Agent](/guides/run-tribal-with-nous-portal)。


:::info Codex 说明
OpenAI Codex 提供商通过设备码（device code）认证——打开一个 URL 并输入验证码。Tribal 将生成的凭据存储在 `~/.tribal/auth.json` 的自有认证存储中，并在存在 `~/.codex/auth.json` 时可导入现有的 Codex CLI 凭据。无需安装 Codex CLI。

如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Tribal 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `tribal auth add codex-oauth`（或 `tribal model` → OpenAI Codex）开始新的设备码登录；成功交换后隔离状态自动解除。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Tribal 将这些任务路由到你的**主聊天模型**——即你在 `tribal model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`tribal setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `tribal model` 或 `tribal tools` 按工具启用。随时使用 `tribal portal status` 查看路由状态。
:::

### 模型管理的两个命令

Tribal 有**两个**模型命令，用途不同：

| 命令 | 运行位置 | 功能 |
|---------|-------------|--------------|
| **`tribal model`** | 终端（任何会话之外） | 完整配置向导——添加提供商、运行 OAuth、输入 API key、配置端点 |
| **`/model`** | Tribal 聊天会话内部 | 在**已配置的**提供商和模型之间快速切换 |

如果你想切换到尚未配置的提供商（例如你只配置了 OpenRouter，想使用 Anthropic），需要使用 `tribal model`，而不是 `/model`。先退出会话（`Ctrl+C` 或 `/quit`），运行 `tribal model`，完成提供商配置，然后开启新会话。
...
通过 `tribal model` → Anthropic OAuth（或 `tribal auth add anthropic --type oauth`）认证时，Tribal 以 Claude Code 身份路由到你的 Anthropic 账户。**仅当你订阅了 Claude Max 计划且购买了额外用量积分时才有效。** Claude Max 基础计划的配额（Claude Code 默认包含的用量）不会被 Tribal 消耗——只有你额外购买的超额积分才会被使用。Claude Pro 订阅者无法使用此路径。
...
# 推荐：通过 `tribal model` 认证
# 如果已使用 Claude Code，Tribal 会直接使用其凭据存储
tribal model
...
tribal chat --provider anthropic  # 自动读取 Claude Code 凭据文件
```

通过 `tribal model` 选择 Anthropic OAuth 时，Tribal 优先使用 Claude Code 自身的凭据存储，而不是将 token 复制到 `~/.tribal/.env`。这样可以保持 Claude 凭据的可刷新性。
...
Tribal 以一等提供商身份支持 GitHub Copilot，提供两种模式：
...
如果未找到 token，`tribal model` 会提供 **OAuth 设备码登录**——与 Copilot CLI 和 opencode 使用的流程相同。
...
| 类型 | 前缀 | 获取方式 |
|------|--------|------------|
| OAuth token | `gho_` | `tribal model` → GitHub Copilot → 使用 GitHub 登录 |
| 细粒度 PAT | `github_pat_` | GitHub 设置 → 开发者设置 → 细粒度 token（需要 **Copilot Requests** 权限） |
| GitHub App token | `ghu_` | 通过 GitHub App 安装获取 |

如果你的 `gh auth token` 返回 `ghp_*` token，请使用 `tribal model` 通过 OAuth 认证。
:::

:::info Tribal 中的 Copilot 认证行为
Tribal 将支持的 GitHub token（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送到 `api.githubcopilot.com`，并附带 Copilot 专用请求头（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

收到 HTTP 401 时，Tribal 在回退前会执行一次性凭据恢复：

1. 通过正常优先级链重新解析 token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新后的请求头重建共享 OpenAI 客户端
3. 重试请求一次

部分旧版社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点对某些账户类型可能不可用（返回 404）。因此 Tribal 以直接 token 认证为主路径，依靠运行时凭据刷新 + 重试保证健壮性。
:::
...
| `TRIBAL_COPILOT_ACP_COMMAND` | 覆盖 Copilot CLI 二进制路径（默认：`copilot`） |
| `TRIBAL_COPILOT_ACP_ARGS` | 覆盖 ACP 参数（默认：`--acp --stdio`） |
...
:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Tribal 会自动探测多个端点（全球版、中国版、编程版）以找到接受你 API key 的端点。无需手动设置 `GLM_BASE_URL`——可用端点会被自动检测并缓存。
:::

### xAI（Grok）— Responses API + Prompt 缓存

xAI 通过 Responses API（`codex_responses` 传输）接入，自动支持 Grok 4 模型的推理——无需 `reasoning_effort` 参数，服务端默认进行推理。在 `~/.tribal/.env` 中设置 `XAI_API_KEY` 并在 `tribal model` 中选择 xAI，或直接用 `grok` 作为快捷方式输入 `/model grok-4-1-fast-reasoning`。

SuperGrok 和 X Premium+ 订阅者可以用浏览器 OAuth 登录，无需 API key——在 `tribal model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或运行 `tribal auth add xai-oauth`。同一 OAuth bearer token 会被 xAI 直连工具（TTS、图像生成、视频生成、转录）自动复用。完整流程参见 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)——如果 Tribal 运行在远程主机上，还需参见 [SSH / 远程主机上的 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道配置。

使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Tribal 会在每次 API 请求中自动发送 `x-grok-conv-id` 请求头以启用 prompt（提示词）缓存。这会将同一会话的请求路由到同一服务器，使 xAI 基础设施能够复用已缓存的系统 prompt 和对话历史。

无需任何配置——检测到 xAI 端点且存在会话 ID 时，缓存自动激活。这可降低多轮对话的延迟和成本。

xAI 还提供专属 TTS 端点（`/v1/tts`）。在 `tribal tools` → 语音与 TTS 中选择 **xAI TTS**，或参见[语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面了解配置。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开源模型目录，无需 GPU。在 `tribal model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API key，Tribal 会自动发现可用模型。
...
阿里巴巴 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `tribal model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，Tribal 会持久化刷新 token。

```bash
tribal model
# → 选择"Qwen OAuth (Portal)"
# → 浏览器打开；使用阿里巴巴账户登录
# → 确认——凭据保存到 ~/.tribal/auth.json

tribal chat   # 使用 portal.qwen.ai/v1 端点
```
...
仅在 portal 端点迁移时才需设置 `TRIBAL_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
...
如果你订阅了阿里巴巴的 **Coding Plan**（独立于标准 DashScope API 访问的计费 SKU），Tribal 将其作为独立的一等提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。与常规 `alibaba` 提供商一样兼容 OpenAI，但基础 URL 和计费面不同。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7——无需 API key。在 `tribal model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Tribal 会持久化访问 token 和刷新 token。底层使用 Anthropic Messages 兼容端点（`/anthropic`）。
...
tribal chat   # 使用 api.minimax.io/anthropic 端点
```
...
# 需要：~/.tribal/.env 中的 NVIDIA_API_KEY
...
Tribal 会在每次向 `build.nvidia.com` 发送请求时自动附加 NIM 计费来源请求头——无需任何配置。这会在 NVIDIA 计费仪表板中将消耗路由到正确的来源。
...
```bash
# GMI Cloud
tribal chat --provider gmi --model deepseek-ai/DeepSeek-R1
# 需要：~/.tribal/.env 中的 GMI_API_KEY
```
...
# 需要：~/.tribal/.env 中的 STEPFUN_API_KEY
```
...
```bash
# 使用任意可用模型
tribal chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# 需要：~/.tribal/.env 中的 HF_TOKEN
...
# → 完成——Tribal 在首次请求时自动开通免费层
```

Tribal 默认使用 Google 的**公开** `gemini-cli` 桌面 OAuth 客户端——与 Google 在其开源 `gemini-cli` 中包含的凭据相同。桌面 OAuth 客户端不是机密客户端（PKCE 提供安全保障）。你无需安装 `gemini-cli` 或注册自己的 GCP OAuth 客户端。

**认证工作原理：**
- 针对 `accounts.google.com` 的 PKCE 授权码流程
- 浏览器回调地址 `http://127.0.0.1:8085/oauth2callback`（端口占用时自动回退到临时端口）
- Token 存储在 `~/.tribal/auth/google_oauth.json`（chmod 0600，原子写入，跨进程 `fcntl` 锁）
- 到期前 60 秒自动刷新
- 无头环境（SSH、`TRIBAL_HEADLESS=1`）→ 粘贴模式回退
...
- 响应转换回 OpenAI 格式，Tribal 其余部分无感知
...
| Workspace / Standard / Enterprise 账户 | 将 `TRIBAL_GEMINI_PROJECT_ID` 或 `GOOGLE_CLOUD_PROJECT` 设置为你的 GCP 项目 ID |
| VPC-SC 保护的组织 | Tribal 检测到 `SECURITY_POLICY_VIOLATED` 后自动强制使用 `standard-tier` |
...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端用于第三方软件违反政策。部分用户反映账户受到限制。为降低风险，建议改用 `gemini` 提供商并通过 API key 访问。Tribal 会在 OAuth 开始前显示警告并要求明确确认。
:::
...
TRIBAL_GEMINI_CLIENT_SECRET=...   # 桌面客户端可选
```
...
Tribal Agent 可与**任何 OpenAI 兼容 API 端点**配合使用。只要服务器实现了 `/v1/chat/completions`，就可以将 Tribal 指向它。这意味着你可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 旧版环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Tribal 的任何部分都不再读取这两个变量——`config.yaml` 是模型和端点配置的唯一来源。如果你的 `.env` 中有过时条目，下次运行 `tribal setup` 或配置迁移时会自动清除。请使用 `tribal model` 或直接编辑 `config.yaml`。
:::
...
:::warning tribal model 与 /model
**`tribal model`**（在终端中运行，任何聊天会话之外）是**完整的提供商配置向导**。用于添加新提供商、运行 OAuth 流程、输入 API key 和配置自定义端点。

**`/model`**（在活跃的 Tribal 聊天会话中输入）只能在**已配置的**提供商和模型之间**切换**。它无法添加新提供商、运行 OAuth 或提示输入 API key。如果你只配置了一个提供商（如 OpenRouter），`/model` 只会显示该提供商的模型。

**添加新提供商：** 退出会话（`Ctrl+C` 或 `/quit`），运行 `tribal model`，配置新提供商，然后开启新会话。
:::
...
切换提供商时，Tribal 会将基础 URL 和提供商持久化到配置中，使更改在重启后保留。从自定义端点切换到内置提供商时，过时的基础 URL 会自动清除。
...
**无法通过 OpenAI 兼容 API**（`/v1/chat/completions`）设置上下文长度。必须在服务端或通过 Modelfile 配置。这是将 Ollama 与 Tribal 等工具集成时最常见的困惑来源。
:::
...
| 标志 | 用途 |
|------|---------|
| `--enable-auto-tool-choice` | `tool_choice: "auto"` 所必需（Tribal 的默认值） |
| `--tool-call-parser <name>` | 模型工具调用格式的解析器 |

支持的解析器：`tribal`（Qwen 2.5、Tribal 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。没有这些标志，工具调用将无法工作——模型会将工具调用以文本形式输出。
...
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试在响应文本中写入 JSON 来调用工具，但 Tribal 不会将其识别为工具调用——你会看到原始 JSON（如 `{"name": "web_search", ...}`）作为消息打印出来，而不是实际执行搜索。

原生工具调用支持（最佳性能）：Llama 3.x、Qwen 2.5（包括 Coder）、Tribal 2/3、Mistral、DeepSeek、Functionary。其他所有模型使用通用处理器，可以工作但效率可能较低。完整列表参见 [llama.cpp 函数调用文档](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md)。
...
Tribal 会自动以 64K 上下文长度加载 LM Studio 模型。
...
**工具调用：** 自 LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Tribal）会被自动检测并显示工具徽章。其他模型使用通用回退，可靠性可能较低。

---

### WSL2 网络（Windows 用户）

由于 Tribal Agent 需要 Unix 环境，Windows 用户在 WSL2 内运行它。如果你的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机**上，需要桥接网络——WSL2 使用具有独立子网的虚拟网络适配器，因此 WSL2 内的 `localhost` 指向 Linux 虚拟机，**而非** Windows 主机。
...
如果收到列出模型的 JSON 响应，说明配置正确。在 Tribal 配置中使用相同的 URL 作为 `base_url`。

---

### 本地模型故障排查

以下问题影响与 Tribal 配合使用的**所有**本地推理服务器。

#### 从 WSL2 连接 Windows 托管模型服务器时"连接被拒绝"

如果你在 WSL2 内运行 Tribal 而模型服务器在 Windows 主机上，在 WSL2 默认 NAT 网络模式下 `http://localhost:<port>` 无法工作。参见上方的 [WSL2 网络](#wsl2-networking-windows-users) 了解解决方案。
...
| **vLLM** | 添加 `--enable-auto-tool-choice --tool-call-parser tribal` |
...
**原因：** 上下文窗口太小。当对话超过上下文限制时，大多数服务器会静默丢弃较早的消息。Tribal 的系统 prompt 加工具 schema 单独就可能占用 4k–8k tokens。
...
Tribal 从服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本不报告），Tribal 使用模型声明的限制，该值可能不正确。
...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 Tribal 中启用[上下文压缩](/user-guide/configuration#context-compression)。
...
然后通过 `tribal model` → 自定义端点 → `http://localhost:4000/v1` 配置 Tribal。
...
然后通过 `tribal model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 配置 Tribal。
...
通过 `tribal model` → 自定义端点，或在 `config.yaml` 中配置任意上述服务：
...
:::note 两个设置，容易混淆
**`context_length`** 是**总上下文窗口**——输入和输出 token 的合计预算（例如 Claude Opus 4.6 为 200,000）。Tribal 用它来决定何时压缩历史记录以及验证 API 请求。
...
Tribal 使用多源解析链来检测模型和提供商的正确上下文窗口：
...
`tribal model` 在配置自定义端点时会提示输入上下文长度。留空则自动检测。
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key 省略——Tribal 对无 key 的本地服务器使用"no-key-required"
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # 由 `tribal model` → 自定义端点向导显式设置；自动检测仍作为回退
...
某些 OpenAI 兼容端点需要特定于提供商的请求体字段。在对应的自定义提供商中添加 `extra_body` 映射，Tribal 会将其合并到该端点的每个 chat-completions 请求中：
...
`tribal model` → 自定义端点向导现在会显式提示 `api_mode` 并将你的答案持久化到 `config.yaml`。当字段留空时，基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）仍作为回退。
...
也可以从交互式 `tribal model` 菜单中选择命名自定义提供商。

---

### 实战配置：Together AI、Groq、Perplexity

[其他兼容提供商](#other-compatible-providers) 中列出的云提供商都使用 OpenAI 的 REST 方言，因此在 `custom_providers:` 下的接入方式相同。以下是三个可直接使用的配置示例。每个示例放入 `~/.tribal/config.yaml`，对应的 API key 放入 `~/.tribal/.env`。
...
Together 的 `/v1/models` 端点可用，因此 `tribal model` 可以自动发现可用模型。
...
:::tip 故障排查
- `tribal doctor` 对于上述任何名称都不应打印 `Unknown provider` 警告（在 #15083 的 CLI 验证器修复之后）。
- 如果某个提供商的 `/v1/models` 端点不可达（Perplexity 是常见情况），`tribal model` 会在警告后持久化模型而不是硬性拒绝——参见 #15136。
...
:::tip
可以随时使用 `tribal model` 切换提供商——无需重启。无论使用哪个提供商，你的对话历史、记忆和技能都会保留。
:::
...
默认情况下，Tribal 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你希望在本地运行 Firecrawl，可以将 Tribal 指向自托管实例。完整配置说明参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
使用 OpenRouter 时，可以控制请求如何在提供商之间路由。在 `~/.tribal/config.yaml` 中添加 `provider_routing` 节：
...
OpenRouter 提供一个实验性编程模型路由器 `openrouter/pareto-code`，自动将请求路由到满足编程质量标准的最便宜模型（按 [Artificial Analysis](https://artificialanalysis.ai/) 排名）。选择此模型并在 `~/.tribal/config.yaml` 中调整 `min_coding_score` 参数：
...
配置一个备用提供商链，当主模型失败时（速率限制、服务器错误、认证失败）Tribal 按顺序尝试。规范格式是顶级 `fallback_providers:` 列表：
...
:::tip
故障转移仅通过 `config.yaml` 配置——或通过 `tribal fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/user-guide/features/fallback-providers)。
:::
