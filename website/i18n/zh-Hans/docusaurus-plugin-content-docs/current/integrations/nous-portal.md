...
sidebar_position: 1
title: "Nous Portal"
description: "一个订阅，300+ 前沿模型，Tool Gateway，以及 Nous Chat —— 运行 Tribal Agent 的推荐方式"
---

# Nous Portal

[Nous Portal](https://portal.nousresearch.com) 是 Nous Research 的统一订阅网关，也是**运行 Tribal Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
这条命令会完成 Portal OAuth 认证，在 `config.yaml` 中将 Nous 设为推理提供商，并开启 Tool Gateway。完成后即可立即运行 `tribal chat`。
...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Tribal** | Tribal-4-70B、Tribal-4-405B（对话，见[下方说明](#a-note-on-tribal-4)） |
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 Tribal Agent 的工具调用路由至 Nous 托管的基础设施。五个后端，一次登录：
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.tribal/auth.json` 中的 refresh token（刷新令牌），Tribal 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
Nous Research 自家的 **Tribal 4** 系列（Tribal-4-70B、Tribal-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。

但**不建议在 Tribal Agent 内部使用它们**。Tribal 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于 [Nous Chat](https://chat.nousresearch.com)、研究工作流，或通过[订阅代理](/user-guide/features/subscription-proxy)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
Portal 自身的[模型信息页](https://portal.nousresearch.com/info)也有相同警告，因此这不是 Tribal 侧的主观意见——这是 Nous Research 的官方指导。
...
2. 将 refresh token 存储至 `~/.tribal/auth.json`
3. 在 `~/.tribal/config.yaml` 中将 Nous 设为推理提供商
4. 开启 Tool Gateway（网页、图像、TTS、浏览器路由）
5. 返回终端，即可运行 `tribal chat`
...
你现有的提供商配置保持不变。可以在会话中途用 `/model` 切换，或在会话间用 `tribal model` 切换——Portal 成为你的可用提供商之一，而非唯一选项。

### 无头环境 / SSH / 远程配置

OAuth 需要浏览器，但回调的 loopback 运行在 Tribal 所在的机器上。对于远程主机，请参阅 [OAuth over SSH / 远程主机](/guides/oauth-over-ssh)——与其他基于 OAuth 的提供商相同的方式同样适用于 Portal（`ssh -L` 端口转发，或在 Cloud Shell / Codespaces 等纯浏览器环境中使用 `--manual-paste`）。

### Profile 配置

如果你使用 [Tribal profiles（配置文件）](/user-guide/profiles)，Portal 的 refresh token 会通过共享令牌存储自动在所有 profile 间共享。在任意 profile 上登录一次，其余 profile 自动获取——无需为每个 profile 重复 OAuth 流程。
...
```bash
tribal portal status     # 登录状态、订阅信息、模型与 gateway 路由
tribal portal tools      # 详细的 Tool Gateway 目录及每个工具的路由信息
tribal portal open       # 在浏览器中打开订阅管理页面
```

`tribal portal status`（或直接 `tribal portal`）给出高层概览：
...
如果你已有 Browserbase 账号并希望继续使用，同时通过 Nous 路由网页搜索和图像生成，这是支持的。使用 `tribal tools` 为每个工具单独选择后端：
...
- **CLI 快捷方式：** `tribal portal open`（在默认浏览器中打开同一页面）

## 配置参考

运行 `tribal setup --portal` 后，`~/.tribal/config.yaml` 将如下所示：
...
OAuth refresh token 单独存储在 `~/.tribal/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。

## 令牌处理

Tribal 在每次推理调用时从存储的 Portal refresh token 生成短期 JWT，而非复用长期 API 密钥。令牌生命周期完全自动管理——刷新、生成、在瞬时 401 时重试——你无需关心这些细节。

如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Tribal 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `tribal auth add nous` 重新登录；隔离状态在下次成功登录时自动清除。

## 故障排查

### `tribal portal status` 显示"not logged in"
...
或使用 `tribal model` 重新选择 Nous Portal。

### 会话中途收到"需要重新认证"提示

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `tribal auth add nous`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/Tribal/tribal/issues)——我们将 Portal 目录同步至 Tribal，缺口通常意味着可以更新的路由配置。

### 账单未出现在我的 Portal 账号中

先检查 `tribal portal status`——如果显示你正在使用其他提供商（`Model: currently openrouter` 而非 `using Nous as inference provider`），说明本地配置已偏离。运行 `tribal model`，选择 Nous Portal，下一次请求将通过你的订阅路由。

## 另请参阅

- **[Tool Gateway](/user-guide/features/tool-gateway)** —— 每个 gateway 工具的完整详情、每工具配置及定价
- **[订阅代理](/user-guide/features/subscription-proxy)** —— 在非 Tribal 工具（其他 agent、脚本、第三方客户端）中使用你的 Portal 订阅
...
- **[Profiles](/user-guide/profiles)** —— 多个 Tribal 配置共享一个 Portal 登录