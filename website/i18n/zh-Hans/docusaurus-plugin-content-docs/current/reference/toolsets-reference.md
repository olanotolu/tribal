...
sidebar_position: 4
title: "工具集参考"
description: "Tribal 核心、复合、平台及动态工具集参考"
---
...
- **平台（Platform）** — 针对特定部署环境的完整工具配置（例如，`tribal-cli` 是交互式 CLI 会话的默认配置）
...
| `code_execution` | `execute_code` | 运行以编程方式调用 Tribal 工具的 Python 脚本。 |
...
| `discord` | `discord` | 核心 Discord 文本/嵌入/私信操作（仅限 gateway）。在 `tribal-discord` 工具集上激活。 |
| `discord_admin` | `discord_admin` | Discord 管理操作（封禁、角色变更、频道管理）。在 `tribal-discord` 工具集上激活；需要 bot 持有相关 Discord 权限。 |
| `feishu_doc` | `feishu_doc_read` | 读取飞书/Lark 文档内容。由飞书文档评论智能回复处理器使用。 |
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | 飞书/Lark 云盘评论操作。仅限评论 agent 使用；不在 `tribal-cli` 或其他消息工具集上暴露。 |
...
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | 多 agent 协调工具。为调度器生成的任务工作者（`TRIBAL_KANBAN_TASK`）以及显式启用 `kanban` 工具集的 profile 注册。工作者可标记任务完成、阻塞、心跳、评论以及创建/关联后续任务；编排器 profile 还额外获得看板路由工具，如 list/unblock。 |
...
| `x_search` | `x_search` | 通过 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子和话题。默认关闭；通过 `tribal tools` 启用。仅在配置了 xAI 凭据（SuperGrok OAuth 或 `XAI_API_KEY`）时注册 schema。 |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | 元宝私信/群组操作和表情包搜索。仅在 `tribal-yuanbao` 上注册。 |

## 平台工具集

平台工具集定义了部署目标的完整工具配置。大多数消息平台使用与 `tribal-cli` 相同的配置：

| 工具集 | 与 `tribal-cli` 的差异 |
|--------|------------------------|
| `tribal-cli` | 完整工具集——交互式 CLI 会话的默认配置。包含 file、terminal、web、browser、memory、skills、vision、image_gen、todo、tts、delegation、code_execution、cronjob、session_search、clarify 和 `safe`（只读）套件，以及标准消息工具。 |
| `tribal-acp` | 移除了 `clarify`、`cronjob`、`image_generate`、`send_message`、`text_to_speech` 以及全部四个 Home Assistant 工具。专注于 IDE 环境中的编码任务。 |
| `tribal-api-server` | 移除了 `clarify`、`send_message` 和 `text_to_speech`。保留其他所有工具——适用于无法进行用户交互的程序化访问场景。 |
| `tribal-cron` | 与 `tribal-cli` 相同。 |
| `tribal-telegram` | 与 `tribal-cli` 相同。 |
| `tribal-discord` | 在 `tribal-cli` 基础上添加了 `discord` 和 `discord_admin`。 |
| `tribal-slack` | 与 `tribal-cli` 相同。 |
| `tribal-whatsapp` | 与 `tribal-cli` 相同。 |
| `tribal-signal` | 与 `tribal-cli` 相同。 |
| `tribal-matrix` | 与 `tribal-cli` 相同。 |
| `tribal-mattermost` | 与 `tribal-cli` 相同。 |
| `tribal-email` | 与 `tribal-cli` 相同。 |
| `tribal-sms` | 与 `tribal-cli` 相同。 |
| `tribal-bluebubbles` | 与 `tribal-cli` 相同。 |
| `tribal-dingtalk` | 与 `tribal-cli` 相同。 |
| `tribal-feishu` | 添加了五个 `feishu_doc_*` / `feishu_drive_*` 工具（仅由文档评论处理器使用，不用于常规聊天适配器）。 |
| `tribal-qqbot` | 与 `tribal-cli` 相同。 |
| `tribal-wecom` | 与 `tribal-cli` 相同。 |
| `tribal-wecom-callback` | 与 `tribal-cli` 相同。 |
| `tribal-weixin` | 与 `tribal-cli` 相同。 |
| `tribal-yuanbao` | 在 `tribal-cli` 基础上添加了五个 `yb_*` 工具（私信/群组/表情包）。 |
| `tribal-homeassistant` | 与 `tribal-cli` 相同（Home Assistant 工具默认已存在，在设置 `HASS_TOKEN` 时激活）。 |
| `tribal-webhook` | 与 `tribal-cli` 相同。 |
| `tribal-gateway` | 内部 gateway 编排器工具集——所有 `tribal-<platform>` 工具集的并集；当 gateway 需要接受任意消息来源时使用。 |
...
`tribal tools` 命令提供基于 curses 的 UI，用于按平台切换单个工具的启用/禁用状态。该操作在工具级别进行（比工具集更细粒度），并持久化到 `config.yaml`。即使工具集已启用，被禁用的工具也会被过滤掉。
