...
sidebar_position: 1
title: "CLI 命令参考"
description: "Tribal 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Tribal profile（配置文件）。覆盖 `tribal profile use` 设置的粘性默认值。 |
...
| `--ignore-user-config` | 忽略 `~/.tribal/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、memory（记忆）和预加载 skill 的自动注入。 |
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `TRIBAL_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `tribal chat` | 与 agent 进行交互式或单次聊天。 |
| `tribal model` | 交互式选择默认 provider 和模型。 |
| `tribal fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `tribal gateway` | 运行或管理消息 gateway 服务。 |
| `tribal proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `tribal lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `tribal setup` | 全部或部分配置的交互式设置向导。 |
| `tribal whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `tribal slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `tribal auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `tribal login` / `logout` | **已弃用** — 请改用 `tribal auth`。 |
| `tribal status` | 显示 agent、auth 和平台状态。 |
| `tribal cron` | 检查并触发 cron 调度器。 |
| `tribal kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `tribal webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `tribal hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `tribal doctor` | 诊断配置和依赖问题。 |
| `tribal security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `tribal dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `tribal debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `tribal backup` | 将 Tribal 主目录备份为 zip 文件。 |
| `tribal checkpoints` | 检查/修剪/清除 `~/.tribal/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `tribal import` | 从 zip 文件恢复 Tribal 备份。 |
| `tribal logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `tribal config` | 显示、编辑、迁移和查询配置文件。 |
| `tribal pairing` | 审批或撤销消息配对码。 |
| `tribal skills` | 浏览、安装、发布、审计和配置 skill。 |
| `tribal bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `tribal curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `tribal memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `tribal honcho`）会自动注册。 |
| `tribal acp` | 将 Tribal 作为 ACP 服务器运行，用于编辑器集成。 |
| `tribal mcp` | 管理 MCP 服务器配置，并将 Tribal 作为 MCP 服务器运行。 |
| `tribal plugins` | 管理 Tribal Agent plugin（安装、启用、禁用、删除）。 |
| `tribal portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `tribal tools` | 按平台配置已启用的工具。 |
| `tribal computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `tribal sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `tribal insights` | 显示 token/费用/活动分析。 |
| `tribal claw` | OpenClaw 迁移辅助工具。 |
| `tribal dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `tribal profile` | 管理 profile——多个隔离的 Tribal 实例。 |
| `tribal completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `tribal version` | 显示版本信息。 |
| `tribal update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `TRIBAL_HOME` 进行快照。 |
| `tribal uninstall` | 从系统中删除 Tribal。 |
...
| `--ignore-user-config` | 忽略 `~/.tribal/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
### `tribal -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`tribal -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
单次运行覆盖（不修改 `~/.tribal/config.yaml`）：

| 标志 | 等效环境变量 | 用途 |
|---|---|---|
| `-m` / `--model <model>` | `TRIBAL_INFERENCE_MODEL` | 覆盖本次运行的模型 |
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `tribal chat -q`；`-z` 专门用于"我只需要最终答案"的场景。

## `tribal model`

交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 Tribal 聊天会话内部运行。
...
:::warning tribal model 与 /model——了解区别
**`tribal model`**（从终端运行，在任何 Tribal 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Tribal 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Tribal 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `tribal model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `tribal model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `TRIBAL_HOME`。当你并行运行多个 profile 并希望在 `tribal update` 后全部重启时很有用。 |
| `--no-supervise` | 在 `run` 时：在 s6-overlay Docker 镜像内部，跳过 s6 自动监管，退回到 pre-s6 前台语义——gateway 作为容器主进程运行，无自动重启。在 s6 镜像之外为空操作。等同于设置 `TRIBAL_GATEWAY_NO_SUPERVISE=1`。 |

:::tip WSL 用户
使用 `tribal gateway run` 而非 `tribal gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s tribal 'tribal gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-tribal-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `tribal setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `tribal setup --portal`。
...
```bash
tribal slack manifest              # 将 manifest 打印到 stdout
tribal slack manifest --write      # 写入 ~/.tribal/slack-manifest.json
tribal slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而非 stdout。裸 `--write` 写入 `$TRIBAL_HOME/slack-manifest.json`。 |
| `--name NAME` | `Tribal` | Slack 中的机器人显示名称。 |
| `--description DESC` | 默认简介 | Slack app 目录中显示的机器人描述。 |
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands`，用于合并到手动维护的 manifest 中。 |

`tribal update` 后重新运行 `tribal slack manifest --write` 以获取新增命令。


## `tribal login` / `tribal logout` *（已弃用）*

:::caution
`tribal login` 已被移除。请使用 `tribal auth` 管理 OAuth 凭据，使用 `tribal model` 选择 provider，或使用 `tribal setup` 进行完整的交互式设置。
:::
...
```bash
tribal auth                                              # 交互式向导
tribal auth list                                         # 显示所有池
tribal auth list openrouter                              # 显示特定 provider
tribal auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
tribal auth add anthropic --type oauth                   # 添加 OAuth 凭据
tribal auth remove openrouter 2                          # 按索引删除
tribal auth reset openrouter                             # 清除冷却时间
tribal auth status anthropic                             # 显示某 provider 的认证状态
tribal auth logout anthropic                             # 登出并清除已存储的认证状态
tribal auth spotify                                      # 通过 PKCE 将 Tribal 与 Spotify 认证
```
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.tribal/kanban.db`（向后兼容）；其他看板位于 `~/.tribal/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。

**全局标志（适用于以下所有操作）：**

| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `tribal kanban boards switch`、`TRIBAL_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `tribal kanban`。Worker 的环境中固定了 `TRIBAL_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.tribal/kanban/current`）。 |
...
tribal kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `TRIBAL_KANBAN_BOARD` 环境变量 → `~/.tribal/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/tribal-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
订阅持久化到 `~/.tribal/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
输出整个 Tribal 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | Tribal 版本、发布日期、git commit hash |
| **Environment** | 操作系统、Python 版本、OpenAI SDK 版本 |
| **Identity** | 活跃 profile 名称、TRIBAL_HOME 路径 |
...
:::tip
`tribal dump` 专为分享而设计。交互式诊断请使用 `tribal doctor`。可视化概览请使用 `tribal status`。
:::
...
报告包含系统信息（操作系统、Python 版本、Tribal 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
```bash
tribal debug share              # 上传调试报告，打印 URL
tribal debug share --lines 500  # 包含更多日志行
tribal debug share --expire 30  # 粘贴保留 30 天
tribal debug share --local      # 在终端打印报告（不上传）
```
...
创建 Tribal 配置、skill、会话和数据的 zip 归档。备份不包含 tribal-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/tribal-backup-<timestamp>.zip`）。 |
| `-q`, `--quick` | 快速快照：仅包含关键状态文件（config.yaml、state.db、.env、auth、cron 任务）。比完整备份快得多。 |
| `-l`, `--label <name>` | 快照标签（仅与 `--quick` 配合使用）。 |

备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 Tribal 正在运行也能正确工作（WAL 模式安全）。
...
- `tribal-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
tribal backup                           # 完整备份到 ~/tribal-backup-*.zip
tribal backup -o /tmp/tribal.zip        # 完整备份到指定路径
tribal backup --quick                   # 仅状态快速快照
tribal backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
检查和管理 `~/.tribal/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `tribal checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
tribal checkpoints                                  # 状态概览
tribal checkpoints prune --retention-days 3         # 激进清理
tribal checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
tribal checkpoints clear-legacy -f                  # 删除 v1 归档目录
tribal checkpoints clear -f                         # 清除所有内容
```
...
将之前创建的 Tribal 备份恢复到 Tribal 主目录。归档中的所有文件会覆盖 Tribal 主目录中的现有文件；`--force` 仅跳过当目标已有 Tribal 安装时触发的确认提示。
...
### 示例
```bash
tribal import ~/tribal-backup-20260423.zip           # 覆盖现有配置前提示确认
tribal import ~/tribal-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Tribal 日志文件。所有日志存储在 `~/.tribal/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Tribal 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`tribal logs list` 子命令显示所有日志文件，包括已轮转的。
...
tribal skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
tribal skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
tribal skills check
...
- `--source well-known` 允许你将 Tribal 指向暴露 `/.well-known/skills/index.json` 的站点。
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.tribal/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.tribal/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.tribal/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.tribal/skills/`（默认使用最新快照） |
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `tribal update` 后的第一次 tick 时立即执行 curator。使用 `tribal curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `tribal model` 相同的选择器）并追加到链末尾 |
...
检查 `~/.tribal/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.tribal/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `tribal <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `tribal honcho`）。未激活的 provider 不暴露其子命令。运行 `tribal --help` 查看当前已连接的命令。
:::
...
将 Tribal 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 Tribal 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Tribal 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Tribal 中使用 MCP](../guides/use-mcp-with-tribal.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-tribal-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `tribal plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Tribal Plugin](../guides/build-a-tribal-plugin.md)。
...
`tribal computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `tribal tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`tribal update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Tribal 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Tribal。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.tribal`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 Tribal 文件（默认：当计划有冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要显式指定。 |
| `--no-backup` | 跳过迁移前对 `~/.tribal/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.tribal/backups/pre-migration-*.zip`；可用 `tribal import` 恢复）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 Tribal 等效项，要么**归档**以供手动审查。
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install tribal-agent[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页需要 `--tui` 加上 `pty` extra。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过 PTY/WebSocket 桥接在后台运行 `tribal --tui`，启用浏览器内 Chat 标签页。需要 `pip install 'tribal-agent[web,pty]'` 以及 Linux、macOS 或 WSL2 等 POSIX PTY 环境。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。会在网络上暴露控制台凭据；仅在受信任的网络控制下使用。 |
| `--stop` | — | 停止正在运行的 `tribal dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `tribal dashboard` 进程并退出。 |
...
管理 profile——多个隔离的 Tribal 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 Tribal 命令、子命令和 profile 名称进行 Tab 补全。
...
拉取最新的 `tribal-agent` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `tribal update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade tribal-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。

| 选项 | 说明 |
|--------|-------------|
| `--check` | 并排打印当前 commit 和最新 `origin/main` commit，同步时退出码为 0，落后时为 1。不拉取、不安装、不重启任何内容。 |
| `--backup` | 在拉取前创建 `TRIBAL_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后重启正在运行的 gateway 服务。如果安装了多个 profile，隐含 `--all` 语义。 |

附加行为：

- **配对数据快照。** 即使 `--backup` 关闭，`tribal update` 也会在 `git pull` 前对 `~/.tribal/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `tribal backup restore --state pre-update` 回滚。
- **旧版 `tribal.service` 警告。** 如果 Tribal 检测到预重命名的 `tribal.service` systemd 单元（而非当前的 `tribal-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| 命令 | 说明 |
|---------|-------------|
| `tribal version` | 打印版本信息。 |
| `tribal update` | 拉取最新变更并重新安装依赖。 |
| `tribal uninstall [--full] [--yes]` | 删除 Tribal，可选择删除所有 config/数据。 |
