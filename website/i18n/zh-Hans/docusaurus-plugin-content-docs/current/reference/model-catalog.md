...
Tribal 从托管于文档站点旁的 JSON 清单中获取 **OpenRouter** 和 **Nous Portal** 的精选模型列表。这样维护者无需发布新的 `tribal-agent` 版本即可更新选择器列表。

当清单不可达时（离线、网络受阻、托管故障），Tribal 会静默回退到随 CLI 一同发布的仓库内置快照。清单永远不会导致选择器崩溃——最坏情况下，你看到的是与已安装版本捆绑的列表。
...
- **`version`** — 整数类型的 schema 版本号。未来的 schema 会递增此值；Tribal 拒绝处理版本号未知的清单，并回退到硬编码快照。
- **`metadata`** — 清单、provider 及模型级别的自由格式字典，支持任意键。Tribal 会忽略未知字段，因此你可以为条目添加注解（如 `"tier": "paid"`、`"tags": [...]` 等），无需协调 schema 变更。
...
| 时机 | 行为 |
|---|---|
| `/model` 或 `tribal model` | 若磁盘缓存已过期则重新获取，否则使用缓存 |
| 磁盘缓存新鲜（< TTL） | 不发起网络请求 |
| 网络故障且有缓存 | 静默回退到缓存，输出一行日志 |
| 网络故障且无缓存 | 静默回退到仓库内置快照 |
| 清单未通过 schema 校验 | 视为不可达 |

缓存位置：`~/.tribal/cache/model_catalog.json`。
...
```bash
# 从仓库内硬编码列表重新生成（在编辑 tribal_cli/models.py 中的
