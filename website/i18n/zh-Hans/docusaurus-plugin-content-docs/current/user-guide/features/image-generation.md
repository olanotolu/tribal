...
description: 通过 FAL.ai 文生图；支持 8 个模型，含 FLUX 2、GPT-Image、Nano Banana Pro、Ideogram、Recraft V4 Pro 等，可用 tribal tools 切换。
sidebar_label: 文生图
sidebar_position: 6
---
...
Tribal Agent 通过 FAL.ai 根据文字提示生成图像。默认内置 8 个模型，在速度、画质与成本上各有取舍。当前模型可通过 `tribal tools` 配置，并持久化在 `config.yaml`。
