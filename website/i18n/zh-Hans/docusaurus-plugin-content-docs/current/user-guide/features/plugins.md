...
title: "Plugins"
description: "通过插件系统为 Tribal 添加自定义工具、hook 和集成"
---

# Plugins

Tribal 提供了一套插件系统，可在不修改核心代码的情况下添加自定义工具、hook（钩子）和集成。

如果你想为自己、团队或某个项目创建自定义工具，这通常是正确的路径。开发者指南中的
[Adding Tools](/developer-guide/adding-tools) 页面针对的是存放在 `tools/` 和 `toolsets.py` 中的 Tribal 内置核心工具。

**→ [构建 Tribal Plugin](/guides/build-a-tribal-plugin)** — 包含完整可运行示例的分步指南。

## 快速概览

在 `~/.tribal/plugins/` 下放入一个目录，包含 `plugin.yaml` 和 Python 代码：
...
启动 Tribal — 你的工具会与内置工具一同出现，模型可立即调用它们。
...
将两个文件放入 `~/.tribal/plugins/hello-world/`，重启 Tribal，模型即可立即调用 `hello_world`。每次工具调用后，hook 会打印一行日志。

`./.tribal/plugins/` 下的项目本地插件默认禁用。仅对可信仓库启用，方法是在启动 Tribal 前设置 `TRIBAL_ENABLE_PROJECT_PLUGINS=true`。
...
| 添加 CLI 命令 | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — 添加 `tribal <plugin> <subcommand>` |
...
| 按环境变量控制 | 在 plugin.yaml 中设置 `requires_env: [API_KEY]` — 在 `tribal plugins install` 时提示输入 |
| 通过 pip 分发 | `[project.entry-points."tribal_agent.plugins"]` |
...
| 来源 | 路径 | 使用场景 |
|--------|------|----------|
| 内置 | `<repo>/plugins/` | 随 Tribal 附带 — 参见 [Built-in Plugins](/user-guide/features/built-in-plugins) |
| 用户 | `~/.tribal/plugins/` | 个人插件 |
| 项目 | `.tribal/plugins/` | 项目专属插件（需要 `TRIBAL_ENABLE_PROJECT_PLUGINS=true`） |
| pip | `tribal_agent.plugins` entry_points | 分发包 |
| Nix | `services.tribal-agent.extraPlugins` / `extraPythonPackages` | NixOS 声明式安装 — 参见 [Nix Setup](/getting-started/nix-setup#plugins) |

名称冲突时，后面的来源会覆盖前面的，因此与内置插件同名的用户插件会替换它。

### 插件子分类

在每个来源内，Tribal 还识别将插件路由到专用发现系统的子分类目录：
...
`~/.tribal/plugins/model-providers/<name>/` 和 `~/.tribal/plugins/memory/<name>/` 下的用户插件会覆盖同名内置插件 — `register_provider()` / `register_memory_provider()` 中后写者胜出。放入一个目录即可替换内置实现，无需修改仓库。

子分类插件在 `tribal plugins list` 和交互式 `tribal plugins` UI 中以**路径派生的 key** 显示 — 例如 `observability/langfuse`、`image_gen/openai`、`platforms/teams`。该 key（而非 manifest 中的 `name:`）是传给 `tribal plugins enable …` / `disable …` 的值，也是在 `config.yaml` 的 `plugins.enabled` 下填写的字符串。

## 插件默认关闭（少数例外）

**通用插件和用户安装的后端默认禁用** — 发现系统会找到它们（因此它们会出现在 `tribal plugins` 和 `/plugins` 中），但在你将插件名称添加到 `~/.tribal/config.yaml` 的 `plugins.enabled` 之前，任何带有 hook 或工具的内容都不会加载。这可防止第三方代码在未经明确同意的情况下运行。
...
```bash
tribal plugins                    # 交互式切换（空格勾选/取消勾选）
tribal plugins enable <name>      # 添加到允许列表
tribal plugins disable <name>     # 从允许列表移除并添加到禁用列表
```

执行 `tribal plugins install owner/repo` 后，会询问 `Enable 'name' now? [y/N]` — 默认为否。脚本化安装时可用 `--enable` 或 `--no-enable` 跳过提示。

### 允许列表不控制的内容

某些类别的插件绕过 `plugins.enabled` — 它们是 Tribal 内置功能的一部分，若默认关闭会破坏基本功能：
...
| **用户安装的平台**（位于 `~/.tribal/plugins/platforms/`） | 通过 `plugins.enabled` 选择加入 — 第三方 gateway 适配器需要明确同意。 |

简而言之：**内置的"始终可用"基础设施自动加载；第三方通用插件需选择加入。** `plugins.enabled` 允许列表专门用于控制用户放入 `~/.tribal/plugins/` 的任意代码。

### 现有用户的迁移

当你升级到支持选择加入插件的 Tribal 版本（config schema v21+）时，已安装在 `~/.tribal/plugins/` 下且不在 `plugins.disabled` 中的用户插件会**自动纳入** `plugins.enabled`。你的现有配置继续正常工作。内置独立插件**不会**自动纳入 — 即使是现有用户也需要明确选择加入。（内置平台/后端插件从未需要纳入，因为它们从未被控制。）
...
| 类型 | 作用 | 选择方式 | 位置 |
|------|-------------|-----------|----------|
| **通用插件** | 添加工具、hook、斜杠命令、CLI 命令 | 多选（启用/禁用） | `~/.tribal/plugins/` |
...
上表展示了四种插件类别，但在"通用插件"中，`PluginContext` 暴露了多个不同的扩展点 — Tribal 还接受 Python 插件系统之外的扩展（配置驱动的后端、shell hook 命令、外部服务器等）。使用下表找到适合你需求的文档：

| 想要添加… | 方式 | 编写指南 |
|---|---|---|
| LLM 可调用的**工具** | Python 插件 — `ctx.register_tool()` | [Build a Tribal Plugin](/guides/build-a-tribal-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| **生命周期 hook**（LLM 前后、会话开始/结束、工具过滤） | Python 插件 — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Tribal Plugin](/guides/build-a-tribal-plugin) |
| CLI / gateway 的**斜杠命令** | Python 插件 — `ctx.register_command()` | [Build a Tribal Plugin](/guides/build-a-tribal-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| `tribal <thing>` 的**子命令** | Python 插件 — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **STT 后端**（自定义 whisper 二进制、本地 ASR CLI） | 配置驱动 — 将 `TRIBAL_LOCAL_STT_COMMAND` 环境变量设置为 shell 模板 | [Voice Message Transcription (STT)](/user-guide/features/tts#voice-message-transcription-stt) |
| **通过 MCP 使用外部工具**（文件系统、GitHub、Linear、Notion、任意 MCP 服务器） | 配置驱动 — 在 `config.yaml` 中以 `command:` / `url:` 声明 `mcp_servers.<name>`。Tribal 自动发现服务器的工具并与内置工具一同注册。 | [MCP](/user-guide/features/mcp) |
| **额外 skill 来源**（自定义 GitHub 仓库、私有 skill 索引） | CLI — `tribal skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [发布自定义 tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway 事件 hook**（在 `gateway:startup`、`session:start`、`agent:end`、`command:*` 时触发） | 将 `HOOK.yaml` + `handler.py` 放入 `~/.tribal/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
在 NixOS 上，插件可通过模块选项声明式安装 — 无需 `tribal plugins install`。完整详情请参见 **[Nix Setup 指南](/getting-started/nix-setup#plugins)**。
...
```bash
tribal plugins                                       # 统一交互式 UI
tribal plugins list                                  # 表格：已启用 / 已禁用 / 未启用
tribal plugins install user/repo                     # 从 Git 安装，然后提示 Enable? [y/N]
tribal plugins install user/repo --enable            # 安装并启用（无提示）
tribal plugins install user/repo --no-enable         # 安装但保持禁用（无提示）
tribal plugins update my-plugin                      # 拉取最新版本
tribal plugins remove my-plugin                      # 卸载
tribal plugins enable my-plugin                      # 添加到允许列表（普通插件）
tribal plugins enable observability/langfuse         # 添加到允许列表（子分类插件）
tribal plugins disable my-plugin                     # 从允许列表移除并添加到禁用列表
```

对于子分类目录下的插件（例如 `plugins/observability/langfuse/`、`plugins/image_gen/openai/`），使用完整的 `<category>/<plugin>` key — 这正是 `tribal plugins list` 在 **Name** 列中显示的内容。

### 交互式 UI

不带参数运行 `tribal plugins` 会打开一个复合交互界面：
...
新安装或内置插件的默认状态为 `not enabled`。`tribal plugins list` 显示全部三种状态，便于区分明确关闭的插件和等待启用的插件。
...
完整的处理器约定、schema 格式、hook 行为、错误处理和常见错误请参见 **[完整指南](/guides/build-a-tribal-plugin)**。