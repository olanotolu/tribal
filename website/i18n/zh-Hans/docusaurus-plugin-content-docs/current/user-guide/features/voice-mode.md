...
sidebar_position: 10
title: "语音模式"
description: "与 Tribal Agent 进行实时语音对话 — CLI、Telegram、Discord（私信、文字频道和语音频道）"
---

# 语音模式

Tribal Agent 支持在 CLI 和消息平台上进行完整的语音交互。通过麦克风与 Agent 对话，听取语音回复，并在 Discord 语音频道中进行实时语音对话。

如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Tribal 的语音模式](/guides/use-voice-mode-with-tribal)。

## 前提条件

使用语音功能前，请确保已完成以下准备：

1. **已安装 Tribal Agent** — `pip install tribal-agent`（参见 [安装](/getting-started/installation)）
2. **已配置 LLM 提供商** — 运行 `tribal model` 或在 `~/.tribal/.env` 中设置首选提供商的凭据
3. **基础设置正常** — 运行 `tribal` 验证 Agent 能够响应文字消息，再启用语音功能

:::tip
`~/.tribal/` 目录和默认的 `config.yaml` 会在首次运行 `tribal` 时自动创建。只需手动创建 `~/.tribal/.env` 来存放 API 密钥。
:::

:::tip Nous Portal 同时覆盖两项
付费的 [Nous Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`tribal setup --portal` 可一次性完成两项配置。
:::
...
语音模式在**经典 CLI**（`tribal chat`）和 **TUI**（`tribal --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.tribal/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
1. 使用 `tribal` 启动 CLI，并通过 `/voice on` 启用语音模式
...
:::tip
录音键可通过 `~/.tribal/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
:::
...
```bash
tribal gateway        # 启动 gateway（连接到已配置的平台）
tribal gateway setup  # 首次配置的交互式设置向导
```
...
**服务器频道：** Bot 仅在被 @提及时响应（例如 `@tribalbyt4 你好`）。请确保从提及弹窗中选择 **Bot 用户**，而非同名角色。

:::tip
如需在服务器频道中禁用提及要求，在 `~/.tribal/.env` 中添加：
```bash
...
2. 或改用私信 — 无需提及
3. 或在 `~/.tribal/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 检查 LLM 模型是否已配置且可访问
- 查看 gateway 日志：`tail -f ~/.tribal/logs/gateway.log`
