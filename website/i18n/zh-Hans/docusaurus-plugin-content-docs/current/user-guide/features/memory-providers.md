...
Tribal Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。

## 快速开始

```bash
tribal memory setup      # 交互式选择器 + 配置
tribal memory status     # 查看当前激活状态
tribal memory off        # 禁用外部提供者
```

也可以通过 `tribal plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。

或在 `~/.tribal/config.yaml` 中手动设置：
...
**安装向导：**
```bash
tribal memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `tribal honcho setup` 命令仍然有效（现在会重定向到 `tribal memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$TRIBAL_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$TRIBAL_HOME/honcho.json` > `~/.tribal/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/tribal-ai/tribal-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/tribal)。
...
:::tip 从 `tribal honcho` 迁移
如果你之前使用过 `tribal honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::

**多 peer 配置：**

Honcho 将对话建模为 peer 之间的消息交换——每个 Tribal profile 对应一个用户 peer 加一个 AI peer，共享同一个 workspace。workspace 是共享环境：用户 peer 在各 profile 间全局共享，每个 AI peer 拥有独立身份。每个 AI peer 从自身的观察中独立构建表示/card，因此 `coder` profile 保持代码导向，而 `writer` profile 针对同一用户保持编辑导向。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **Workspace** | 共享环境。同一 workspace 下的所有 Tribal profile 共享同一用户身份。 |
| **用户 peer**（`peerName`） | 人类用户。在 workspace 内跨 profile 共享。 |
| **AI peer**（`aiPeer`） | 每个 Tribal profile 一个。host key `tribal` → 默认；其他 profile 使用 `tribal.<profile>`。 |
...
`--clone` 在 `honcho.json` 中创建一个 `tribal.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
扫描所有 Tribal profile，为没有 host 块的 profile 创建 host 块，从默认 `tribal` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/tribal-ai/tribal-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/tribal)。
...
# 然后配置 Tribal
tribal memory setup    # 选择 "openviking"
# 或手动配置：
...
**安装：**
```bash
tribal memory setup    # 选择 "mem0"
# 或手动配置：
tribal config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.tribal/.env
```

**配置：** `$TRIBAL_HOME/mem0.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `tribal-user` | 用户标识符 |
| `agent_id` | `tribal` | Agent 标识符 |
...
**安装：**
```bash
tribal memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p tribal ui start`

**配置：** `$TRIBAL_HOME/hindsight/config.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` 或 `local` |
| `bank_id` | `tribal` | 记忆库标识符 |
...
| `retain_context` | `conversation between Tribal Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/Tribal/tribal/blob/main/plugins/memory/hindsight/README.md)。
...
**安装：**
```bash
tribal memory setup    # 选择 "holographic"
# 或手动配置：
tribal config set memory.provider holographic
```

**配置：** `plugins.tribal-memory-store` 下的 `config.yaml`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$TRIBAL_HOME/memory_store.db` | SQLite 数据库路径 |
...
**安装：**
```bash
tribal memory setup    # 选择 "retaindb"
# 或手动配置：
...
# 然后配置 Tribal
tribal memory setup    # 选择 "byterover"
# 或手动配置：
...
- 知识树存储于 `$TRIBAL_HOME/byterover/`（profile 范围隔离）
...
**安装：**
```bash
tribal memory setup    # 选择 "supermemory"
# 或手动配置：
tribal config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.tribal/.env
```

**配置：** `$TRIBAL_HOME/supermemory.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `container_tag` | `tribal` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `tribal-{identity}` → `tribal-coder`），按 Tribal profile 隔离记忆
...
- **本地存储提供者**（Holographic、ByteRover）使用 `$TRIBAL_HOME/` 路径，各 profile 路径不同
- **配置文件提供者**（Honcho、Mem0、Hindsight、Supermemory）将配置存储在 `$TRIBAL_HOME/` 中，每个 profile 拥有独立凭证
