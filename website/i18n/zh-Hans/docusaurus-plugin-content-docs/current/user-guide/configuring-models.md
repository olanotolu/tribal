...
[Nous Portal](/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `tribal setup --portal` 即可登录并一键将 Nous 设为提供商。使用 `tribal portal status` 查看当前配置。
:::
...
- **右列** — 所选提供商的精选模型列表。这些是 Tribal 针对该提供商推荐的 agentic 模型，而非原始的 `/models` 接口返回结果（OpenRouter 的原始列表包含 400+ 个模型，涵盖 TTS、图像生成器和重排序器）。

在过滤框中输入提供商名称、slug 或模型 ID 进行筛选。

选择模型后点击 **Switch**，Tribal 会将其写入 `~/.tribal/config.yaml` 的 `model` 部分。**此操作仅对新会话生效** — 已打开的聊天标签页将继续使用启动时的模型。如需在当前聊天中热切换，请在聊天内使用 `/model` 斜杠命令。
...
每个辅助任务默认为 `auto`，即 Tribal 对该任务也使用主模型。当某个边缘任务需要更便宜或更快的模型时，可单独覆盖该槽位。
...
| **Skills Hub（技能中心）** | `tribal skills search` 使用此槽位。通常保持 `auto` 即可。 |
...
通过仪表板保存时，Tribal 写入 `~/.tribal/config.yaml`：
...
`provider: auto` 加 `model: ''` 表示 Tribal 对该任务使用主模型。

## 何时生效？

- **CLI**（`tribal chat`）：下次执行 `tribal chat` 时生效。
- **Gateway**（Telegram、Discord、Slack 等）：下一个*新*会话生效。现有会话保持原有模型。如需强制所有会话使用新配置，重启 gateway（`tribal gateway restart`）。
...
Tribal 仅列出具有有效凭据的提供商。检查侧边栏中的 **Keys** — 应存在以下之一：API key、成功的 OAuth 或自定义端点 URL。若所需提供商不在列表中，运行 `tribal setup` 进行配置，或前往 **Keys** 添加环境变量。
...
```bash
tribal model            # 交互式提供商 + 模型选择器（切换默认值的标准方式）
```

`tribal model` 引导你选择提供商、完成认证（OAuth 流程会打开浏览器；API key 提供商会提示输入密钥），然后从该提供商的精选目录中选择具体模型。选择结果写入 `~/.tribal/config.yaml` 的 `model.provider` 和 `model.model` 字段。

如需在不启动选择器的情况下列出提供商/模型，请使用仪表板或下方的 REST 端点。查看 CLI 当前实际使用的配置：`tribal config get model` 和 `tribal status`。

### 直接编辑配置文件

编辑 `~/.tribal/config.yaml` 后重启相关服务。完整 schema 请见[配置参考](./configuration.md)。
...
session token 在启动时注入仪表板 HTML，每次服务器重启后轮换。如需对运行中的仪表板编写脚本，可从浏览器开发者工具中获取（`window.__TRIBAL_SESSION_TOKEN__`）。