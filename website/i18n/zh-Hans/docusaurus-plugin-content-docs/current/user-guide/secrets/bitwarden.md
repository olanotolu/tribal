...
在进程启动时从 [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) 拉取 API 密钥，而不是以明文形式存储在 `~/.tribal/.env` 中。一个引导密钥（机器账户访问令牌）替代了 N 个提供商密钥，轮换凭据只需在 Bitwarden Web 应用中修改一次即可。

## 工作原理

1. 在 Bitwarden Secrets Manager 中创建一个**机器账户**，授予其对某个项目的读取权限，并生成一个**访问令牌**。
2. Tribal 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.tribal/.env` 中。
3. 每次 `tribal`（或 gateway，或 cron 任务）启动时，在加载 `~/.tribal/.env` 之后，Tribal 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
4. 默认情况下，Tribal **覆盖**环境中已有的值，因此 Bitwarden 是唯一可信来源——在 Web 应用中轮换一次密钥，每个 Tribal 进程在下次启动时即可获取最新值。如果希望 `.env` 优先，可在配置中将 `override_existing: false`。

`bws` 二进制文件在首次使用时会自动下载到 `~/.tribal/bin/`，无需 `apt`、`brew` 或 `sudo`。
...
2. 创建或选择一个**项目**（例如"Tribal keys"）。
3. 将提供商密钥添加为 secret。secret 的**名称**将成为环境变量名——使用 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等。
4. **Machine accounts → New machine account → My Tribal machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
1. 下载并验证 `bws v2.0.0`，存放至 `~/.tribal/bin/bws`。
2. 提示输入访问令牌（输入内容隐藏）。以 `BWS_ACCESS_TOKEN` 形式存储在 `~/.tribal/.env` 中。
...
此后，每次调用 `tribal` 都会在启动时拉取最新 secret。进程中首次应用 secret 时，stderr 会显示一行摘要信息。

## CLI

| 命令 | 功能 |
|---|---|
| `tribal secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `tribal secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
| `tribal secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `tribal secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `tribal secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `tribal secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |

## 配置

`~/.tribal/config.yaml` 中的默认值：
...
| `cache_ttl_seconds` | `300` | 进程内拉取结果的复用时长。设为 `0` 可禁用缓存。缓存按进程隔离；新的 `tribal` 调用从头开始。 |
| `override_existing` | `true` | 为 true 时，Bitwarden 的值会覆盖环境中已有的任何值（使 Web 应用中的轮换真正生效）。如果希望本地 `.env` / shell 导出优先，设为 `false`。 |
| `auto_install` | `true` | 为 true 时，首次使用时自动将 `bws` 下载到 `~/.tribal/bin/`。 |

## 故障模式

Bitwarden 永远不会阻塞 Tribal 启动。如果出现任何问题，stderr 会显示一行警告，Tribal 继续使用 `.env` 中已有的凭据：

| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `tribal secrets bitwarden setup` |
...
- 即使 `override_existing: true`，Tribal 也会拒绝让 Bitwarden 覆盖引导令牌本身。如果你将 `BWS_ACCESS_TOKEN` 作为 secret 存储在项目中，应用时会静默跳过。
- `bws` 二进制文件的下载会与同一 GitHub release 中发布的 SHA-256 校验和进行验证。不匹配时将中止安装。
- 固定版本（撰写本文时为 `bws v2.0.0`）通过向本仓库提交 PR 的方式更新——Tribal 不会将 `bws` 自动升级到"最新版本"，因为上游 release 的结构可能发生变化。

## 不适用场景

- **单机个人使用**，`~/.tribal/.env` 已经够用。你只是用一个凭据换了另一个，并在启动时增加了网络依赖。
- **无法访问 `api.bitwarden.com` 的隔离环境**。
- **CI/CD** 场景，已有现成的 secret 注入机制（GitHub Actions secrets、Vault 等）——选择一种方式，不要两者并用。

适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 Tribal 安装进行集中轮换和吊销管理的场景。