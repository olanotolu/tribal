...
启动守护进程后，与你的 Agent 联系人开启一段对话。联系人 ID 将出现在会话日志中，或通过 `tribal send_message action=list` 查看。
...
2. 使用 **DM 配对**——向 Bot 发送任意消息，Bot 将回复一个配对码。通过 `tribal gateway pair` 输入该配对码。
...
- Tribal 与守护进程之间的连接为本地 WebSocket（`ws://127.0.0.1:5225`）——数据不会离开你的机器
