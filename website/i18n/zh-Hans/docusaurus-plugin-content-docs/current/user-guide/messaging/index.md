...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Tribal 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Tribal 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Tribal](/guides/use-voice-mode-with-tribal)。
...
```bash
tribal gateway setup        # 交互式配置所有消息平台
```
...
```bash
tribal gateway              # 在前台运行
tribal gateway setup        # 交互式配置消息平台
tribal gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo tribal gateway install --system   # 仅 Linux：安装开机启动的系统服务
tribal gateway start        # 启动默认服务
tribal gateway stop         # 停止默认服务
tribal gateway status       # 检查默认服务状态
tribal gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.tribal/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
tribal pairing list          # 查看待审核和已批准的用户
tribal pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Tribal 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.tribal/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.tribal/config.yaml` 中的 `display.background_process_notifications` 控制：
...
tribal gateway start                 # 启动服务
tribal gateway stop                  # 停止服务
tribal gateway status                # 检查状态
journalctl --user -u tribal-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Tribal 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Tribal 安装（使用不同的 `TRIBAL_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.tribal` 使用 `tribal-gateway`；其他安装使用 `tribal-gateway-<hash>`。`tribal gateway` 命令会自动针对当前 `TRIBAL_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
tribal gateway install               # 安装为 launchd agent
tribal gateway start                 # 启动服务
tribal gateway stop                  # 停止服务
tribal gateway status                # 检查状态
tail -f ~/.tribal/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.tribal.gateway.plist`。它包含三个环境变量：
...
- **TRIBAL_HOME** — 将网关限定到你的 Tribal 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `tribal gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `TRIBAL_HOME` 目录都有自己的 launchd 标签。默认的 `~/.tribal` 使用 `ai.tribal.gateway`；其他安装使用 `ai.tribal.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `tribal-cli` | 完全访问 |
| Telegram | `tribal-telegram` | 完整工具，包括终端 |
| Discord | `tribal-discord` | 完整工具，包括终端 |
| WhatsApp | `tribal-whatsapp` | 完整工具，包括终端 |
| Slack | `tribal-slack` | 完整工具，包括终端 |
| Google Chat | `tribal-google_chat` | 完整工具，包括终端 |
| Signal | `tribal-signal` | 完整工具，包括终端 |
| SMS | `tribal-sms` | 完整工具，包括终端 |
| Email | `tribal-email` | 完整工具，包括终端 |
| Home Assistant | `tribal-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `tribal-mattermost` | 完整工具，包括终端 |
| Matrix | `tribal-matrix` | 完整工具，包括终端 |
| DingTalk | `tribal-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `tribal-feishu` | 完整工具，包括终端 |
| WeCom | `tribal-wecom` | 完整工具，包括终端 |
| WeCom Callback | `tribal-wecom-callback` | 完整工具，包括终端 |
| Weixin | `tribal-weixin` | 完整工具，包括终端 |
| BlueBubbles | `tribal-bluebubbles` | 完整工具，包括终端 |
| QQBot | `tribal-qqbot` | 完整工具，包括终端 |
| Yuanbao | `tribal-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `tribal-teams` | 完整工具，包括终端 |
| API Server | `tribal-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `tribal-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.tribal/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
