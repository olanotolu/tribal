...
通过 [BlueBubbles](https://bluebubbles.app/) 将 Tribal 连接至 Apple iMessage——这是一款免费、开源的 macOS 服务端，可将 iMessage 桥接至任意设备。
...
- Tribal 与 BlueBubbles 服务端之间的网络连通性
...
**DM 配对（推荐）：**
当有人向你的 iMessage 发送消息时，Tribal 会自动向其发送配对码。使用以下命令批准：
```bash
tribal pairing approve bluebubbles <CODE>
```
使用 `tribal pairing list` 查看待处理的配对码和已授权用户。

**预授权特定用户**（在 `~/.tribal/.env` 中）：
```bash
...
Tribal 将连接至你的 BlueBubbles 服务端，注册 webhook，并开始监听 iMessage 消息。
...
- **出站：** Tribal 通过 BlueBubbles REST API 发送消息。
...
自动将消息标记为已读由 `~/.tribal/config.yaml` 中 `platforms.bluebubbles.extra` 下的 `send_read_receipts` 键控制（默认值：`true`）。该选项没有对应的环境变量。
...
### 聊天寻址
你可以通过邮箱或手机号寻址聊天——Tribal 会自动将其解析为 BlueBubbles 聊天 GUID，无需使用原始 GUID 格式。
...
- 查看 `tribal logs gateway` 中的 webhook 错误（或使用 `tribal logs -f` 实时跟踪）
