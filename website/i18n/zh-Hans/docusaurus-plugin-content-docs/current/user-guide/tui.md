...
sidebar_position: 2
title: "TUI"
description: "启动 Tribal 的现代终端 UI——支持鼠标操作、丰富的浮层面板和非阻塞输入。"
---

# TUI

TUI 是 Tribal 的现代前端——一个终端 UI（用户界面），与 [Classic CLI](cli.md) 共享同一 Python 运行时。相同的 agent、相同的会话、相同的斜杠命令；交互界面更简洁、响应更流畅。

这是以交互方式运行 Tribal 的推荐方式。
...
- **即时首帧** — banner 在应用加载完成前就已渲染，因此 Tribal 启动时终端不会出现卡顿感。
...
- **Node.js** ≥ 20 — TUI 作为从 Python CLI 启动的子进程运行。`tribal doctor` 会验证此项。
- **TTY** — 与 classic CLI 一样，通过管道传入 stdin 或在非交互式环境中运行时，将回退到单次查询模式。

首次启动时，Tribal 会将 TUI 的 Node 依赖安装到 `ui-tui/node_modules`（一次性操作，耗时数秒）。后续启动速度很快。拉取新版 Tribal 后，若源文件比 dist 更新，TUI bundle 将自动重新构建。

### 外部预构建

发行版若附带预构建 bundle（如 Nix、系统包），可将 Tribal 指向该 bundle：
...
| `/reload` | 将 `~/.tribal/.env` 重新读入运行中的 TUI 进程，使新添加的 API 密钥无需重启即可生效 |
...
1. `TRIBAL_TUI_THEME` 环境变量——最高优先级。可选值：`light`、`dark`，或原始 6 位背景十六进制色值（如 `ffffff`、`1a1a2e`）。
...
状态栏忙碌指示器可插拔——默认在 agent 工作期间每 2.5 秒轮换一次 Tribal 的 kawaii 表情调色板。通过配置或 `/indicator` 斜杠命令选择不同样式：
...
默认情况下，`tribal --tui` 每次启动都会开启新会话。若要自动重新连接到最近的 TUI 会话（在终端或 SSH 连接意外断开时很有用），可选择启用：

```bash
export TRIBAL_TUI_RESUME=1          # 最近的 TUI 会话
# 或：
export TRIBAL_TUI_RESUME=<session-id>   # 指定会话
```
...
- **工作目录及 git 分支** — `~/projects/tribal-agent (docs/two-week-gap-sweep)`。在旁边的终端执行 `git checkout` 时，分支后缀会更新（mtime 缓存），TUI 反映的是实际活跃分支，而非启动时的分支。
...
- **`⚠ YOLO`** — 每当 YOLO 模式开启时（`tribal --yolo`、`/yolo` 或 `TRIBAL_YOLO_MODE=1`）显示的可见警告。同一徽章也出现在启动 banner 中，确保你不会在未注意到的情况下启动自动审批会话。

## 配置

TUI 遵循所有标准 Tribal 配置：`~/.tribal/config.yaml`、profile、personality、skin、快捷命令、凭证池、内存提供商、工具/skill 启用状态。不存在 TUI 专属配置文件。
...
会话在 TUI 和 classic CLI 之间共享——两者均写入同一个 `~/.tribal/state.db`。可以在一个界面开始会话，在另一个界面恢复。会话选择器显示来自两个来源的会话，并带有来源标签。

会话生命周期、搜索、压缩和导出，请参阅[会话](sessions.md)。

## 连接到运行中的 gateway

默认情况下，TUI 会在进程内启动自己的 gateway，因此每个 TUI 实例是自包含的。如果你已有一个长期运行的 gateway（例如在 tmux 中运行 `tribal gateway run`，或 systemd / launchd 服务），可以将 TUI 指向该 gateway——TUI 将成为一个瘦客户端，与连接到同一 gateway 的所有其他界面（消息平台、Web 仪表板、其他 TUI 会话）共享状态。
...
不带 `--tui` 启动 `tribal` 将继续使用 classic CLI。若要让某台机器默认使用 TUI，在 shell profile 中设置 `TRIBAL_TUI=1`。若要回退，取消设置即可。

如果 TUI 启动失败（无 Node、缺少 bundle、TTY 问题），Tribal 会打印诊断信息并回退——而不是让你陷入困境。
