...
| | |
|---|---|
| 来源 | 可选 — 通过 `tribal skills install official/research/darwinian-evolver` 安装 |
| 路径 | `optional-skills/research/darwinian-evolver` |
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), Tribal Agent |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 Tribal 本身。
...
```bash
mkdir -p ~/.tribal/cache/darwinian-evolver && cd ~/.tribal/cache/darwinian-evolver
...
```bash
cd ~/.tribal/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.tribal/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.tribal/skills/research/darwinian-evolver
DE_DIR=~/.tribal/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 Tribal 核心中使用 `from darwinian_evolver import ...`。`~/.tribal/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
