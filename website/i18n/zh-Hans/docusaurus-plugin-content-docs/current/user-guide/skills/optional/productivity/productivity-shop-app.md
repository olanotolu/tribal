...
| | |
|---|---|
| 来源 | 可选 — 使用 `tribal skills install official/productivity/shop-app` 安装 |
...
Tribal 的 gateway 适配器（Telegram、Discord、Slack、iMessage……）会自动渲染 markdown 和图片 URL。正常书写 markdown，图片 URL 单独成行 — 适配器负责处理平台特定的布局。**不要**虚构 `message()` 工具调用（那属于 Shop.app 自身的运行时，不属于 Tribal）。
