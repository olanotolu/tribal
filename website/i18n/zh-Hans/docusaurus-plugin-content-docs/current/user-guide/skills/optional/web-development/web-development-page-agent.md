...
- 用户希望 **Tribal 本身驱动浏览器** → 使用 Tribal 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
