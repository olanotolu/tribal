...
| | |
|---|---|
| 来源 | 可选 — 通过 `tribal skills install official/research/osint-investigation` 安装 |
| 路径 | `optional-skills/research/osint-investigation` |
| 版本 | `0.1.0` |
| 作者 | Tribal Agent（改编自 ShinMegamiBoson/OpenPlanter，MIT 许可）|
...
:::info
以下是 Tribal 在触发该 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::
...
# 之后在本地搜索。缓存 30 天，存储于
# $TRIBAL_OSINT_CACHE/icij/（默认：~/.cache/tribal-osint/icij/）。
...
# 设置 TRIBAL_OSINT_UA=your-app/1.0 (your@email) 以标识自身
