...
通过 AgentMail 为 Agent 提供专属电子邮件收件箱。使用 Agent 专属电子邮件地址（例如 tribal-agent@agentmail.to）自主发送、接收和管理电子邮件。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 使用 `tribal skills install official/email/agentmail` 安装 |
...
### 2. 配置 MCP 服务器
添加至 `~/.tribal/config.yaml`（粘贴实际密钥 — MCP 环境变量不会从 .env 展开）：
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
### 创建收件箱并发送邮件
1. 创建专属收件箱：
   - 使用 `create_inbox` 并指定用户名（例如 `tribal-agent`）
   - Agent 获得地址：`tribal-agent@agentmail.to`
2. 发送邮件：
