...
description: "配置并使用 Honcho 记忆功能与 Tribal -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。"
---
...
配置并使用 Honcho 记忆功能与 Tribal -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。适用于设置 Honcho、排查记忆问题、通过 Honcho peers 管理配置文件，或调整观察、召回和辩证设置。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `tribal skills install official/autonomous-ai-agents/honcho` 安装 |
...
| 相关 skills | [`tribal-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-tribal-agent) |
...
Honcho 提供 AI 原生的跨会话用户建模。它在多次对话中学习用户特征，并为每个 Tribal 配置文件提供独立的 peer 身份，同时共享统一的用户视图。
...
参见：https://docs.honcho.dev/v3/guides/integrations/tribal#running-honcho-locally-with-tribal
...
3. **AI peer 卡片** -- 此 Tribal 配置文件的 AI peer 身份卡片
...
Honcho 将对话建模为 **peers** 之间的交互。Tribal 每个会话创建两个 peers：

- **用户 peer**（`peerName`）：代表人类用户。Honcho 从观察到的消息中构建用户表示。
- **AI peer**（`aiPeer`）：代表此 Tribal 实例。每个配置文件拥有自己的 AI peer，使 agents 形成独立视角。
...
| `per-session` | 每次 Tribal 运行创建新的 Honcho 会话 |
| `global` | 跨所有目录使用单一会话 |

手动覆盖：`tribal honcho map my-project-name`
...
每个 Tribal 配置文件拥有自己的 Honcho AI peer，同时共享同一工作区（用户上下文）。这意味着：
...
`--clone` 对 Honcho 的作用：
1. 在 `honcho.json` 中创建 `tribal.coder` host 块
...
配置文件：`$TRIBAL_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
运行 `tribal honcho setup`。确保 `~/.tribal/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `tribal honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`tribal profile create <name> --clone`。对于现有配置文件：`tribal honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `tribal honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `tribal honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `tribal honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `tribal honcho disable` | 为当前配置文件禁用 Honcho |
| `tribal honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `tribal honcho peers` | 显示所有配置文件的 peer 身份 |
| `tribal honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `tribal honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `tribal honcho sessions` | 列出已知的目录到会话名称映射 |
| `tribal honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `tribal honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `tribal honcho sync` | 为所有尚未拥有 host 块的 Tribal 配置文件创建 host 块 |
| `tribal honcho migrate` | 从 OpenClaw 原生记忆迁移到 Tribal + Honcho 的分步指南 |
| `tribal memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `tribal memory status` | 显示当前活跃的记忆提供商及配置 |
| `tribal memory off` | 禁用外部记忆提供商 |