...
**许可证警告：** OBLITERATUS 采用 AGPL-3.0 许可证。**绝对不要**将其作为 Python 库导入。始终通过 CLI（`obliteratus` 命令）或子进程调用。这样可保持 Tribal Agent 的 MIT 许可证不受污染。

## 视频指南

Tribal agent 使用 OBLITERATUS 对 Gemma 进行 abliterate 的完整演示：
