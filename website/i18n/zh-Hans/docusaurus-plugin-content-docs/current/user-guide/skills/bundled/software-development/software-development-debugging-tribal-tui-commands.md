...
description: "调试 Tribal TUI slash 命令：Python、gateway、Ink UI"
---
...
调试 Tribal TUI slash（斜杠）命令：Python、gateway、Ink UI。
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/software-development/debugging-tribal-tui-commands` |
| 版本 | `1.0.0` |
| 作者 | Tribal Agent |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `debugging`, `tribal-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# 调试 Tribal TUI Slash 命令

## 概述

Tribal slash 命令跨越三个层次——Python 命令注册表、tui_gateway JSON-RPC 桥接层，以及 Ink/TypeScript 前端。当某个命令出现异常（不在自动补全中显示、在 CLI 中正常但在 TUI 中不工作、配置已持久化但 UI 未更新），问题几乎总是某一层与另一层不同步所致。

当你在 Tribal TUI 中遇到 slash 命令问题时使用本 skill，尤其是命令未出现在自动补全中、在 TUI 中无法正常工作，或需要添加/更新命令时。
...
<!-- ascii-guard-ignore -->
```
Python backend (tribal_cli/commands.py)     <- 规范的 COMMAND_REGISTRY
       │
       ▼
...
1. 在 `tribal_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef` 条目：
   ```python
...
4. 如果命令在服务端运行，在 `cli.py` 的 `TribalCLI.process_command()` 中添加处理器：
   ```python
...
1. **命令在 TUI 中显示但不在自动补全中。** 命令已在 TUI 代码库中定义，但 `tribal_cli/commands.py` 的 `COMMAND_REGISTRY` 中缺失。自动补全数据由 Python 端提供。
...
4. 执行命令并确认：
   - 预期行为已触发
   - 所有持久化配置正确更新（`read_file ~/.tribal/config.yaml`）
