...
2. 在环境中设置 `LINEAR_API_KEY`（通过 `tribal setup` 或你的环境配置）
...
末尾的十六进制段即为 `slugId`。示例：`https://linear.app/nousresearch/document/rfc-tribal-permission-gateway-discord-38359beef67c` → `slugId` 为 `38359beef67c`。
