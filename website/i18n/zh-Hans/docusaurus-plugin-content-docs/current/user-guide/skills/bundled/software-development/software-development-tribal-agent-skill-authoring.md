---
title: "Tribal Agent Skill 编写——在仓库中编写 SKILL"
sidebar_label: "Tribal Agent Skill 编写"
description: "在仓库中编写 SKILL.md"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Tribal Agent Skill 编写
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/software-development/tribal-agent-skill-authoring` |
| 版本 | `1.0.0` |
| 作者 | Tribal Agent |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `skills`, `authoring`, `tribal-agent`, `conventions`, `skill-md` |
...
# 编写 Tribal-Agent Skills（仓库内）

## 概述

SKILL.md 可以存放在两个位置：

1. **用户本地：** `~/.tribal/skills/<maybe-category>/<name>/SKILL.md` — 个人使用，不共享。通过 `skill_manage(action='create')` 创建。
2. **仓库内（本 skill 讨论此情况）：** `/home/bb/tribal-agent/skills/<category>/<name>/SKILL.md` — 已提交，随包一起发布。使用 `write_file` + `git add`。`skill_manage(action='create')` **不**针对此目录树。

## 使用时机

- 用户要求你"在此分支 / 仓库 / 提交中"添加一个 skill
- 你正在提交一个应随 tribal-agent 一起发布的可复用工作流
- 你正在编辑 `/home/bb/tribal-agent/skills/` 下的现有 skill（小改动用 `patch`，重写用 `write_file`；`skill_manage` 对仓库内 skill 的 `patch` 仍有效，但 `create` 无效）
...
`metadata.tribal.related_skills` 在加载时会合并两个目录树（仓库内 `skills/` 和 `~/.tribal/skills/`）。你**可以**从仓库内 skill 引用用户本地 skill，但对于全新克隆仓库的其他用户，该引用无法解析。仓库内 skill 优先只引用仓库内 skill。如果某个频繁被引用的 skill 仅存在于 `~/.tribal/skills/`，请考虑将其提升到仓库中。
...
1. **对仓库内 skill 使用 `skill_manage(action='create')`。** 它会写入 `~/.tribal/skills/`，而非仓库目录树。仓库内创建请使用 `write_file`。
...
- [ ] 文件位于 `skills/<category>/<name>/SKILL.md`（不在 `~/.tribal/skills/` 中）
- [ ] Frontmatter 从字节 0 以 `---` 开头，以 `\n---\n` 结束
- [ ] `name`、`description`、`version`、`author`、`license`、`metadata.tribal.{tags, related_skills}` 均已填写
- [ ] Name ≤ 64 个字符，小写加连字符
