...
| 相关 skill | [`systematic-debugging`](/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-tribal-tui-commands`](/user-guide/skills/bundled/software-development/software-development-debugging-tribal-tui-commands) |
...
- 需要逐步执行某个函数并观察集合的变化
- 长期运行的进程（tribal gateway、tui_gateway）出现异常且无法重启
...
适用于长期运行的进程：Tribal gateway、tui_gateway、daemon，或已出现异常且无法干净重启的进程。
...
最简便的终端侧 DAP 客户端是 VS Code CLI 或一个小脚本。在 Tribal 内部有两个实用选项：
...
最简单：在可疑行附近添加 `breakpoint()`，然后正常运行 `tribal`。控制权将在暂停点返回到你的终端。

### `tui_gateway` 子进程（由 `tribal --tui` 启动）
...
debugpy.wait_for_client()
```
启动 `tribal --tui`。TUI 将显示为冻结状态（其后端正在等待）。附加客户端后，执行在你 `continue` 时恢复。
...
9. **fork / 多进程。** pdb 不会跟随 fork。每个子进程需要自己的 `breakpoint()` 或 `set_trace()`。对于 Tribal 子 agent，每次只调试一个进程。
