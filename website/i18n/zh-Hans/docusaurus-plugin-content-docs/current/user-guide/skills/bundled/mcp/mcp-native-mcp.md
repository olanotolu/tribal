...
Tribal Agent 内置了一个 MCP 客户端，它在启动时连接到 MCP 服务器，发现其工具，并将其作为一等工具直接提供给 agent 调用。无需桥接 CLI——来自 MCP 服务器的工具与 `terminal`、`read_file` 等内置工具并列显示。

## 使用场景

在以下情况下使用此 skill：
- 连接到 MCP 服务器并在 Tribal Agent 中使用其工具
...
在 `~/.tribal/config.yaml` 的 `mcp_servers` 键下添加 MCP 服务器：
...
Tribal Agent 启动时，`discover_mcp_tools()` 在工具初始化期间被调用：

1. 从 `~/.tribal/config.yaml` 读取 `mcp_servers`
2. 对每个服务器，在专用后台事件循环中生成连接
3. 初始化 MCP 会话并调用 `list_tools()` 发现可用工具
4. 在 Tribal 工具注册表中注册每个工具
...
发现完成后，MCP 工具会自动注入所有 `tribal-*` 平台工具集（CLI、Discord、Telegram 等）。这意味着 MCP 工具无需任何额外配置即可在每次对话中使用。
...
最常见的传输方式。Tribal 将 MCP 服务器作为子进程启动，并通过 stdin/stdout 通信。
...
对于 stdio 服务器，Tribal **不会**将你的完整 shell 环境传递给 MCP 子进程。只有以下安全基线变量会被继承：
...
`~/.tribal/config.yaml` 中没有 `mcp_servers` 键，或该键为空。请至少添加一个服务器。
...
Tribal 支持 MCP 的 `sampling/createMessage` 能力——MCP 服务器可在工具执行期间通过 agent 请求 LLM 补全。这支持 agent-in-the-loop 工作流（数据分析、内容生成、决策制定）。
