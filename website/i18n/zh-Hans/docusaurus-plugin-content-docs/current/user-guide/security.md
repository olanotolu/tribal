...
Tribal Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
在执行任何命令之前，Tribal 会将其与一份精心维护的危险模式列表进行比对。若匹配，用户必须明确批准。

### 审批模式

审批系统支持三种模式，通过 `~/.tribal/config.yaml` 中的 `approvals.mode` 配置：
...
1. **CLI 标志**：使用 `tribal --yolo` 或 `tribal chat --yolo` 启动会话
2. **斜杠命令**：在会话中输入 `/yolo` 以切换开/关
3. **环境变量**：设置 `TRIBAL_YOLO_MODE=1`
...
YOLO 模式在 CLI 和 gateway 会话中均可使用。在内部，它会设置 `TRIBAL_YOLO_MODE` 环境变量，该变量在每次命令执行前都会被检查。

当 YOLO 激活时，Tribal 会显示两个持久的视觉提醒，以确保用户不会忘记审批提示已被绕过：
...
某些命令极具破坏性——不可逆的文件系统清除、fork 炸弹、直接写入块设备——无论以下任何情况，Tribal 都**拒绝**执行：
...
| `tee` 写入 `/etc/`、`~/.ssh/`、`~/.tribal/.env` | 通过 tee 覆盖敏感文件 |
| `>` / `>>` 写入 `/etc/`、`~/.ssh/`、`~/.tribal/.env` | 通过重定向覆盖敏感文件 |
...
| `pkill`/`killall` tribal/gateway | 防止自我终止 |
...
运行 gateway 时，`TRIBAL_EXEC_ASK=1` 环境变量会自动设置。

### 永久允许列表

通过"always"批准的命令会保存到 `~/.tribal/config.yaml`：
...
:::tip
使用 `tribal config edit` 查看或删除永久允许列表中的模式。
:::

## 用户授权（Gateway）

运行消息 gateway 时，Tribal 通过分层授权系统控制谁可以与机器人交互。
...
在 `~/.tribal/.env` 中以逗号分隔的值设置允许的用户 ID：
...
为实现更灵活的授权，Tribal 提供了基于验证码的配对系统。无需预先提供用户 ID，未知用户会收到一次性配对码，由机器人所有者通过 CLI 批准。

**工作原理：**

1. 未知用户向机器人发送 DM
2. 机器人回复一个 8 位配对码
3. 机器人所有者在 CLI 上运行 `tribal pairing approve <platform> <code>`
4. 该用户在该平台上获得永久批准

在 `~/.tribal/config.yaml` 中控制未授权私信的处理方式：
...
**存储：** 配对数据存储于 `~/.tribal/pairing/`，按平台分为独立的 JSON 文件：
...
使用 `docker` 终端后端时，Tribal 对每个容器应用严格的安全加固。
...
- **持久模式**（`container_persistent: true`）：从 `~/.tribal/sandboxes/docker/<task_id>/` 绑定挂载 `/workspace` 和 `/root`
...
某些技能需要在沙箱中访问**文件**（而非仅环境变量）——例如，Google Workspace 将 OAuth token 存储为活跃 profile 的 `TRIBAL_HOME` 下的 `google_token.json`。技能在 frontmatter 中声明这些文件：
...
加载后，Tribal 会检查这些文件是否存在于活跃 profile 的 `TRIBAL_HOME` 中，并将其注册为挂载：
...
路径相对于 `~/.tribal/`。文件在容器内挂载到 `/root/.tribal/`。
...
| **terminal**（本地） | 阻止明确的 Tribal 基础设施变量（提供商密钥、gateway token、工具 API 密钥） | ✅ 透传变量绕过黑名单 |
...
- Tribal 基础设施密钥（提供商 API 密钥、gateway token）不应添加到 `env_passthrough`——它们有专用机制
...
Tribal 集成了 [tirith](https://github.com/sheeki03/tirith) 用于在执行前进行内容级命令扫描。Tirith 能检测单纯模式匹配所遗漏的威胁：
...
Tirith 为 Linux（x86_64 / aarch64）和 macOS（x86_64 / arm64）提供预构建二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过——模式匹配防护仍然运行，CLI 不会显示"不可用"横幅。若要在 Windows 上使用 tirith，请在 WSL 下运行 Tribal。
...
4. **安全存储密钥** — 将 API 密钥保存在具有适当文件权限的 `~/.tribal/.env` 中
...
9. **监控日志** — 检查 `~/.tribal/logs/` 中的未授权访问尝试
10. **保持更新** — 定期运行 `tribal update` 以获取安全补丁
...
为获得最高安全性，请在独立的机器或虚拟机上运行 gateway。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 `~/.tribal/.env` 中的环境变量提供主机详情：
...
Tribal 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `tribal_cli/security_advisories.py`。

运行方式：

- **CLI 启动横幅。** 若有任何公告匹配，会打印一行警告，并指向 `tribal doctor` 获取完整修复方案。
- **`tribal doctor`。** 显示所有活跃公告的版本详情和 2-4 步修复说明。
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。Tribal 在首次使用时**懒加载**安装这些包，而非在 `tribal-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
...
3. 若安装失败或用户已禁用懒加载安装，调用会抛出 `FeatureUnavailable`，附带实际的 pip stderr 和指向 `tribal tools` 的提示。
...
禁用后，需要可选依赖的后端会提示用户手动运行安装（`pip install …`）或通过 `tribal tools` 选择其他后端。