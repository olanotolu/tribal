...
description: "使用 Nix 安装和部署 Tribal Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Tribal Agent 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`tribal setup`、`tribal gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 tribal 的方式与管理其他 NixOS 服务相同。
:::
...
执行 `nix profile install` 后，`tribal`、`tribal-agent` 和 `tribal-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`tribal setup` 引导你完成提供商选择，`tribal gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.tribal/`。
...
就这些。`nixos-rebuild switch` 会创建 `tribal` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `tribal` CLI 添加到系统 PATH，**并**在系统范围内设置 `TRIBAL_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `tribal` 会创建独立的 `~/.tribal/` 目录。
:::

### 容器感知 CLI

:::info
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**所有** `tribal` 命令都会自动路由到托管容器中执行。这意味着你的交互式 CLI 会话在与 gateway 服务相同的环境中运行——可以访问所有容器内安装的包和工具。

- 路由是透明的：`tribal chat`、`tribal sessions list`、`tribal version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时显示 5 秒 spinner，脚本中静默等待 10 秒），然后以明确的错误退出——不会静默回退
- 对于在 tribal 代码库上工作的开发者，设置 `TRIBAL_DEV=1` 可绕过容器路由，直接运行本地检出版本

设置 `container.hostUsers` 可创建 `~/.tribal` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
`hostUsers` 中列出的用户会自动加入 `tribal` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo tribal chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Tribal 默认使用 OpenRouter。
:::
...
这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$TRIBAL_HOME/config.yaml`。
...
| 添加 API 密钥 | `environmentFiles` | `[ config.sops.secrets."tribal-env".path ]` |
| 给 Agent 设置个性 | `${services.tribal-agent.stateDir}/.tribal/SOUL.md` | 直接管理该文件 |
...
| 覆盖 tribal 包 | `package` | `inputs.tribal-agent.packages.${system}.default.override { ... }` |
| 更改状态目录 | `stateDir` | `"/opt/tribal"` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$TRIBAL_HOME/.env` 中。Tribal 在每次启动时读取此文件，因此更改在 `systemctl restart tribal-agent` 后生效——无需重建容器。
...
```yaml
# secrets/tribal.yaml（使用 sops 加密）
tribal-env: |
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Tribal 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Tribal 从 `$TRIBAL_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.tribal-agent.stateDir}/.tribal/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$TRIBAL_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Tribal 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$TRIBAL_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Tribal 将授权 URL 打印到 stdout/日志，而不是打开浏览器。

**方案 A：交互式引导** — 通过 `docker exec`（容器）或 `sudo -u tribal`（原生）运行一次流程：
...
# 确保：chown tribal:tribal，chmod 0600
```
...
当 tribal 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `tribal setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `tribal config edit` | 配置由 `settings` 生成 |
| `tribal config set <key> <value>` | 配置由 `settings` 生成 |
| `tribal gateway install` | systemd 服务由 NixOS 管理 |
| `tribal gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`TRIBAL_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `TRIBAL_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it tribal-agent tribal config set ...` 也会被屏蔽）
...
启用容器模式后，tribal 在持久化 Ubuntu 容器内运行，Nix 构建的二进制文件以只读方式从主机绑定挂载：

```
主机                                    容器
────                                    ─────────
/nix/store/...-tribal-agent-0.1.0  ──►  /nix/store/... (ro)
~/.tribal -> /var/lib/tribal/.tribal       （符号链接桥接，按 hostUsers）
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/tribal gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。

### 各事件的持久性

| 事件 | 容器重建？ | `/data`（状态） | `/home/tribal` | 可写层（`apt`/`pip`/`npm`） |
|---|---|---|---|---|
| `systemctl restart tribal-agent` | 否 | 保留 | 保留 | 保留 |
...
仅当容器的**身份哈希**发生变化时才会重建容器。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、settings、文档或 tribal 包本身的变更**不会**触发重建。

:::warning 可写层丢失
当身份哈希发生变化（镜像升级、新卷、新容器选项）时，容器会被销毁并从 `container.image` 的全新拉取重建。可写层中通过 `apt install`、`pip install` 或 `npm install` 安装的包将丢失。`/data` 和 `/home/tribal` 中的状态会保留（这些是绑定挂载）。

如果 Agent 依赖特定包，考虑将其烘焙到自定义镜像中（`container.image = "my-registry/tribal-base:latest"`），或在 Agent 的 SOUL.md 中编写安装脚本。
:::

### GC Root 保护

`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 tribal 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `tribal plugins install`。

### 目录插件（`extraPlugins`）

对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [tribal-lcm](https://github.com/stephenschoettler/tribal-lcm)）：
...
插件在激活时以符号链接方式安装到 `$TRIBAL_HOME/plugins/`。Tribal 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。

### 入口点插件（`extraPythonPackages`）

对于通过 `[project.entry-points."tribal_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-tribal](https://github.com/ogallotti/rtk-tribal)）：
...
该包的 `site-packages` 会添加到 tribal wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。

### 可选依赖组（`extraDependencyGroups`）

对于已在 tribal-agent 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
:::note
构建时冲突检查可防止插件包覆盖核心 tribal 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
nix build .#checks.x86_64-linux.managed-guard       # TRIBAL_MANAGED 屏蔽变更操作
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `tribal` 和 `tribal-agent` 二进制文件存在且 `tribal version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `tribal --help` 暴露 `gateway` 和 `config` 子命令 |
| `managed-guard` | `TRIBAL_MANAGED=true tribal config set ...` 打印 NixOS 错误 |
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `TRIBAL_BUNDLED_SKILLS` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 tribal-agent 服务 |
| `package` | `package` | `tribal-agent` | 使用的 tribal-agent 包 |
| `user` | `str` | `"tribal"` | 系统用户 |
| `group` | `str` | `"tribal"` | 系统组 |
| `createUser` | `bool` | `true` | 自动创建用户/组 |
| `stateDir` | `str` | `"/var/lib/tribal"` | 状态目录（`TRIBAL_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `tribal` CLI 添加到系统 PATH 并在系统范围内设置 `TRIBAL_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$TRIBAL_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `tribal gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 tribal 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$TRIBAL_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.tribal` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `tribal` 组 |
...
```
/var/lib/tribal/                     # stateDir（归 tribal:tribal 所有，权限 0750）
├── .tribal/                         # TRIBAL_HOME
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Tribal 二进制文件 + 所有 Nix 依赖 |
| `/data` | `/var/lib/tribal` | `rw` | 所有状态、配置、工作区 |
| `/home/tribal` | `${stateDir}/home` | `rw` | 持久化 Agent home——`pip install --user`、工具缓存 |
...
| `tribal version` 显示旧版本 | 容器未重启 | `systemctl restart tribal-agent` |
| `/var/lib/tribal` 权限拒绝 | 状态目录为 `0750 tribal:tribal` | 使用 `docker exec` 或 `sudo -u tribal` |
| `nix-collect-garbage` 删除了 tribal | GC root 缺失 | 重启服务（preStart 会重新创建 GC root） |
| `no container with name or ID "tribal-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user tribal` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart tribal-agent` |