...
description: "通过 Termux 在 Android 手机上直接运行 Tribal Agent"
---

# 在 Android 上通过 Termux 运行 Tribal

这是在 Android 手机上通过 [Termux](https://termux.dev/) 直接运行 Tribal Agent 的已验证路径。
...
这并不妨碍 Tribal 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。

---

## 方式一：一行安装命令

Tribal 现已内置 Termux 感知的安装路径：
...
- 将 `tribal` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `tribal` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/tribal" "$PREFIX/bin/tribal"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `tribal` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `tribal doctor` 提示缺少 ripgrep 或 Node
