...
sidebar_position: 4
title: "贡献指南"
description: "如何为 Tribal Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 Tribal Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 Tribal 核心？从 [构建 Tribal 插件](../guides/build-a-tribal-plugin.md) 开始
- 为 Tribal 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
- **Profile 安全路径**：不得硬编码 `~/.tribal` — 代码路径使用 `tribal_constants` 中的 `get_tribal_home()`，面向用户的消息使用 `display_tribal_home()`。完整规则参见 [AGENTS.md](https://github.com/Tribal/tribal/blob/main/AGENTS.md#profiles-multi-instance-support)。

## 跨平台兼容性

Tribal 官方支持 **Linux、macOS、WSL2 以及原生 Windows（早期 beta — 通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。原生 Windows 路径较新且迭代较快 — 如果您主要在 Windows 上开发，请做好遇到并修复粗糙边缘的准备。
...
- 使用 [GitHub Issues](https://github.com/Tribal/tribal/issues)
- 请包含：操作系统、Python 版本、Tribal 版本（`tribal version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/Tribal/tribal/blob/main/LICENSE) 授权。