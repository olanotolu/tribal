...
sidebar_position: 3
title: "创建 Skill"
description: "如何为 Tribal Agent 创建 skill——SKILL.md 格式、规范与发布"
---

# 创建 Skill

Skill 是为 Tribal Agent 添加新能力的首选方式。与 tool 相比，skill 更易于创建，无需修改 agent 代码，且可与社区共享。
...
当 skill 需要 API key 或 token 时，使用 `required_environment_variables`。缺少值**不会**将 skill 从发现列表中隐藏。Tribal 会在本地 CLI 加载 skill 时安全地提示用户输入。
...
用户可以跳过配置并继续加载 skill。Tribal 不会将原始密钥值暴露给模型。Gateway 和消息会话会显示本地配置指引，而不是在带内收集密钥。
...
- `prompt`（可选）——`tribal config migrate` 时显示的提示文本；若未设置则回退到 `description`
...
2. **发现：** `tribal config migrate` 扫描所有已启用的 skill，找出未配置的项并提示用户。配置项也会在 `tribal config show` 的"Skill Settings"部分显示。
...
:::tip 如何选择
对 API key、token 及其他**密钥**使用 `required_environment_variables`（存储在 `~/.tribal/.env`，不向模型展示）。对**路径、偏好设置及非敏感配置**使用 `config`（存储在 `config.yaml`，在 config show 中可见）。
:::
...
每个条目支持：
- `path`（必需）——相对于 `~/.tribal/` 的文件路径
- `description`（可选）——说明该文件的用途及创建方式

加载时，Tribal 会检查这些文件是否存在。缺少文件会触发 `setup_needed`。已存在的文件会自动：
- **挂载到 Docker** 容器中作为只读绑定挂载
- **同步到 Modal** 沙箱（在创建时及每次命令前同步，因此会话中途的 OAuth 也能正常工作）
- 在**本地**后端无需任何特殊处理即可使用

:::tip 如何选择
对简单的 API key 和 token（存储在 `~/.tribal/.env` 中的字符串）使用 `required_environment_variables`。对 OAuth token 文件、客户端密钥、服务账号 JSON、证书或任何以磁盘文件形式存在的凭证使用 `required_credential_files`。
:::
...
优先使用标准库 Python、curl 以及现有 Tribal tool（`web_extract`、`terminal`、`read_file`）。若确实需要依赖项，请在 skill 中记录安装步骤。
...
| Token | 替换为 |
|---|---|
| `${TRIBAL_SKILL_DIR}` | skill 目录的绝对路径 |
| `${TRIBAL_SESSION_ID}` | 当前会话 ID（若无会话则保留原样） |
...
内置 skill（位于 `skills/`）随每次 Tribal 安装一起发布，应对**大多数用户广泛有用**：

- 文档处理、网页研究、常见开发工作流、系统管理
- 被广泛人群定期使用

如果你的 skill 是官方的且有用，但并非所有人都需要（例如付费服务集成、重量级依赖），请放入 **`optional-skills/`**——它随仓库一起发布，可通过 `tribal skills browse` 发现（标记为"official"），并以内置信任级别安装。

如果你的 skill 是专业化的、社区贡献的或小众的，更适合放在 **Skills Hub**——将其上传到注册表并通过 `tribal skills install` 分享。
...
Tribal 现在可以通过多种外部发现模型使用第三方 skill：
