...
description: "如何为 Tribal Agent 构建视频生成后端插件"
---
...
Tribal 在三个位置扫描视频生成后端：

1. **内置** — `<repo>/plugins/video_gen/<name>/`（通过 `kind: backend` 自动加载）
2. **用户** — `~/.tribal/plugins/video_gen/<name>/`（通过 `plugins.enabled` 选择启用）
3. **Pip** — 声明了 `tribal_agent.plugins` 入口点的包

每个插件的 `register(ctx)` 函数调用 `ctx.register_video_gen_provider(...)`。活跃 provider 由 `config.yaml` 中的 `video_gen.provider` 指定；`tribal tools` → Video Generation 引导用户完成选择。与 `image_generate` 不同，此处没有内置的遗留后端——每个 provider 都是插件。
...
Provider 的 `capabilities()` 声明上述哪些参数会被实际处理。Agent 在工具描述中看到的是当前活跃后端的能力信息，当用户通过 `tribal tools` 切换后端时会动态重建。
...
用户在 `tribal tools` 中只需选择一次 `veo3.1`。Agent 无需关心端点——它只负责传入（或不传入）`image_url`。
...
如果你的后端返回 base64 数据，使用 `save_b64_video()` 将其写入 `$TRIBAL_HOME/cache/videos/`。对于通过后续 HTTP 请求获取的原始字节，使用 `save_bytes_video()`。否则直接返回上游 URL——gateway 在交付时会解析远程 URL。
