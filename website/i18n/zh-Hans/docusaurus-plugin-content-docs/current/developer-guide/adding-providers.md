...
sidebar_position: 5
title: "添加 Provider"
description: "如何向 Tribal Agent 添加新的推理 provider——认证、运行时解析、CLI 流程、适配器、测试与文档"
---

# 添加 Provider

Tribal 已经可以通过自定义 provider 路径与任何 OpenAI 兼容的端点通信。除非你需要为某个服务提供一流的用户体验，否则不要添加内置 provider：

- provider 专属的认证或 token 刷新
- 精选的模型目录
- setup / `tribal model` 菜单条目
...
1. `tribal_cli/auth.py` 决定如何查找凭据。
2. `tribal_cli/runtime_provider.py` 将其转换为运行时数据：
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `tribal_cli/models.py` 和 `tribal_cli/main.py` 使 provider 在 CLI 中可见。（`tribal_cli/setup.py` 自动委托给 `main.py`——无需在此处做任何修改。）
...
:::tip
`tribal_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `tribal setup` 中。
:::
...
2. 就这些。Provider 插件在任何代码首次调用 `get_provider_profile()` 或 `list_providers()` 时自动加载——捆绑插件（本仓库）和位于 `$TRIBAL_HOME/plugins/model-providers/` 的用户插件都会被加载。
...
7. `tribal model` 菜单包含该 provider
8. `tribal setup` 向导自动委托给 `main.py`
...
位于 `$TRIBAL_HOME/plugins/model-providers/<name>/` 的用户插件会覆盖同名的捆绑插件（`register_provider()` 中后写者获胜）——因此第三方可以在不编辑本仓库的情况下对任何内置 profile 进行 monkey-patch 或替换。
...
- 带有特定认证流程的 provider 专属 `tribal model` 菜单条目
...
- `tribal_cli/auth.py` 中的 `PROVIDER_REGISTRY`
- `tribal_cli/models.py` 中的 `_PROVIDER_LABELS`
- `tribal_cli/auth.py` 和 `tribal_cli/models.py` 中的 `_PROVIDER_ALIASES`
- `tribal_cli/main.py` 中的 CLI `--provider` 选项
- setup / 模型选择分支
- 辅助模型默认值
- 测试
...
## 第 2 步：在 `tribal_cli/auth.py` 中添加认证元数据
...
## 第 3 步：在 `tribal_cli/models.py` 中添加模型目录和别名
...
## 第 4 步：在 `tribal_cli/runtime_provider.py` 中解析运行时数据
...
注意 API key 优先级。Tribal 已经包含避免将 OpenRouter key 泄露给无关端点的逻辑。新 provider 应同样明确地指定哪个 key 对应哪个 base URL。

## 第 5 步：在 `tribal_cli/main.py` 中接线 CLI

在交互式 `tribal model` 流程中出现之前，provider 是不可发现的。

在 `tribal_cli/main.py` 中更新以下内容：
...
:::tip
`tribal_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `tribal model` 和 `tribal setup` 中。
:::
...
添加原生 provider 时，仔细检查 Tribal 只向该 provider 发送它实际理解的字段。
...
- [ ] 在 `tribal_cli/auth.py` 中添加 `ProviderConfig`
- [ ] 在 `tribal_cli/auth.py` 和 `tribal_cli/models.py` 中添加别名
- [ ] 在 `tribal_cli/models.py` 中添加模型目录
- [ ] 在 `tribal_cli/runtime_provider.py` 中添加运行时分支
- [ ] 在 `tribal_cli/main.py` 中添加 CLI 接线（setup.py 自动继承）
...
### 7. 更新了 `tribal model` 但未更新 `tribal setup`
