...
本指南介绍如何向 Tribal gateway 添加新的消息平台。平台适配器将 Tribal 连接到外部消息服务（Telegram、Discord、WeCom 等），使用户可以通过该服务与 agent 交互。

:::tip
添加平台有两种方式：
- **Plugin**（推荐用于社区/第三方）：将 plugin 目录放入 `~/.tribal/plugins/` — 无需修改任何核心代码。参见下方 [Plugin 路径](#plugin-path-recommended)。
...
Plugin 系统允许你在不修改任何 Tribal 核心代码的情况下添加平台适配器。你的 plugin 是一个包含两个文件的目录：
...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `tribal config` UI 条目（参见下方[在 tribal config 中暴露环境变量](#surfacing-env-vars-in-tribal-config)）。
...
def register(ctx):
    """Plugin 入口点 — 由 Tribal plugin 系统调用。"""
...
| `tribal config` UI 条目 | `plugin.yaml` 中的 `requires_env` / `optional_env` 自动填充 |
...
| PII 脱敏 | `pii_safe` 标志 |
| `tribal status` | 显示带 `(plugin)` 标签的 plugin 平台 |
| `tribal gateway setup` | Plugin 平台出现在设置菜单中 |
| `tribal tools` / `tribal skills` | Plugin 平台出现在每平台配置中 |
| Token 锁（多配置文件） | 在 `connect()` 中使用 `acquire_scoped_lock()` |
| 孤立配置警告 | Plugin 缺失时输出描述性日志 |

## 环境变量驱动的自动配置

大多数用户通过将环境变量写入 `~/.tribal/.env` 来配置平台，而不是编辑 `config.yaml`。`env_enablement_fn` hook 允许你的 plugin 在适配器构建**之前**读取这些环境变量，使 `tribal gateway status`、`get_connected_platforms()` 和 cron 投递无需实例化平台 SDK 即可看到正确状态。
...
`cron_deliver_env_var` 使你的平台成为可识别的 `deliver=` 目标。要在 cron 任务运行于独立进程（即 `tribal cron run` 与 `tribal gateway` 分离）时使实际发送成功，需注册 `standalone_sender_fn`：
...
## 在 `tribal config` 中暴露环境变量 {#surfacing-env-vars-in-tribal-config}

`tribal_cli/config.py` 在导入时扫描 `plugins/platforms/*/plugin.yaml`，并从 `requires_env` 和（可选的）`optional_env` 块自动填充 `OPTIONAL_ENV_VARS`。使用富字典形式可提供完整的描述、prompt、password 标志和 URL — CLI 设置 UI 会自动识别。
...
:::note
此清单用于将平台直接添加到 Tribal 核心代码库 — 通常由核心贡献者为官方支持的平台执行。社区/第三方平台应使用上方的 [Plugin 路径](#plugin-path-recommended)。
:::
...
1. **`tribal_cli/config.py`** — 将所有 `NEWPLAT_*` 变量添加到 `_EXTRA_ENV_KEYS`
2. **`tribal_cli/gateway.py`** — 在 `_PLATFORMS` 列表中添加条目，包含 key、label、emoji、token_var、setup_instructions 和 vars
3. **`tribal_cli/platforms.py`** — 添加带 label 和 default_toolset 的 `PlatformInfo` 条目（供 `skills_config` 和 `tools_config` TUI 使用）
4. **`tribal_cli/setup.py`** — 添加 `_setup_newplat()` 函数（可委托给 `gateway.py`）并将元组添加到消息平台列表
5. **`tribal_cli/status.py`** — 添加平台检测条目：`"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`tribal_cli/dump.py`** — 将 `"newplat": "NEWPLAT_TOKEN"` 添加到平台检测字典
...
1. **`toolsets.py`** — 添加带 `_TRIBAL_CORE_TOOLS` 的 `"tribal-newplat"` toolset 定义
2. **`toolsets.py`** — 将 `"tribal-newplat"` 添加到 `"tribal-gateway"` 的 includes 列表
