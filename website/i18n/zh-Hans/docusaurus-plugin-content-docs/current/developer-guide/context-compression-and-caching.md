---
title: 上下文压缩与缓存
description: Tribal Agent 如何通过双重压缩系统和 Anthropic prompt 缓存高效管理上下文窗口。
---

# 上下文压缩与缓存

Tribal Agent 使用双重压缩系统和 Anthropic prompt（提示词）缓存，在长对话中高效管理上下文窗口用量。
...
通过 `tribal plugins` → Provider Plugins → Context Engine 进行配置，或直接编辑 `config.yaml`。
...
Anthropic 每次请求最多允许 4 个 `cache_control` 断点。Tribal 使用"system_and_3"策略：
