...
sidebar_position: 1
title: "架构"
description: "Tribal Agent 内部结构——主要子系统、执行路径、数据流及延伸阅读指引"
---

# 架构

本页是 Tribal Agent 内部结构的顶层导图。用它在代码库中定位自己，然后深入各子系统专项文档了解实现细节。
...
├── cli.py                    # TribalCLI — 交互式终端 UI（大文件）
├── model_tools.py            # 工具发现、schema 收集、分发
├── toolsets.py               # 工具分组与平台预设
├── tribal_state.py           # 带 FTS5 的 SQLite 会话/状态数据库
├── tribal_constants.py       # TRIBAL_HOME、感知 profile 的路径
...
│
├── tribal_cli/               # CLI 子命令与设置
│   ├── main.py               # 入口点——所有 `tribal` 子命令（大文件）
...
│   ├── skills_config.py      # tribal skills——按平台启用/禁用
│   ├── skills_hub.py         # /skills 斜杠命令
│   ├── tools_config.py       # tribal tools——按平台启用/禁用
│   ├── plugins.py            # PluginManager——发现、加载、hook
│   ├── callbacks.py          # 终端回调（clarify、sudo、approval）
│   └── gateway.py            # tribal gateway 启动/停止
│
├── tools/                    # 工具实现（每个工具一个文件）
│   ├── registry.py           # 中央工具注册表
│   ├── approval.py           # 危险命令检测
...
- **`prompt_builder.py`** — 从以下来源组装系统 prompt：个性（SOUL.md）、记忆（MEMORY.md、USER.md）、skill、上下文文件（AGENTS.md、.tribal.md）、工具使用指引以及模型专项指令
...
三种发现来源：`~/.tribal/plugins/`（用户级）、`.tribal/plugins/`（项目级）和 pip entry point。插件通过上下文 API 注册工具、hook 和 CLI 命令。存在两种专用插件类型：记忆提供者（`plugins/memory/`）和上下文引擎（`plugins/context_engine/`）。两者均为单选——每种同时只能激活一个，通过 `tribal plugins` 或 `config.yaml` 配置。

→ [插件指南](/guides/build-a-tribal-plugin)，[记忆提供者插件](./memory-provider-plugin.md)
...
通过 stdio/JSON-RPC 将 Tribal 作为编辑器原生 agent 暴露给 VS Code、Zed 和 JetBrains。
...
| **Profile 隔离** | 每个 profile（`tribal -p <name>`）拥有独立的 TRIBAL_HOME、配置、记忆、会话和 gateway PID。多个 profile 可并发运行。 |
