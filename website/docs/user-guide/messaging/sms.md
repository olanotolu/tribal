...
title: "SMS (Twilio)"
description: "Set up Tribal Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Tribal connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `pip install 'tribal-agent[sms]'`
...
If you're running Tribal locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Tribal will respond via SMS.
...
Tribal validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Tribal gateway logs for Twilio API errors
