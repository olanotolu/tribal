...
sidebar_position: 17
title: "LINE"
description: "Set up Tribal Agent as a LINE Messaging API bot"
---

# LINE Setup

Run Tribal Agent as a [LINE](https://line.me/) bot via the official LINE Messaging API. The adapter lives as a bundled platform plugin under `plugins/platforms/line/` — no core edits, just enable it like any other platform.

LINE is the dominant messaging app in Japan, Taiwan, and Thailand. If your users live there, this is how they reach you.

> Run `tribal gateway setup` and pick **LINE** for a guided walk-through.
...
# devtunnel
devtunnel create tribal-line --allow-anonymous
devtunnel port create tribal-line -p 8646 --protocol https
devtunnel host tribal-line
```
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.tribal/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
...
**"already in use by another profile".** The same channel access token is bound to another running Tribal profile. Stop the other gateway or use a separate channel.
