...
sidebar_position: 4
title: "Slack"
description: "Set up Tribal Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Tribal Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Tribal instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Tribal uses the modern
...
The fastest path is to paste a manifest Tribal generates for you. It
...
### Option A: From a Tribal-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   tribal slack manifest --write
   ```
   This writes `~/.tribal/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Tribal Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Tribal can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `tribal-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Tribal configuration issue.
:::
...
Tribal uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
tribal gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
tribal gateway              # Foreground
tribal gateway install      # Install as a user service
sudo tribal gateway install --system   # Linux only: boot-time system service
```
...
Every Tribal command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Tribal command with its description.

Under the hood: Tribal ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Tribal/tribal/blob/main/tribal_cli/commands.py)
...
When Tribal adds new commands (e.g. after `tribal update`), regenerate
...
3. Paste the new contents of `~/.tribal/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/tribal <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/tribal btw run the tests` — Tribal routes it the same way as `/btw
run the tests`. Free-form questions also work: `/tribal what's the
...
Slack never delivers them to Tribal.

As a workaround, Tribal recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Tribal treats it identically to the slash form and replies in the same
thread.
...
```bash
tribal slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Tribal behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Tribal Agent what time is it?`). In channels, Tribal replies in a thread attached to that message. |
| **Threads** | If you @mention Tribal inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.tribal/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Tribal in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Tribal will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Tribal will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Tribal Agent`).

---

## Multi-Workspace Support

Tribal can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Tribal also loads tokens from an **OAuth token file** at:

```
~/.tribal/slack_tokens.json
```
...
- When a message arrives, Tribal uses the correct workspace-specific client to respond.
...
Tribal supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Tribal Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Tribal Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Tribal now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Tribal Agent`)
...
- Tokens should be stored in `~/.tribal/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Tribal config directory
