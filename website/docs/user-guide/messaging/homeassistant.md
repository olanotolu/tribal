...
description: Control your smart home with Tribal Agent via Home Assistant integration.
...
Tribal Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Tribal Agent"
5. Copy the token
...
Tribal Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.tribal/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Tribal Agent".
...
The adapter reads credentials from `~/.tribal/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Tribal profile
...
running Tribal — `curl -H "Authorization: Bearer <token>" <url>/api/` should
