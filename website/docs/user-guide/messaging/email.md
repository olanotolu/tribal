...
sidebar_position: 7
title: "Email"
description: "Set up Tribal Agent as an email assistant via IMAP/SMTP"
---

# Email Setup

Tribal can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread — no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
|---|---|---|
| Let people email the Tribal agent and receive replies | Email gateway adapter on this page | None beyond an IMAP/SMTP email account |
...
- **A dedicated email account** for your Tribal agent (don't use your personal email)
...
```bash
# Required
EMAIL_ADDRESS=tribal@gmail.com
...
Email access follows the same pattern as all other Tribal platforms:
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `tribal gateway status`. |
...
- The password is stored in `~/.tribal/.env` — protect this file (`chmod 600`)
