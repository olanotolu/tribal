...
> Run `tribal gateway setup` and pick **SimpleX** for a guided walk-through.
...
After starting the daemon, open a conversation with your agent contact. The contact ID will appear in session logs or via `tribal send_message action=list`.
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `tribal gateway pair`.
...
- The connection between Tribal and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
