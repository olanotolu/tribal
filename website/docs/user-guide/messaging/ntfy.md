...
ntfy makes a great lightweight push channel for Tribal: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `tribal gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `tribal-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Tribal dependency.
...
Add these to `~/.tribal/.env`:

```
NTFY_TOPIC=tribal-myname-2026
NTFY_ALLOWED_USERS=tribal-myname-2026
NTFY_HOME_CHANNEL=tribal-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Tribal adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
- Or **pick a long, unguessable topic name** (`tribal-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `tribal-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `tribal-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=tribal-myname-2026' >> ~/.tribal/.env
   echo 'NTFY_ALLOWED_USERS=tribal-myname-2026' >> ~/.tribal/.env
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Tribal replies:
...
If you only want Tribal to *push* notifications to ntfy (cron summaries, alerts) and never accept messages back, set both `NTFY_TOPIC` and `NTFY_PUBLISH_TOPIC` to the same value and skip `NTFY_ALLOWED_USERS` entirely. With no allowlist, the agent never responds to inbound messages — your phone gets the pushes, but the conversation is one-way.

## Limits

- **Message size**: ntfy caps message bodies at 4096 chars. Tribal truncates with a warning when this is exceeded.
