...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Tribal Agent — Webhooks Tutorial"
...
1. Enable via `tribal gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `tribal webhook subscribe`
...
Add the `github-pr` route to your `~/.tribal/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Tribal processes the event, and posts a review comment on the PR.
...
```bash
tribal webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `tribal webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
tribal webhook remove github-issues
```

### Test a subscription

```bash
tribal webhook test github-issues
tribal webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.tribal/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `tribal webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `tribal gateway run`
