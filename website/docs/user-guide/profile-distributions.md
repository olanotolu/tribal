...
A **profile distribution** packages a complete Tribal agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Tribal agent meant sending someone:
...
```bash
tribal profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `tribal profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Tribal in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`tribal profile export` / `import`](../reference/profile-commands.md#tribal-profile-export) — that's what those are for.
...
```bash
tribal profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.tribal/profiles/research-bot/SOUL.md
...
Create `~/.tribal/profiles/research-bot/distribution.yaml`:
...
```bash
cd ~/.tribal/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `tribal profile update research-bot` will pull the latest.
...
```bash
tribal profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.tribal/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
tribal profile install github.com/you/research-bot

# Full HTTPS
tribal profile install https://github.com/you/research-bot.git

# SSH
tribal profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
tribal profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
tribal profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
tribal profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
tribal profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
tribal profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Tribal distribution.
...
```bash
cp ~/.tribal/profiles/research-bot/.env.EXAMPLE ~/.tribal/profiles/research-bot/.env
...
```bash
tribal profile info research-bot
```
...
`tribal profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
tribal profile update research-bot
```
...
```bash
tribal profile delete research-bot
```
...
Path:    ~/.tribal/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `tribal profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
```bash
# Engineering lead
cd ~/.tribal/profiles/pr-reviewer
# ... build and tune ...
...
# Each engineer
tribal profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `tribal profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.tribal/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/tribal-polymarket-trader.git
git push -u origin main --tags

# Anyone
tribal profile install github.com/you/tribal-polymarket-trader --alias
```
...
You built Tribal-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
tribal profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
tribal profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `tribal profile info <name>` and hold off on updates until you're ready.
:::
...
tribal profile info research-bot | grep Version
...
```yaml
# ~/.tribal/profiles/research-bot/local/my-overrides.yaml
...
tribal profile delete research-bot --yes
tribal profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
tribal profile update research-bot --force-config --yes
```
...
tribal profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.tribal/profiles/forked-research-bot/
...
tribal profile install ~/.tribal/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
tribal profile delete research-bot-test --yes
tribal profile install ~/.tribal/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `tribal -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `tribal profile` — not a parallel command tree.
...
- Reserved profile names (`tribal`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`tribal profile export` / `import`](../reference/profile-commands.md#tribal-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Tribal](../guides/use-soul-with-tribal.md) — authoring personalities
