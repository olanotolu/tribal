...
Tribal Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.tribal/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Tribal stores session history so it can resume conversations, but it does not
...
Tribal explicitly injects for that turn.
...
For example, if a user sends an image and asks Tribal to make a meme from it,
Tribal may inspect that image once with vision and run an image-processing
...
`tribal sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`tribal` or `tribal chat`) |
...
tribal -c "my project"   # → resumes "my project #3"
```
...
tribal --resume 20250305_091523_a1b2c3d4
tribal -r 20250305_091523_a1b2c3d4

# Resume by title
tribal --resume "refactoring auth"

# Or with the chat subcommand
tribal chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `tribal sessions list`.

### Conversation Recap on Resume

When you resume a session, Tribal displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Tribal session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.tribal/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `tribal -r "<title>"` from the shell) and pick up where the platform left off.
...
Tribal automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `tribal sessions list` or `tribal sessions browse`.
...
```bash
tribal sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Tribal creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`tribal -c "my project"`), it automatically picks the most recent session in the lineage.
...
Tribal provides a full set of session management commands via `tribal sessions`:
...
# Filter by platform
tribal sessions list --source telegram

# Show more sessions
tribal sessions list --limit 50
```
...
tribal sessions export backup.jsonl

# Export sessions from a specific platform
tribal sessions export telegram-history.jsonl --source telegram

# Export a single session
tribal sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
tribal sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
tribal sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
tribal sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
tribal sessions prune

# Custom age threshold
tribal sessions prune --older-than 30

# Only prune sessions from a specific platform
tribal sessions prune --source telegram --older-than 60

# Skip confirmation
tribal sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`tribal insights`](/reference/cli-commands#tribal-insights).
...
When Tribal cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Tribal uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Tribal in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.tribal/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.tribal/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.tribal/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.tribal/sessions/`. They are no longer written or
read by Tribal. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Tribal process in the same `TRIBAL_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.tribal/config.yaml`:
...
tribal sessions delete <session_id>
