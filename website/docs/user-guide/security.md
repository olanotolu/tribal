...
Tribal Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Tribal checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.tribal/config.yaml`:
...
| `timeout` | `60` | Seconds Tribal waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `tribal --yolo` or `tribal chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `TRIBAL_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `TRIBAL_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Tribal shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Tribal refuses to run them **regardless** of:
...
Configure the timeout in `~/.tribal/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.tribal/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.tribal/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` tribal/gateway | Self-termination prevention |
...
The `TRIBAL_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.tribal/config.yaml`:
...
:::tip
Use `tribal config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Tribal controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.tribal/.env`:
...
For more flexible authorization, Tribal includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `tribal pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.tribal/config.yaml`:
...
tribal pairing approve telegram ABC12DEF

# Revoke a user's access
tribal pairing revoke telegram 123456789

# Clear all pending codes
tribal pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.tribal/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Tribal applies strict security hardening to every container.
...
Container resources are configurable in `~/.tribal/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.tribal/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `TRIBAL_HOME`. Skills declare these in frontmatter:
...
When loaded, Tribal checks if these files exist in the active profile's `TRIBAL_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Tribal infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Tribal infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Tribal integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Tribal under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.tribal/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.tribal/logs/` for unauthorized access attempts
10. **Keep updated** — run `tribal update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.tribal/.env
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.tribal/.env`:
...
TERMINAL_SSH_USER=tribal
TERMINAL_SSH_KEY=~/.ssh/tribal_agent_key
```
...
Tribal ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `tribal_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `tribal doctor` for the full remediation.
- **`tribal doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Tribal installs these **lazily** on first use rather than eagerly under `tribal-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `tribal tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `tribal tools`.
