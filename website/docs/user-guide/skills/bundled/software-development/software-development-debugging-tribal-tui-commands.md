---
title: "Debugging Tribal Tui Commands — Debug Tribal TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging Tribal Tui Commands"
description: "Debug Tribal TUI slash commands: Python, gateway, Ink UI"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Debugging Tribal Tui Commands

Debug Tribal TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-tribal-tui-commands` |
| Version | `1.0.0` |
| Author | Tribal Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `tribal-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging Tribal TUI Slash Commands

## Overview

Tribal slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the Tribal TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (tribal_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path tribal_cli/
   search_files --pattern "commandname" --path tribal_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `TribalCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `tribal_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/tribal-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.tribal/config.yaml`)
