...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-tribal-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-tribal-tui-commands) |
...
- A long-running process (tribal gateway, tui_gateway) misbehaves and you can't restart it
...
The tribal test runner and pytest both support this:
...
For long-lived processes: Tribal gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/tribal-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Tribal you have two practical options:
...
```json
{
  "name": "Attach to Tribal",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/tribal-agent" }
  ]
}
```
...
## Debugging Tribal-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `tribal` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `tribal --tui`)
...
debugpy.wait_for_client()
```
Start `tribal --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Tribal subagents, debug one process at a time.
