...
description: "Use when a Tribal Kanban worker wants to run Codex CLI as an isolated implementation lane while Tribal keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Tribal Kanban worker wants to run Codex CLI as an isolated implementation lane while Tribal keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`tribal-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-tribal-agent) |
...
- A bounded diff can be evaluated by Tribal in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Tribal can run the relevant tests itself after Codex exits.
...
1. Tribal owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Tribal board CLI as a substitute for the worker.
2. Tribal owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Tribal owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Tribal with the repo's canonical wrapper.
4. Tribal owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Tribal owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${TRIBAL_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Tribal edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Tribal can do the task directly.
...
Tribal owns the Kanban lifecycle; do not call Tribal kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Tribal reconciliation.
...
- Explicit statement: Tribal owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Tribal will run afterward.
...
Tribal must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Tribal.
...
- [ ] Tribal ran the canonical tests itself, using `scripts/run_tests.sh` for Tribal Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Tribal-run tests.
- [ ] Accepted commits/diffs were applied to the Tribal-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "tribal"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Tribal edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Tribal.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Tribal writes board state.
...
- [ ] Tribal reviewed `git diff` and safety-sensitive files.
- [ ] Tribal ran canonical tests independently.
