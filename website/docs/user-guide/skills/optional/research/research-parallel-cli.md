...
| | |
|---|---|
| Source | Optional — install with `tribal skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Tribal core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Tribal native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Tribal native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Tribal native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Tribal usage patterns
...
- Do not silently choose Parallel when Tribal native tools are already sufficient.
