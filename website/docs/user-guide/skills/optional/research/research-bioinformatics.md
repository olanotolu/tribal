...
| | |
|---|---|
| Source | Optional — install with `tribal skills install official/research/bioinformatics` |
...
3. Read the specific skill:
   ...
   ```
4. Follow the fetched skill as reference material. These are NOT Tribal-format skills — treat them as expert domain guides. They contain correct parameters, proper tool flags, and validated pipelines.
...
- The fetched skills are NOT in Tribal SKILL.md format. They use their own structure (bioSkills: code pattern cookbooks; ClawBio: README + Python scripts). Read them as expert reference material.
