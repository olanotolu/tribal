---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Tribal Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Tribal Agent"
---
...
Migrate a user's OpenClaw customization footprint into Tribal Agent. Imports Tribal-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `tribal skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Tribal Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Tribal`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Tribal Migration

Use this skill when a user wants to move their OpenClaw setup into Tribal Agent with minimal manual cleanup.
...
```bash
tribal claw migrate              # Full interactive migration
tribal claw migrate --dry-run    # Preview what would be migrated
tribal claw migrate --preset user-data   # Migrate without secrets
tribal claw migrate --overwrite  # Overwrite existing conflicts
tribal claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `tribal setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_tribal.py` to:

- import `SOUL.md` into the Tribal home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Tribal memory entries
- merge OpenClaw command approval patterns into Tribal `command_allowlist`
- migrate Tribal-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.tribal/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Tribal workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.tribal/tts/`
- archive non-secret docs that do not have a direct Tribal destination
...
- `scripts/openclaw_to_tribal.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py`

Do not guess a shorter path like `~/.tribal/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.tribal/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Tribal-compatible secrets, currently:
...
Tribal CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Tribal. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Tribal workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Tribal skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.tribal/skills/migration/openclaw-migration/scripts/openclaw_to_tribal.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Tribal unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Tribal targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Tribal destination. Unsupported auth blobs must still be reported as skipped.
...
- Tribal persona state imported
- Tribal memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.tribal/skills/openclaw-imports/`
