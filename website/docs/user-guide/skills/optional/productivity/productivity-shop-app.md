...
| | |
|---|---|
| Source | Optional — install with `tribal skills install official/productivity/shop-app` |
...
Tribal's gateway adapters (Telegram, Discord, Slack, iMessage, …) render markdown and image URLs automatically. Write normal markdown with image URLs on their own line — the adapter handles platform-specific layout. Do **not** invent a `message()` tool call (that belongs to Shop.app's own runtime, not Tribal).
