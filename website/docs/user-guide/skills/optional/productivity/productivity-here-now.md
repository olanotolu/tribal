...
| | |
|---|---|
| Source | Optional — install with `tribal skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${TRIBAL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TRIBAL_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `tribal`)    |
