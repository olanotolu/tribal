...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Tribal. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/nicholasgasior/gws) when available for broader coverage, and falls back to Google's Python client libraries otherwise.

**Skill path:** `skills/productivity/google-workspace/`

## Setup

The setup is fully agent-driven — ask Tribal to set up Google Workspace and it walks you through each step. The flow:
...
3. **Authorize** — Tribal generates an auth URL, you approve in the browser, paste back the redirect URL
...
| Problem | Fix |
|---------|-----|
| `NOT_AUTHENTICATED` | Run setup (ask Tribal to set up Google Workspace) |
