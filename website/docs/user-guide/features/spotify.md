...
Tribal can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.tribal/auth.json` and refreshed automatically on 401; you only log in once per machine.

Unlike Tribal' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `tribal auth spotify` walks you through it.
...
- Tribal Agent installed and running.
...
### One-shot: `tribal tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `tribal setup` / `tribal setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `tribal tools`.

Tribal drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the toolset is enabled AND authenticated in one pass.
...
If no `TRIBAL_SPOTIFY_CLIENT_ID` is set, Tribal walks you through the app registration inline:
...
4. Saves it to `~/.tribal/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.tribal/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
| Field | Value |
|-------|-------|
| App name | anything (e.g. `tribal-agent`) |
| App description | anything (e.g. `personal Tribal integration`) |
| Website | leave blank |
| Redirect URI | `http://127.0.0.1:43827/spotify/callback` |
| Which API/SDKs? | check **Web API** |

Agree to the terms and click **Save**. On the next page click **Settings** → copy the **Client ID** and paste it into the Tribal prompt. That's the only value Tribal needs — PKCE doesn't use a client secret.

### Running over SSH / in a headless environment

If `SSH_CLIENT` or `SSH_TTY` is set, Tribal skips the automatic browser open during both the wizard and the OAuth step. Copy the dashboard URL and the authorization URL Tribal prints, open them in a browser on your local machine, and proceed normally — the local HTTP listener still runs on the remote host on port `43827`. Your laptop's browser can't reach the remote loopback without an SSH local-forward:
...
```bash
tribal auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Tribal restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `tribal auth logout spotify`.
...
Because Spotify tools are regular Tribal tools, a cron job running in a Tribal session can trigger playback on any schedule. No new code needed.
...
1. Cron spins up a headless Tribal session.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `tribal tools` for the cron session to see the Spotify tools.
...
```bash
tribal auth logout spotify
```

Removes tokens from `~/.tribal/auth.json`. To also clear the app config, delete `TRIBAL_SPOTIFY_CLIENT_ID` (and `TRIBAL_SPOTIFY_REDIRECT_URI` if you set it) from `~/.tribal/.env`, or run the wizard again.
...
**`204 No Content` on `get_currently_playing`** — nothing is currently playing on any device. This is Spotify's normal response, not an error; Tribal surfaces it as an explanatory empty result (`is_playing: false`).

**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Tribal is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `TRIBAL_SPOTIFY_REDIRECT_URI` in `~/.tribal/.env` to whatever you registered.

**`429 Too Many Requests`** — Spotify's rate limit. Tribal returns a friendly error; wait a minute and retry. If this persists, you're probably running a tight loop in a script — Spotify's quota resets roughly every 30 seconds.

**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `tribal auth spotify` again.

**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, Tribal detects it and skips the auto-open. Copy the dashboard URL it prints and open it manually.

## Advanced: custom scopes

By default Tribal requests the scopes needed for every shipped tool. Override if you want to restrict access:

```bash
tribal auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
tribal auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.tribal/.env`:

```
TRIBAL_SPOTIFY_CLIENT_ID=<your_id>
TRIBAL_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| File | Contents |
|------|----------|
| `~/.tribal/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.tribal/.env` | `TRIBAL_SPOTIFY_CLIENT_ID`, optional `TRIBAL_SPOTIFY_REDIRECT_URI` |
