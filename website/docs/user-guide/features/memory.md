...
description: "How Tribal Agent remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Tribal Agent has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
Both are stored in `~/.tribal/memories/` and are injected into the system prompt as a frozen snapshot at session start. The agent manages its own memory via the `memory` tool — it can add, replace, or remove entries.
...
- All CLI and messaging sessions are stored in SQLite (`~/.tribal/state.db`) with FTS5 full-text search
...
```bash
tribal sessions list    # Browse past sessions
```
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Tribal ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
tribal memory setup      # pick a provider and configure it
