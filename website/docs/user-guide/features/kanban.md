...
description: "Durable SQLite-backed task board for coordinating multiple Tribal profiles"
---
...
Tribal Kanban is a durable task board, shared across all your Tribal profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.tribal/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.tribal/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `tribal kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `tribal kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/tribal-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `TRIBAL_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
tribal kanban boards list

# Create a new board.
tribal kanban boards create atm10-server \
    --name "ATM10 Server" \
...
tribal kanban --board atm10-server list
tribal kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
tribal kanban boards switch atm10-server
tribal kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
tribal kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `TRIBAL_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.tribal/kanban/current` — the slug persisted by `tribal kanban
   boards switch`.
4. `default`.
...
`tribal dashboard` → Kanban tab shows a board switcher at the top as soon
...
tribal gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
tribal kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `tribal kanban show t_abcd`.
...
Override the config flag at runtime via `TRIBAL_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `tribal gateway
...
until one comes up — `tribal kanban create` warns about this at creation
time.

Running `tribal kanban daemon` as a separate process is **deprecated**;
...
tribal kanban create "nightly ops review" \
    --assignee ops \
...
```bash
tribal kanban complete t_abc t_def t_hij --result "batch wrap"
tribal kanban archive  t_abc t_def t_hij
tribal kanban unblock  t_abc t_def
tribal kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `tribal kanban`.** When the dispatcher spawns a worker it sets `TRIBAL_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `tribal kanban` CLI.
...
kanban_show()                                     # no args — uses TRIBAL_KANBAN_TASK
...
### Why tools instead of shelling to `tribal kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `tribal kanban complete` *inside* the container, where `tribal` isn't installed and `~/.tribal/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.tribal/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `tribal chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `TRIBAL_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/tribal_cli/test_kanban_db.py -q"],
...
2. `cd $TRIBAL_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
tribal -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
tribal -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
tribal kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

tribal kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
tribal -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
tribal -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `tribal kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `tribal kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `tribal profile create --description "..."`, `tribal profile describe <name> --text "..."`, `tribal profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.tribal/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `tribal profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.tribal/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Tribal dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
tribal kanban init                                     # create kanban.db + print daemon hint
tribal kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
tribal kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
tribal kanban show <id> [--json]
tribal kanban assign <id> <profile>                    # or 'none' to unassign
tribal kanban link <parent_id> <child_id>
tribal kanban unlink <parent_id> <child_id>
tribal kanban claim <id> [--ttl SECONDS]
tribal kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
tribal kanban complete <id>... [--result "..."]
tribal kanban block <id> "<reason>" [--ids <id>...]
tribal kanban unblock <id>...
tribal kanban archive <id>...

tribal kanban tail <id>                                # follow a single task's event stream
tribal kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
tribal kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
tribal kanban runs <id> [--json]                       # attempt history (one row per run)
tribal kanban assignees [--json]                       # profiles on disk + per-assignee task counts
tribal kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
tribal kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `tribal gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
tribal kanban stats [--json]                           # per-status + per-assignee counts
tribal kanban log <id> [--tail BYTES]                  # worker log from ~/.tribal/kanban/logs/
tribal kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
tribal kanban notify-list [<id>] [--json]
tribal kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
tribal kanban context <id>                             # what a worker sees
tribal kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
tribal kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
tribal kanban create "nightly backup audit" \
...
Every `tribal kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `tribal chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `tribal_cli.kanban.run_slash()` entry point that reuses the `tribal kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `tribal kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.tribal/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`tribal kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `tribal kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/tribal-kanban-v1-spec.pdf`.
...
```bash
tribal kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$TRIBAL_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
tribal kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
tribal kanban notify-list
tribal kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
tribal kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
tribal kanban runs t_abcd
...
**Bulk close caveat.** `tribal kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`tribal kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `tribal kanban heartbeat $TASK` to signal liveness during long operations. |
...
`tribal kanban tail <id>` shows these for a single task. `tribal kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.tribal/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/tribal-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
