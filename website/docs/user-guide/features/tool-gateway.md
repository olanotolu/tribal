...
The Tool Gateway is included with every paid [Nous Portal](https://portal.nousresearch.com) subscription. It routes Tribal' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Tribal default to FLUX 2 Klein. |
...
```bash
tribal setup --portal     # Nous OAuth, set Nous as provider, and turn on the Tool Gateway in one go
```

Already have Tribal configured? Just switch your provider:

```bash
tribal model              # Pick Nous Portal — Tribal will offer to turn on the Tool Gateway
```

When you select Nous Portal, Tribal offers to turn on the Tool Gateway. Accept, and you're done — every supported tool is live on the next run.

Check what's active at any time:

```bash
tribal portal status      # Portal auth + Tool Gateway routing summary
tribal portal tools       # Gateway catalog with current routing per tool
tribal status             # Full system status (Tool Gateway is one section)
```

`tribal portal status` shows a section like:
...
```bash
tribal tools          # Interactive picker for each tool category
```
...
The set evolves — `tribal tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `tribal model` and `tribal tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # Tribal now uses FIRECRAWL_API_KEY from .env
```

`tribal tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.tribal/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `tribal tools`. Tribal shows a clear error pointing at the portal.
...
Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `tribal setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, Tribal skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
