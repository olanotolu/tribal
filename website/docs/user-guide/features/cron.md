...
Schedule tasks to run automatically with natural language or cron expressions. Tribal exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Tribal itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
Cron jobs use whatever provider `tribal model` selected. `tribal setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Tribal disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
tribal cron create "every 2h" "Check server status"
tribal cron create "every 1h" "Summarize new feed items" --skill blogwatcher
tribal cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Tribal will use the unified `cronjob` tool internally.
...
tribal cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever Tribal profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `TRIBAL_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
tribal cron create "every 1d at 03:00" \
...
:::note Serialization
Jobs with a `profile` set also run sequentially, for the same reason as `workdir`-pinned jobs: switching `TRIBAL_HOME` is a process-global mutation, so two profile-pinned jobs running in parallel would race each other. Unpinned jobs still run in the normal parallel pool.
:::
...
```bash
tribal cron edit <job_id> --schedule "every 4h"
tribal cron edit <job_id> --prompt "Use the revised task"
tribal cron edit <job_id> --skill blogwatcher --skill maps
tribal cron edit <job_id> --add-skill maps
tribal cron edit <job_id> --remove-skill blogwatcher
tribal cron edit <job_id> --clear-skills
```
...
```bash
tribal cron list
tribal cron pause <job_id_or_name>
tribal cron resume <job_id_or_name>
tribal cron run <job_id_or_name>
tribal cron remove <job_id_or_name>
tribal cron edit <job_id_or_name> [...flags]
tribal cron status
tribal cron tick
```
...
```bash
tribal gateway install     # Install as a user service
sudo tribal gateway install --system   # Linux: boot-time system service for servers
tribal gateway             # Or run in foreground
...
1. loads jobs from `~/.tribal/cron/jobs.json`
...
A file lock at `~/.tribal/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.tribal/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.tribal/cron/output/`), but no message is sent to the delivery target.
...
Or set the `TRIBAL_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.tribal/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Tribal directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Tribal will write the check script to `~/.tribal/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.tribal/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.tribal/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.tribal/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.tribal/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Tribal reads Job 1's most recent output from `~/.tribal/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Tribal skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `tribal tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `tribal tools` cron-platform config wins; otherwise Tribal falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Tribal should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.tribal/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.tribal/scripts/.feed-changed.last"
...
:::tip
Tribal's own `~/.tribal/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/Tribal/tribal/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.tribal/cron/jobs.json`. Output from job runs is saved to `~/.tribal/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Tribal resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
