...
When Tribal Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.tribal/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.tribal/config.yaml` (switchable per session
via `/personality`).
...
If you use Tribal' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.tribal/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Tribal Agent's deliverable
