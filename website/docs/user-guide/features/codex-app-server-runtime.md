...
Tribal can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Tribal becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Tribal behavior is unchanged unless you flip the flag. Tribal never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `tribal setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Tribal session.
- **Tribal' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Tribal for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Tribal' message shape so the self-improvement loop sees a normal-looking transcript.
...
These ship with `codex app-server` itself — no Tribal involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Tribal' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Tribal also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Tribal queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Tribal tool callback (MCP server, registered in `~/.codex/config.toml`)

Tribal registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Tribal' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Tribal' skill library.
- **`text_to_speech`** — TTS through Tribal' configured provider.

When the model wants one of these, codex spawns the `tribal_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Tribal' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Tribal tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Tribal' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Tribal' todo store (codex's `update_plan` is the in-runtime equivalent)
...
- The Tribal tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `TRIBAL_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `TRIBAL_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `TRIBAL_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `tribal-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Tribal also passes narrow app-server sandbox overrides when `TRIBAL_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`TRIBAL_KANBAN_WORKSPACES_ROOT`, `TRIBAL_KANBAN_WORKSPACE`, legacy `TRIBAL_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/Tribal/tribal/issues/27941)).
...
|  | Tribal default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Tribal' own `tribal auth login codex` writes to `~/.tribal/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Tribal auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Tribal will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
- Migrates user MCP servers from `~/.tribal/config.yaml` to `~/.codex/config.toml`.
...
- **Registers Tribal' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
You can also set it manually in `~/.tribal/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Tribal runtime)
```

## Self-improvement loop (memory + skill nudges)

Tribal' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Tribal runtime.
...
One detail: the review fork itself needs to call Tribal' agent-loop tools (`memory`, `skill_manage`), which require Tribal' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Tribal owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Tribal' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Tribal shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Tribal' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Tribal' managed block:

```toml
default_permissions = ":read-only"
```

(Tribal will preserve your override on re-migration as long as it lives outside the `# managed by tribal-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Tribal' auxiliary client uses the main provider/model when no per-task override is set.
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.tribal/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Tribal downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Tribal' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Tribal wraps everything it manages between two marker comments:

```toml
# managed by tribal-agent — `tribal codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end tribal-agent managed section
```
...
- Add your own MCP servers Tribal doesn't know about
...
By default, Tribal points the codex subprocess at `~/.codex/` regardless of which Tribal profile is active. This means `tribal -p work` and `tribal -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `TRIBAL_HOME`:

```bash
# Inside the work profile, you might wrap tribal:
CODEX_HOME=~/.tribal/profiles/work/codex tribal chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `tribal -p work` will operate on isolated Codex state.
...
Tribal does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Tribal' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Tribal (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Tribal-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Tribal writes a `[plugins."<name>@openai-curated"]` block enabling it in your Tribal session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Tribal' codex runtime, Tribal activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Tribal doesn't touch credentials.

## Tribal tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Tribal registers itself as an MCP server in `~/.codex/config.toml`:

```toml
[mcp_servers.tribal-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.tribal_tools_mcp_server"]
env = { TRIBAL_HOME = "/your/.tribal", PYTHONPATH = "...", TRIBAL_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed Tribal tool), codex spawns the `tribal_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Tribal runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Tribal Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Tribal UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `tribal-tools` callback)
...
- Tribal web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Tribal auth and codex auth are separate sessions.** You need both `codex login` AND `tribal auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Tribal' `_import_codex_cli_tokens` — Tribal won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/Tribal/tribal/issues) with the output of `tribal logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Tribal shell (CLI / TUI / gateway) ───┐
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  tribal_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For implementation details, see [PR #24182](https://github.com/Tribal/tribal/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
