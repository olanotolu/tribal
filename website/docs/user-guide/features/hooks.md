...
Tribal has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.tribal/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.tribal/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.tribal/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".tribal" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.tribal/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.tribal/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".tribal" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.tribal/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.tribal/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/tribal-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.tribal/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Tribal does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.tribal/BOOT.md` with natural-language startup instructions.
...
Create `~/.tribal/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `tribal cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.tribal/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.tribal/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.tribal/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.tribal/hooks/boot-md/handler.py`**

```python
"""Run ~/.tribal/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".tribal" / "BOOT.md"
...
```bash
tribal logs --follow --level INFO | grep boot-md
```
...
Delete `~/.tribal/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Tribal shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.tribal/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Tribal's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/guides/build-a-tribal-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Tribal will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`tribal_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.tribal/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.tribal/agent-hooks/` (by convention) | `~/.tribal/plugins/<name>/` | `~/.tribal/hooks/<name>/` |
...
Each time the event fires, Tribal spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Tribal-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.tribal/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.tribal/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.tribal/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.tribal/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.tribal/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.tribal/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Tribal event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.tribal/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.tribal/agent-hooks/log-orchestration.sh
log=~/.tribal/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Tribal sees it, then persists the decision to `~/.tribal/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `tribal --accept-hooks chat`)
2. `TRIBAL_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `tribal hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `tribal hooks` CLI

| Command | What it does |
|---------|--------------|
| `tribal hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `tribal hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `tribal hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `tribal hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.tribal/agent-hooks/` so the path is easy to audit.
- Re-run `tribal hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
