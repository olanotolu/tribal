...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Tribal Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Tribal Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Tribal automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Tribal — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Tribal opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.tribal/auth.json` and refreshed automatically before they expire.
...
```bash
tribal auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Tribal detects the remote environment and prints the authorization URL instead of opening a browser.
...
tribal auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Tribal inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Tribal skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
tribal auth add xai-oauth --manual-paste
# Or via the model picker:
tribal model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/Tribal/tribal/issues/26923).

## How the Login Works

1. Tribal opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Tribal and the tokens are saved to `~/.tribal/auth.json`.
4. From then on, Tribal refreshes the access token in the background — you stay signed in until you `tribal auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
tribal config set model.default grok-4.3
tribal config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.tribal/config.yaml` will contain:
...
```bash
tribal --provider xai-oauth        # canonical
tribal --provider grok-oauth       # alias
tribal --provider x-ai-oauth       # alias
tribal --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `tribal tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `tribal tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `tribal setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

Tribal refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Tribal surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Tribal marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `tribal auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Tribal raises a timeout error.

**Fix:** re-run `tribal auth add xai-oauth` (or `tribal model`). The flow starts fresh.

### State mismatch (possible CSRF)

Tribal detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Tribal prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `tribal model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/Tribal/tribal/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
tribal config set model.provider xai
```
...
**Fix:** run `tribal model` and pick the xAI Grok OAuth provider, or run `tribal auth add xai-oauth`.
...
```bash
tribal auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `tribal auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `tribal auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Tribal is on a different machine than your browser
