---
sidebar_position: 9
title: "Run Tribal Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Tribal Agent entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run Tribal Locally with Ollama — Zero API Cost
...
You'll set up Tribal Agent running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Tribal works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Tribal connected to Ollama as a custom endpoint
...
```bash
# ~/.tribal/.env
TRIBAL_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Tribal is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Tribal experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Tribal with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Alternatively, edit `~/.tribal/config.yaml` directly:
...
Tribal will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Tribal requires at least 64,000 tokens for agentic work with tools:
...
Then update your Tribal config to use `gemma4-64k` as the model name.
...
Once Tribal works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.

### Telegram

1. Create a bot via [@BotFather](https://t.me/BotFather) and get the token
2. Add to your `~/.tribal/config.yaml`:
...
- **Tribal has auto-repair** — it detects malformed tool calls and attempts to fix them automatically.
- **Set up a fallback** — if the local model fails 3 times, Tribal falls back to a cloud provider.
...
- **Terminal commands** — Tribal wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below Tribal' 64K minimum unless you configure them
