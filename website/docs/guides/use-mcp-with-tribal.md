---
sidebar_position: 6
title: "Use MCP with Tribal"
description: "A practical guide to connecting MCP servers to Tribal Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Tribal

This guide shows how to actually use MCP with Tribal Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Tribal tool
- you want Tribal to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Tribal to internal APIs, databases, or company systems without modifying Tribal core

Do not use MCP when:
- a built-in Tribal tool already solves the job well
...
- Tribal remains the agent
- MCP servers contribute tools
- Tribal discovers those tools at startup or reload time
...
If you installed Tribal with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Tribal banner/status should show MCP integration when configured
- ask Tribal what tools it has available
...
## WSL2: bridge Tribal in WSL to Windows Chrome

This is the practical setup when:

- Tribal runs inside WSL2
...
In this setup, Tribal does **not** connect to Chrome directly. Instead:

- Tribal runs in WSL
- Tribal starts a local stdio MCP server
...
```text
Tribal (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Tribal stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Tribal core browser transport
...
```bash
tribal mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
tribal mcp test chrome-devtools-win
```

Then start a fresh Tribal session or run:
...
Once loaded, Tribal can use the MCP-prefixed browser tools directly. For example:
...
If Tribal runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Tribal from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Tribal from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Tribal:
...
2. Tribal-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Tribal will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Tribal to reason over a bounded workspace.
...
Now Tribal can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Tribal core.
...
Because Tribal now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
