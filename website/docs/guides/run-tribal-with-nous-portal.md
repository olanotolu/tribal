...
3. Sets `model.provider: nous` in `~/.tribal/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Tribal is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
tribal setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
tribal auth add nous --type oauth --manual-paste
# Then re-run `tribal setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Tribal Agent release notes" and summarize the top 3 hits.
```

You should see Tribal call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `tribal setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
tribal config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Tribal-4 for agent work

Tribal-4-70B and Tribal-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Tribal Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Tribal-side opinion.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Tribal will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
tribal cron create "every day at 9am" \
...
If you use [Tribal profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.tribal/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `tribal portal status` shows "not logged in" after `tribal setup --portal`
...
```bash
tribal config set model.provider nous
```
...
Re-verify with `tribal portal status`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Tribal doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/Tribal/tribal/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`tribal portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Tribal profiles where you're using the wrong one (check `tribal profile current`)

### Want to revoke and start clean

```bash
tribal auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Tribal tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Tribal configurations
