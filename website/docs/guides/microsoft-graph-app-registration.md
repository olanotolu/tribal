...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.tribal/.env` at the end.
...
4. Fill in:
   - **Name:** `Tribal Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `tribal-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.tribal/.env`:
...
Tribal ships a Graph auth smoke-test. From your Tribal install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.tribal/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `tribal gateway restart`.
