...
title: "Build a Tribal Plugin"
description: "Step-by-step guide to building a complete Tribal plugin with tools, hooks, data files, and skills"
---

# Build a Tribal Plugin

This guide walks through building a complete Tribal plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Tribal has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `TRIBAL_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.tribal/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `tribal skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.tribal/plugins/calculator
cd ~/.tribal/plugins/calculator
```
...
This tells Tribal: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Tribal may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `tribal my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Tribal continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `TRIBAL_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
TRIBAL_PLUGINS_DEBUG=1 tribal plugins list
```
...
The same logs are always written to `~/.tribal/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
tribal logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `tribal plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.tribal/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.tribal/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.tribal/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.tribal/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `tribal plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Tribal into installing arbitrary packages — only specs Tribal itself ships are eligible |
...
auditable in `~/.tribal/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Tribal injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: tribal-agent"}
...
- **The system prompt is Tribal's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `tribal <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for tribal my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: tribal my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for tribal my-plugin."""
...
After registration, users can run `tribal my-plugin status`, `tribal my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `tribal name` in a terminal |
...
Tribal has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.tribal/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `tribal gateway status` without SDK instantiation.
...
Tribal also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Tribal without any Python plugin. Declare them in `~/.tribal/config.yaml`:
...
Tribal connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.tribal/hooks/<name>/`:

```yaml
# ~/.tribal/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.tribal/hooks/long-task-alert/handler.py
...
tribal skills search my-workflow --source myorg/skills-repo
tribal skills install myorg/skills-repo/my-workflow
```
...
For STT, point `TRIBAL_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."tribal_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install tribal-plugin-calculator
# Plugin auto-discovered on next tribal startup
```
...
```python
# Wrong — will break if Tribal passes extra context
def handler(args):
    ...
