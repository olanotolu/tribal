...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Tribal Agent — no API key required"
---

# MiniMax OAuth

Tribal Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Tribal automatically refreshes your session.
...
# → Tribal opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.tribal/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.tribal/.env
```
...
```bash
tribal auth add minimax-oauth --no-browser
```

Tribal will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Tribal implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Tribal generates a PKCE verifier / challenge pair and a random state value.
...
4. Tribal polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.tribal/auth.json` under the `minimax-oauth` key.
...
```bash
tribal config set model.default MiniMax-M2.7
tribal config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.tribal/config.yaml` will contain entries similar to:
...
```bash
tribal --provider minimax-oauth    # canonical
tribal --provider minimax-portal   # alias
tribal --provider minimax-global   # alias
tribal --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `tribal setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
tribal --provider minimax-oauth
```
...
Tribal refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Tribal marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Tribal marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `tribal auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Tribal raises a timeout error.

**Fix:** re-run `tribal auth add minimax-oauth` (or `tribal model`). The flow starts fresh.

### State mismatch (possible CSRF)

Tribal detected that the `state` value returned by the authorization server does not match what it sent.
...
If `tribal` cannot open a browser window, use `--no-browser`:

```bash
tribal auth add minimax-oauth --no-browser
```

Tribal prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `tribal model` and select MiniMax (OAuth), or run `tribal auth add minimax-oauth`.
...
```bash
tribal auth remove minimax-oauth
```
...
- [tribal doctor](../reference/cli-commands.md)
