...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Tribal new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Tribal how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Tribal installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Tribal to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Tribal but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
tribal skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.tribal/skills/`
...
For how to ship skills in your own plugin, see [Build a Tribal Plugin → Bundle skills](/guides/build-a-tribal-plugin#bundle-skills).
...
When a skill with config is first loaded, Tribal prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
tribal skills config gif-search

# View all skill config
tribal config get skills.config
```
...
```bash
mkdir -p ~/.tribal/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.tribal/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
tribal chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.tribal/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Tribal may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Tribal will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.tribal/skills/devops/`, `~/.tribal/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Tribal to update the skill with what you learned. Skills that aren't maintained become liabilities.
