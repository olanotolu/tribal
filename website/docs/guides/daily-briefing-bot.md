...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `tribal setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Tribal spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.tribal/cron/output/` and you can read them anytime.
:::
...
Tribal will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Tribal Agent
```
...
Before creating cron jobs, ensure Tribal has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Tribal what you want:
...
Tribal will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Tribal to delegate each topic to a sub-agent:
...
Tribal will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
