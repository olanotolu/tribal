---
sidebar_position: 8
title: "Use Voice Mode with Tribal"
description: "A practical guide to setting up and using Tribal voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Tribal
...
- you want Tribal sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Tribal.
...
## Step 1: make sure normal Tribal works first
...
Tribal supports both local and cloud speech stacks.
...
### If you use `tribal setup`

If you choose NeuTTS in the setup wizard, Tribal checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Tribal transcribes and responds
...
- asking Tribal to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Tribal loop.

## Tuning CLI behavior

### Silence threshold

If Tribal starts/stops too aggressively, tune:
...
Tribal stays a normal chat bot, but can speak replies.
...
- you want Tribal to function like a portable research or ops assistant
...
Tribal joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Tribal detects speech boundaries
- transcripts are posted in the associated text channel
- Tribal responds in text and audio
...
1. get text Tribal working
2. install `tribal-agent[voice]`
