...
This tutorial walks you through setting up a Telegram bot powered by Tribal Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Tribal Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.tribal/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Tribal itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Tribal Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_tribal_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Tribal Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Tribal Gateway...
...
# View live logs
journalctl --user -u tribal-gateway -f
...
tribal gateway start
tribal gateway stop
tail -f ~/.tribal/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `tribal gateway install` to update the plist.
:::
...
```bash
tribal gateway stop && tribal gateway start
```
...
tribal pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.tribal/.env`:
...
Control how much detail the bot shows when using tools. In `~/.tribal/config.yaml`:
...
Customize how the bot communicates by editing `~/.tribal/SOUL.md`:

For a full guide, see [Use SOUL.md with Tribal](/guides/use-soul-with-tribal).
...
```bash
# From the CLI
tribal cron list          # View all scheduled jobs
tribal cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u tribal-gateway` (Linux) or `~/.tribal/logs/gateway.log` (macOS) |
| Cron job output | `~/.tribal/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.tribal/cron/jobs.json` |
| Pairing data | `~/.tribal/pairing/` |
| Session history | `~/.tribal/sessions/` |
