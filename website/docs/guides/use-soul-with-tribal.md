---
sidebar_position: 7
title: "Use SOUL.md with Tribal"
description: "How to use SOUL.md to shape Tribal Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Tribal

`SOUL.md` is the **primary identity** for your Tribal instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Tribal to feel like the same assistant every time you talk to it — or if you want to replace the Tribal persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Tribal should be
- what Tribal should avoid stylistically
- how Tribal should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Tribal is and how Tribal speaks
...
Tribal now uses only the global SOUL file for the current instance:

```text
~/.tribal/SOUL.md
```

If you run Tribal with a custom home directory, it becomes:
...
Tribal automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Tribal does not overwrite it
- if the file exists but is empty, Tribal adds nothing from it to the prompt

## How Tribal uses it

When Tribal starts a session, it reads `SOUL.md` from `TRIBAL_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Tribal feels.
...
Tribal already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Tribal should sound.

# Avoid
What Tribal should not do.

# Defaults
How Tribal should behave when ambiguity appears.
```
...
Then restart Tribal or start a new session.
...
### I edited SOUL.md but Tribal still sounds the same

Check:
- you edited `~/.tribal/SOUL.md` or `$TRIBAL_HOME/SOUL.md`
...
### Tribal is ignoring parts of my SOUL.md
