...
description: "How Tribal stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `tribal_cli/cron.py` | CLI `tribal cron` subcommands |
...
Jobs are stored in `~/.tribal/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `tribal cron` commands are run or during active CLI sessions.
...
```python
# ~/.tribal/scripts/check_competitors.py
...
2. **Environment variable** — `TRIBAL_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.tribal/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `tribal cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `tribal cron` CLI provides direct job management:

```bash
tribal cron list                    # Show all jobs
tribal cron create                  # Interactive job creation (alias: add)
tribal cron edit <job_id>           # Edit job configuration
tribal cron pause <job_id>          # Pause a running job
tribal cron resume <job_id>         # Resume a paused job
tribal cron run <job_id>            # Trigger immediate execution
tribal cron remove <job_id>         # Delete a job
```
