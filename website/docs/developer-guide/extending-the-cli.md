...
description: "Build wrapper CLIs that extend the Tribal TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Tribal exposes protected extension hooks on `TribalCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Tribal."""
...
Called after Tribal registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
