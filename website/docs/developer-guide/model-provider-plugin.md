...
description: "How to build a model provider (inference backend) plugin for Tribal Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Tribal can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$TRIBAL_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with Tribal
2. **User plugins** — `$TRIBAL_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$TRIBAL_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `tribal plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `tribal_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `tribal_cli/main.py` | Accepts `acme-inference` |
| `tribal model` picker | `tribal_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `tribal doctor` | `tribal_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `tribal setup` | `tribal_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `tribal_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `tribal model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.tribal/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Tribal picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Tribal' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `TRIBAL_HOME` at a temp directory so you don't pollute your real config:

```bash
export TRIBAL_HOME=/tmp/tribal-plugin-test
mkdir -p $TRIBAL_HOME/plugins/model-providers/my-provider
cat > $TRIBAL_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
tribal -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `tribal plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$TRIBAL_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any Tribal plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."tribal.plugins"]
acme-inference = "acme_tribal_plugin:register"
```

…where `acme_tribal_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Tribal Plugin](/guides/build-a-tribal-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Tribal Plugin](/guides/build-a-tribal-plugin) — general plugin authoring
