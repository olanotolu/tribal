...
This guide covers adding a new messaging platform to the Tribal gateway. A platform adapter connects Tribal to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.tribal/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Tribal code. Your plugin is a directory with two files:

```
~/.tribal/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `tribal config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-tribal-config) below).
...
| `tribal config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `tribal status` | Shows plugin platforms with `(plugin)` tag |
| `tribal gateway setup` | Plugin platforms appear in setup menu |
| `tribal tools` / `tribal skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.tribal/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `tribal gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `tribal cron run` separate from `tribal gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `tribal config`

`tribal_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Tribal Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #tribal)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Tribal core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`tribal_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`tribal_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`tribal_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`tribal_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`tribal_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`tribal_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"tribal-newplat"` toolset definition with `_TRIBAL_CORE_TOOLS`
2. **`toolsets.py`** — Add `"tribal-newplat"` to the `"tribal-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | tribal-newplat toolset |
