...
Tribal Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `tribal plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Tribal has two separate compression layers that operate independently:
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Tribal
