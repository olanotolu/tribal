...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Tribal already has applies. The plugin never sees a
...
[`tribal-example-plugins`](https://github.com/NousResearch/tribal-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
tribal-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/tribal-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Tribal provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Tribal provider
        # (must be one Tribal already knows about — same names as
        # `tribal model` and config.yaml model.provider).
        ...
        # plugin sends — Tribal does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.tribal/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Tribal' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Tribal subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Tribal/tribal/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Tribal/tribal/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/tribal-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/tribal-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
