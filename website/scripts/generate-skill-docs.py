...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Tribal/tribal/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    tribal_meta = (fm.get("metadata") or {}).get("tribal") or {}
    tags = tribal_meta.get("tags") or []
    related = tribal_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `tribal skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Tribal Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Tribal ships with a large built-in skill library copied into `~/.tribal/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Tribal also syncs bundled skills on `tribal update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.tribal/skills/` tree, it is still shipped with Tribal; restore it with `tribal skills reset <name> --restore`.",
        "",
