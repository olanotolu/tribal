"""Centralized logging setup for Tribal Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.tribal/logs/`` (profile-aware via ``get_tribal_home()``).
...
from tribal_constants import get_config_path, get_tribal_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_tribal_session_injector", False):
    ...
    _session_record_factory._tribal_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``tribal logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "tribal_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Tribal logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    tribal_home
        Override for the Tribal home directory.  Falls back to
        ``get_tribal_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_tribal_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._tribal_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the tribal group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from tribal_cli.config import is_managed
