"""langfuse — Tribal plugin for Langfuse observability.

Traces Tribal conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Tribal plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``tribal plugins enable
observability/langfuse``, or by checking the box in the interactive
``tribal plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.tribal/.env):
  TRIBAL_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  TRIBAL_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  TRIBAL_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  TRIBAL_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  TRIBAL_LANGFUSE_RELEASE     - release/version tag
  TRIBAL_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  TRIBAL_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  TRIBAL_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("TRIBAL_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Tribal plugin system —
    ...
    public_key = _env("TRIBAL_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("TRIBAL_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("TRIBAL_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("TRIBAL_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset TRIBAL_LANGFUSE_PUBLIC_KEY / TRIBAL_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("TRIBAL_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("TRIBAL_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("TRIBAL_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("TRIBAL_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid TRIBAL_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("TRIBAL_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Tribal turn",
                tags=["tribal", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Tribal branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Tribal also has a turn-scoped
    ...
    # messages is a list only for legacy Tribal branches that fired
    # pre_llm_call with API messages directly. Current Tribal fires
...
def register(ctx) -> None:
    ...
    # Tribal versions.  pre_api_request / post_api_request fire per API
