"""disk-cleanup plugin — auto-cleanup of ephemeral Tribal session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``TRIBAL_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$TRIBAL_HOME/disk-cleanup/cleanup.log``.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/tribal-x/test_foo.py`
        try:
...
All operations are scoped to TRIBAL_HOME and /tmp/tribal-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside TRIBAL_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Tribal session files.",
    )
