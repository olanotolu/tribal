...
Auto-tracks and cleans up ephemeral files created during Tribal Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$TRIBAL_HOME` and `/tmp/tribal-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `TRIBAL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `TRIBAL_HOME` or `/tmp/tribal-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$TRIBAL_HOME/disk-cleanup/` is itself excluded
- `$TRIBAL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
