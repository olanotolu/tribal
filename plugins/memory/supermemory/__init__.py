...
def _load_supermemory_config(tribal_home: str) -> dict:
    config = _default_config()
    config_path = Path(tribal_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, tribal_home: str) -> None:
    config_path = Path(tribal_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `tribal memory setup`.
        # All other options are documented for $TRIBAL_HOME/supermemory.json
    ...
    def save_config(self, values, tribal_home):
        ...
        _save_supermemory_config(sanitized, tribal_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from tribal_constants import get_tribal_home
        self._tribal_home = kwargs.get("tribal_home") or str(get_tribal_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._tribal_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "tribal", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "tribal_memory", "target": target, "type": "explicit_memory"},
