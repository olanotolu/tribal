...
Resolution order for config file:
  1. $TRIBAL_HOME/honcho.json  (instance-local, enables isolated Tribal instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Tribal profile.

    Resolution order:
      1. TRIBAL_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``tribal.<profile>``
      3. Fallback: ``"tribal"`` (default profile)
    """
    explicit = os.environ.get("TRIBAL_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $TRIBAL_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.tribal/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_tribal_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_tribal_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Tribal system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.tribal
    ...
    def from_global_config(
        ...
        Resolution: $TRIBAL_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Tribal profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.tribal block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Tribal session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Tribal session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Tribal session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'tribal honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run tribal honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `tribal honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            tribal_cfg = load_config()
            honcho_cfg = tribal_cfg.get("honcho", {})
