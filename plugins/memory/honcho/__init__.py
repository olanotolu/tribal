...
Config: Uses the existing Honcho config chain:
  1. $TRIBAL_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, tribal_home):
        """Write config to $TRIBAL_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(tribal_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `tribal honcho setup`
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every Tribal run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_tribal_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "tribal-default",
