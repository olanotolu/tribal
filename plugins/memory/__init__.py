...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with tribal-agent)
2. User-installed providers: ``$TRIBAL_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$TRIBAL_HOME/plugins/`` or None if unavailable."""
    try:
        from tribal_constants import get_tribal_home
        d = get_tribal_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$TRIBAL_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_tribal_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_tribal_user_memory.{active_provider}.cli"
    try:
