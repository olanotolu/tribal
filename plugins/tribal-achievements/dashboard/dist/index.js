(function () {
  "use strict";
  // tribal-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/tribal-achievements (MIT).
  // Bundled into tribal-agent. Upstream repo remains the staging ground for new
  ...
  if (!SDK || !window.__TRIBAL_PLUGINS__) return;
  ...
  async function api(path, options) {
    const url = "/api/plugins/tribal-achievements" + path;
    const token = window.__TRIBAL_SESSION_TOKEN__ || "";
    const headers = { ...((options && options.headers) || {}) };
    if (token) headers["X-Tribal-Session-Token"] = token;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("TRIBAL AGENT  ·  tribal.dev", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "tribal-achievement-" + (achievement.id || "badge") + ".png";
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in Tribal Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@NousResearch · https://tribal.dev";
    }
    ...
    return React.createElement("div", { className: "ha-page ha-page-loading" },
      React.createElement("section", { className: "ha-hero ha-loading-hero" },
        React.createElement("div", null,
          ...
          React.createElement("h1", null, tx(t, "hero.title", "Tribal Achievements")),
          React.createElement("p", null, tx(t, "hero.scan_subtitle", "Scanning Tribal session history. First scan can take 5–10 seconds on large histories."))
        ),
      ...
      React.createElement("section", { className: "ha-guide ha-loading-guide" },
        React.createElement("div", null,
          ...
          React.createElement("p", null, tx(t, "guide.scan_status_body", "Tribal is scanning local history once, then cards will appear automatically. Nothing is stuck if this takes a few seconds."))
        ),
        React.createElement("div", null,
    ...
    return React.createElement("div", { className: "ha-page" },
      React.createElement("section", { className: "ha-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, tx(t, "hero.kicker", "Agentic Gamerscore")),
          React.createElement("h1", null, tx(t, "hero.title", "Tribal Achievements")),
          React.createElement("p", null, tx(t, "hero.subtitle", "Collectible Tribal badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears."))
        ),
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, { label: tx(t, "stats.latest", "Latest"), value: latest[0] ? latest[0].name : tx(t, "stats.none_yet", "None yet"), hint: latest[0] ? latest[0].category : tx(t, "stats.latest_hint_empty", "run Tribal more") })
      ),
      React.createElement("section", { className: "ha-guide" },
        ...
        React.createElement("div", null,
          ...
          React.createElement("p", null, tx(t, "guide.secret_body", "Secrets hide their exact trigger. Once Tribal sees a related signal, the card becomes Discovered and shows its requirement."))
        )
      ),
  ...
  window.__TRIBAL_PLUGINS__.register("tribal-achievements", AchievementsPage);
})();
