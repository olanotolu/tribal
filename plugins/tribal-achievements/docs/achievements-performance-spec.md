# Tribal Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: tribal-achievements plugin
...
### `GET /api/plugins/tribal-achievements/achievements`
...
### `POST /api/plugins/tribal-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/tribal-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/tribal-achievements/overview`
...
- Persist plugin data under:
  - `~/.tribal/plugins/tribal-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.tribal/plugins/tribal-achievements/scan_checkpoint.json`
...
- `~/.tribal/plugins/tribal-achievements/`
