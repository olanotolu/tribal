...
Let the tribal agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where tribal runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `tribal meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `tribal meet` CLI |
...
| `cli.py` | `tribal meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TRIBAL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `tribal meet node {run,list,approve,remove,status,ping}` |
...
tribal meet install                                      # pip + Chromium
tribal meet setup                                        # preflight
tribal meet auth                                         # optional
tribal meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
tribal meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.tribal/.env
tribal meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
tribal meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
tribal meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.tribal/.env
tribal meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, tribal will **not** switch your system audio input automatically — the
...
tribal plugins enable google_meet
tribal meet node run --display-name my-mac --host 0.0.0.0 --port 18789
