"""`tribal meet node ...` subcommand tree.

Wired into the existing ``tribal meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``tribal meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="tribal-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             tribal meet node approve <name> ws://<host>:{args.port} {token}")
        try:
