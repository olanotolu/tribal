...
``$TRIBAL_HOME/workspace/meetings/<meeting-id>/``. The main tribal process
...
Run standalone for debugging::
    ...
    TRIBAL_MEET_URL=https://meet.google.com/abc-defg-hij \\
    TRIBAL_MEET_OUT_DIR=/tmp/meet-debug \\
    TRIBAL_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``TRIBAL_MEET_OUT_DIR``.
...
# and exposes ``window.__tribalMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__tribalMeetInstalled) return;
  window.__tribalMeetInstalled = true;
  window.__tribalMeetQueue = [];
  ...
  window.__tribalMeetDrain = () => {
    const out = window.__tribalMeetQueue.slice();
    window.__tribalMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "tribal_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("TRIBAL_MEET_URL", "").strip()
    out_dir_env = os.environ.get("TRIBAL_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("TRIBAL_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("TRIBAL_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("TRIBAL_MEET_GUEST_NAME", "Tribal Agent")
    duration_s = _parse_duration(os.environ.get("TRIBAL_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when TRIBAL_MEET_MODE=realtime.
    mode = os.environ.get("TRIBAL_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("TRIBAL_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("TRIBAL_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("TRIBAL_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("TRIBAL_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — TRIBAL_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: TRIBAL_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in TRIBAL_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("TRIBAL_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__tribalMeetDrain && window.__tribalMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__tribalMeetInstalled) {
