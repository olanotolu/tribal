...
def _auth_state_path() -> Path:
    return Path(get_tribal_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``tribal meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="Tribal Agent")
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"tribal meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: tribal meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: tribal meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: tribal meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "tribal meet join https://meet.google.com/abc-defg-hij"
        )
    else:
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  tribal will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: tribal meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: tribal meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="tribal meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
