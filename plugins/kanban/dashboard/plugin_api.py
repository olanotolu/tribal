...
``tribal_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__TRIBAL_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``tribal_cli/web_server.py``.

This means ``tribal dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from tribal_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from tribal_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``tribal_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from tribal_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from tribal_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``TRIBAL_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from tribal_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``tribal kanban
...
# spawn failures, stuck-blocked). See tribal_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``tribal kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``tribal kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``tribal kanban specify <task_id>``.
    ...
    try:
        os.environ["TRIBAL_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from tribal_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``tribal kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.tribal/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Tribal profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.tribal/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from tribal_constants import get_tribal_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_tribal_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``tribal profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from tribal_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``tribal kanban decompose <task_id>``.
    ...
    try:
        os.environ["TRIBAL_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from tribal_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.tribal/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in tribal_cli/web_server.py.
