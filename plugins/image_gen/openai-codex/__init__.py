...
Output is saved as PNG under ``$TRIBAL_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `tribal auth codex` (or `tribal setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`tribal auth codex` (or `tribal setup` → Codex) to sign in."
                ),
