...
three virtual model IDs so the ``tribal tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `tribal tools` → Image "
                    "Generation → OpenAI to configure, or `tribal setup` "
                    "to add the key."
                ),
