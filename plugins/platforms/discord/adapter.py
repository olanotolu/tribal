...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.tribal/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("TRIBAL_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("TRIBAL_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_tribal_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Tribal manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Tribal session")
        ...
        @tree.command(name="status", description="Show Tribal session status")
        ...
        @tree.command(name="stop", description="Stop the running Tribal agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.tribal/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Tribal Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Tribal gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Tribal session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Tribal in the thread",
        ...
        # tribal_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from tribal_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from tribal_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Tribal skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from tribal_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Tribal session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Tribal: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Tribal"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Tribal: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Tribal handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``tribal update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``tribal update`` prompts.
...
# is not in this process (e.g. ``tribal cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``tribal_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import tribal_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'tribal-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # tribal_cli/setup.py::_setup_discord function.  Same shape as Teams.
