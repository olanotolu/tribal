...
# ``model_tools`` import time, so ``tribal status``, ``tribal chat``, etc.
...
# ``tribal_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Tribal is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a tribal MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TRIBAL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _tribal_home = _get_tribal_home()
        except (ModuleNotFoundError, ImportError):
            _tribal_home = _Path.home() / ".tribal"
        self._thread_count_store = _ThreadCountStore(
            _tribal_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Tribal is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TRIBAL_HOME", str(_Path.home() / ".tribal"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TRIBAL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a tribal MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Tribal is thinking…' marker message.
        ...
        "Tribal is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Tribal is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Tribal is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Tribal is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Tribal is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``tribal setup``.

    The setup wizard at ``tribal_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.tribal/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. tribal-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.tribal/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'tribal-agent[google_chat]'",
        ...
        # the "Tribal is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Tribal is thinking…' marker message "
