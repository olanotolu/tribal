#!/usr/bin/env python3
"""
SQLite State Store for Tribal Agent.
...
DEFAULT_DB_PATH = get_tribal_home() / "state.db"
...
# tribal_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``tribal_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see tribal_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple tribal processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``tribal_state._set_last_init_error(None)`` explicitly.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Tribal, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Tribal session.

        A Tribal session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Tribal session is already bound to any Telegram DM topic.
