...
Environment:
    TRIBAL_TEST_WORKERS  Override worker count (default: os.cpu_count())
    TRIBAL_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        nousresearch/tribal-agent:test image, via
#                        ``TRIBAL_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or TRIBAL_TEST_FILE_TIMEOUT.
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/tribal-agent/tribal-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("TRIBAL_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $TRIBAL_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("TRIBAL_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("TRIBAL_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            "Default: 600 (10 min), env: TRIBAL_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: TRIBAL_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    # Parse --slice (or TRIBAL_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("TRIBAL_TEST_SLICE")
