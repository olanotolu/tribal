...
VERSION_FILE = REPO_ROOT / "tribal_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "tribal@marian.local": "Schrotti77",
    ...
    "274096618+tribal-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "tribal@nousresearch.com": "NousResearch",
    ...
    "tribal@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (TRIBAL_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (TRIBAL_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: tribal-eval community link)
    ...
    "tribalagent26@gmail.com": "tribalagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+tribalagent26@users.noreply.github.com": "tribalagent26",
    ...
    "vigo@tribal": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.tribal@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Tribal runtime files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"tribal-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "tribal@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Tribal/tribal",
    ...
    lines.append(f"# Tribal Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Tribal Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Tribal Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Tribal Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Tribal Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Tribal Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Tribal Agent v{new_version} ({calver_date})' "
