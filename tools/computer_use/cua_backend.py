...
can break on OS updates. Pin the installed version via `TRIBAL_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("TRIBAL_CUA_DRIVER_VERSION", "0.5.0")

_CUA_DRIVER_CMD = os.environ.get("TRIBAL_CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or TRIBAL_CUA_DRIVER_CMD resolves."""
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "Or run `tribal tools` and enable the Computer Use toolset to install it automatically."
    )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps tribal `capture(mode, app)` → cua-driver `list_windows` +
