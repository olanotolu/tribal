...
# regardless of approval level (e.g. logout kills the session Tribal runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("TRIBAL_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown TRIBAL_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `tribal tools` and enable Computer Use to install cua-driver.",
        })
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$TRIBAL_HOME/cache/vision/``
    ...
    try:
        ...
        cache_dir = get_tribal_dir("cache/vision", "temp_vision_images")
