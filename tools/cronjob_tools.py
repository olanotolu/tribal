"""
Cron job management tools for Tribal Agent.
...
#      saying "cat ~/.tribal/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `tribal-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.tribal/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("TRIBAL_SESSION_PLATFORM")
    origin_chat_id = get_session_env("TRIBAL_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("TRIBAL_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("TRIBAL_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via tribal model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within TRIBAL_HOME/scripts/.
    ...
    # Only relative paths within ~/.tribal/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.tribal/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.tribal/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_tribal_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_tribal_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
            ...
            "profile": {
                "type": "string",
                "description": "Optional Tribal profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local Tribal home override, loads that profile's config/.env for the run, and bridges TRIBAL_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root Tribal profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("TRIBAL_INTERACTIVE")
        or env_var_enabled("TRIBAL_GATEWAY_SESSION")
        or env_var_enabled("TRIBAL_EXEC_ASK")
    )
