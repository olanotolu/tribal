...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.tribal/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Tribal-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Tribal operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Tribal should recover an existing Camofox tab ID."""
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Tribal. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Tribal profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"tribal_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_tribal_home() / "browser_screenshots"
