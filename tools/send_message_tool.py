    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.tribal/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.tribal/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: tribal config set {platform_name.upper()}_HOME_CHANNEL <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("TRIBAL_SESSION_PLATFORM", "cli")
                user_id = get_session_env("TRIBAL_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("TRIBAL_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("TRIBAL_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("TRIBAL_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"tribal_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'tribal-agent[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``TRIBAL_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``TRIBAL_HOME`` which has no ``gateway.pid``, so the gateway-running check
