...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "tribal-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.tribal"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.tribal directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.tribal"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="tribal-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        ...
        # paths (e.g. home/user/.tribal/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.tribal".lstrip("/")
        ssh_cmd = self._build_ssh_command()
