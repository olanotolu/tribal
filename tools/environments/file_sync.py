...
_FORCE_SYNC_ENV = "TRIBAL_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.tribal") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.tribal to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``TRIBAL_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, tribal_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.tribal/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="tribal-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.tribal/skills/a.md`` →
        ``~/.tribal/skills/a.md``, a new remote file at
        ``/root/.tribal/skills/b.md`` maps to ``~/.tribal/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
