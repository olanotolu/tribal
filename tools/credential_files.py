...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.tribal",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``TRIBAL_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside TRIBAL_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    tribal_home = _resolve_tribal_home()

    # Reject absolute paths — they bypass the TRIBAL_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to TRIBAL_HOME)",
            relative_path,
        )
        return False

    host_path = tribal_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape TRIBAL_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, tribal_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.tribal/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="tribal-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching tribal_constants.get_tribal_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_tribal_dir()`` for backward compatibility with old directory layouts.
    """
    from tribal_constants import get_tribal_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_tribal_dir(new_subpath, old_name)
        if host_dir.is_dir():
