...
def _global_allow_private_urls() -> bool:
    ...
    1. ``TRIBAL_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("TRIBAL_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``TRIBAL_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
