...
# Must match tribal_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (tribal-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("tribal-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``tribal-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. tribal-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.tribal/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'tribal auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (tribal_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Tribal subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
