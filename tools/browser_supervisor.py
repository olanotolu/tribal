...
One ``CDPSupervisor`` runs per Tribal ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "tribal-dialog-bridge.invalid"
...
(() => {
  if (window.__tribalDialogBridgeInstalled) return;
  window.__tribalDialogBridgeInstalled = true;
  const ENDPOINT = "http://tribal-dialog-bridge.invalid/";
