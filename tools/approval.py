...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("TRIBAL_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("TRIBAL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("TRIBAL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set TRIBAL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind TRIBAL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds TRIBAL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("TRIBAL_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $TRIBAL_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_TRIBAL_ENV_PATH = (
    r'(?:~\/\.tribal/|'
    r'(?:\$home|\$\{home\})/\.tribal/|'
    r'(?:\$tribal_home|\$\{tribal_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\btribal\s+gateway\s+(stop|restart)\b', "stop/restart tribal gateway (kills running agents)"),
    (r'\btribal\s+update\b', "tribal update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart tribal-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart tribal-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(tribal|gateway|cli\.py)\b', "kill tribal/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill tribal` but not
    # `kill -9 $(pgrep -f tribal)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("TRIBAL_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set TRIBAL_INTERACTIVE or TRIBAL_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("TRIBAL_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("TRIBAL_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("TRIBAL_EXEC_ASK")
