...
-----------------------------------------------------------------------------
    ...
    ~/.tribal/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/tribal/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_tribal_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/tribal"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Tribal convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("TRIBAL_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Tribal infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``tribal checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `tribal checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "tribal@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Tribal Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``TRIBAL_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at TRIBAL_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "TRIBAL_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "TRIBAL_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `tribal checkpoints` CLI
# ---------------------------------------------------------------------------
