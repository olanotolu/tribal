...
This module provides the glue:
    - ``TribalTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Tribal Agent"
"""
...
from tribal_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__tribal_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses TRIBAL_HOME so each profile gets its own OAuth tokens.
    Layout: ``TRIBAL_HOME/mcp-tokens/``
    """
    try:
        from tribal_constants import get_tribal_home
        base = Path(get_tribal_home())
    except ImportError:
        base = Path(os.environ.get("TRIBAL_HOME", str(Path.home() / ".tribal")))
...
# ---------------------------------------------------------------------------
# TribalTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class TribalTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        TRIBAL_HOME/mcp-tokens/<server_name>.json         -- tokens
        TRIBAL_HOME/mcp-tokens/<server_name>.client.json   -- client info
        TRIBAL_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Tribal records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Tribal.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://tribal.dev/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `tribal mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Tribal Agent")
    scope = cfg.get("scope")
