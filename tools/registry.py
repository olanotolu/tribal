"""Central registry for all tribal-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``tribal tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``tribal tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``tribal tools enable foo``)
