...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.tribal/config.yaml``. Tribal
...
Configuration is loaded from ~/.tribal/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``tribal_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from tribal_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    ...
    return str(get_tribal_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.tribal/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.tribal/config.yaml.
    ...
    try:
        ...
        logger.debug("tribal_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Tribal without any Python code
...
#           output_format: wav
#
# Tribal writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__TRIBAL_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `tribal model` or set XAI_API_KEY.")
    ...
    # output_format when Tribal actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Tribal caches Piper voice models.

    Resolves to ``~/.tribal/cache/piper-voices/`` under the active
    TRIBAL_HOME so voice downloads follow profile boundaries.
    """
    from tribal_constants import get_tribal_dir
    root = Path(get_tribal_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.tribal/config.yaml (tts: section).
    ...
    platform = get_session_env("TRIBAL_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run tribal setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'tribal setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'tribal tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_tribal_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
