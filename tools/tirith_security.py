...
it is automatically downloaded from GitHub releases to $TRIBAL_HOME/bin/tirith.
...
def _get_tribal_home() -> str:
    """Return the Tribal home directory, respecting TRIBAL_HOME env var."""
    return str(get_tribal_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_tribal_home(), ".tirith-install-failed")
...
def _tribal_bin_dir() -> str:
    """Return $TRIBAL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_tribal_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TRIBAL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_tribal_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TRIBAL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TRIBAL_HOME/bin/tirith
    ...
    tribal_bin = os.path.join(_tribal_bin_dir(), "tirith")
    if os.path.isfile(tribal_bin) and os.access(tribal_bin, os.X_OK):
        _resolved_path = tribal_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return tribal_bin
