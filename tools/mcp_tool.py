...
transport, discovers their tools, and registers them into the tribal-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.tribal/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.tribal/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            tribal_home = os.path.expanduser(
                os.getenv(
                    "TRIBAL_HOME", os.path.join(os.path.expanduser("~"), ".tribal")
                )
            )
            candidates = [
                os.path.join(tribal_home, "node", "bin", resolved_command),
                ...
                # image (which the Tribal Docker image copies node + npm +
                # corepack from), and macOS Homebrew on Intel. Without this
                # candidate, any MCP server configured with an env.PATH that
                # omits /usr/local/bin fails with ENOENT at execvp, and a
                # naive symlink workaround into the user's PATH only fails one
                # layer deeper because npx's shebang re-execs /usr/bin/env
                # node which needs the same directory.
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Tribal MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Tribal gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Tribal is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `tribal mcp login {server_name}` (or delete the tokens "
            f"file under ~/.tribal/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Tribal config file.
    ...
    ``os.environ`` (which includes ``~/.tribal/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Tribal' MEDIA: tag convention and out to messaging
            ...
            # Tribal' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Tribal's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Tribal registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``tribal tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Tribal registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple tribal
