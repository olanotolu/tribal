...
selectable via ``tribal tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `tribal tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`tribal setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `tribal tools` "
        "→ Image Generation (run `tribal plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping tribal with a FAL key configured
