...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/tribal-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/tribal-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/tribal-results"
HEREDOC_MARKER = "TRIBAL_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/tribal-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"TRIBAL_PERSIST_{uuid.uuid4().hex[:8]}"
