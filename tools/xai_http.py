...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``tribal tools`` repaint, tool-registration scans,
    ...
    2. ``~/.tribal/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_tribal_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.tribal/.env`` first, then ``os.environ``.

    Wraps :func:`tribal_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from tribal_cli.config import get_env_value as _tribal_get_env_value

        value = _tribal_get_env_value(name)
...
def tribal_xai_user_agent() -> str:
    """Return a stable Tribal-specific User-Agent for xAI HTTP calls."""
    try:
        from tribal_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Tribal-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Tribal-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``tribal_cli.config.get_env_value`` so keys
    stored in ``~/.tribal/.env`` (the standard Tribal location) are honored —
