...
the actual `setup.sh` that creates the project workspace, configures Tribal
...
2. **Create profiles** — `tribal profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.tribal/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `tribal kanban create` assigned to the director
...
```bash
tribal profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.tribal/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.tribal/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Tribal
...
Each profile gets a `SOUL.md` at `~/.tribal/profiles/<name>/SOUL.md` that
...
```bash
tribal kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.tribal/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.tribal/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.tribal/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.tribal/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY tribal ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY tribal OPENROUTER_API_KEY || exit 1
# ...
```
