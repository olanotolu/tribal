...
Maps each role archetype to the Tribal skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public tribal-agent
...
## Tribal skills relevant to video production

### Visual / rendering skills (`tribal-agent/skills/creative/`)
...
### Design / pre-production skills (`tribal-agent/skills/creative/`)
...
### Audio / media skills (`tribal-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`tribal-agent/skills/devops/`)
...
These are **not** Tribal skills but external CLIs / APIs that profiles invoke.
...
## Built-in Tribal tools for media review

These are native Tribal tools — not invoked via terminal but through their own
...
key is present in `~/.tribal/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Tribal profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.tribal/.env` → environment variable.
...
When a new Tribal-public video skill ships:
