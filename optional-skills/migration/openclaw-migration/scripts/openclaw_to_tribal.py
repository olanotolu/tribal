#!/usr/bin/env python3
"""OpenClaw -> Tribal migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Tribal Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Tribal.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Tribal memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Tribal memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Tribal-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Tribal-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Tribal command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.tribal/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.tribal/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Tribal .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Tribal .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Tribal .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Tribal .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Tribal .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Tribal config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Tribal config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Tribal.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Tribal MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Tribal config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'tribal cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Tribal config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Tribal session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Tribal custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Tribal config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Tribal config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Tribal config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Tribal config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Tribal in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Tribal`` (prose), but lowercase matches
# like ``openclaw`` → ``tribal`` (so filesystem paths like ``~/.openclaw``
# become ``~/.tribal`` — the real Tribal home — not the broken ``~/.Tribal``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Tribal'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Tribal'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Tribal'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Tribal`` but maps ``openclaw`` → ``tribal`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.tribal/config.yaml`` (the real Tribal home) instead of the broken
    ``~/.Tribal/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Tribal.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Tribal Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-tribal/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Tribal config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-tribal/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Tribal session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Tribal config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Tribal
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Tribal-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Tribal-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: tribal config set",
                        )
                    continue
        ...
        for oc_key, tribal_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and tribal_key not in secret_additions:
                secret_additions[tribal_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, tribal_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and tribal_key not in secret_additions:
                        secret_additions[tribal_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tribal_config = load_yaml_file(destination)
        current_model = tribal_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = tribal_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                tribal_config["model"] = {"default": model_str}
            dump_yaml_file(destination, tribal_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Tribal still uses "edge"
        ...
        tribal_config = load_yaml_file(destination)
        existing_tts = tribal_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Tribal destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Tribal-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Tribal-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tribal_cfg_path = self.target_root / "config.yaml"
        tribal_cfg = load_yaml_file(tribal_cfg_path)
        existing_mcp = tribal_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Tribal config")
                continue

            tribal_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                tribal_srv["command"] = srv["command"]
                if srv.get("args"):
                    tribal_srv["args"] = srv["args"]
                if srv.get("env"):
                    tribal_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    tribal_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                tribal_srv["url"] = srv["url"]
                if srv.get("headers"):
                    tribal_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    tribal_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                tribal_srv["enabled"] = False
            if srv.get("timeout"):
                tribal_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                tribal_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                tribal_srv["tools"] = {}
                if tools_cfg.get("include"):
                    tribal_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    tribal_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(tribal_cfg_path)
            tribal_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(tribal_cfg_path, tribal_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'tribal cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = tribal_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Tribal reasoning_effort
        ...
        if compaction:
            compression = tribal_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'tribal gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("TRIBAL_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tribal_cfg_path = self.target_root / "config.yaml"
        tribal_cfg = load_yaml_file(tribal_cfg_path)
        sr = tribal_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tribal_cfg_path = self.target_root / "config.yaml"
        tribal_cfg = load_yaml_file(tribal_cfg_path)
        custom_providers = tribal_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(tribal_cfg_path)
            tribal_cfg["custom_providers"] = custom_providers
            dump_yaml_file(tribal_cfg_path, tribal_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Tribal config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            tribal_cfg_path = self.target_root / "config.yaml"
            tribal_cfg = load_yaml_file(tribal_cfg_path)
            discord_tribal = tribal_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_tribal["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_tribal["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                tribal_cfg["discord"] = discord_tribal
                dump_yaml_file(tribal_cfg_path, tribal_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        tribal_cfg_path = self.target_root / "config.yaml"
        tribal_cfg = load_yaml_file(tribal_cfg_path)
        browser_tribal = tribal_cfg.get("browser") or {}
        changed = False

        # Map fields that have Tribal equivalents
        if browser.get("cdpUrl"):
            browser_tribal["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_tribal["headless"] = browser["headless"]
            changed = True

        if changed:
            tribal_cfg["browser"] = browser_tribal
            if self.execute:
                self.maybe_backup(tribal_cfg_path)
                dump_yaml_file(tribal_cfg_path, tribal_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            tribal_mode = mode_map.get(mode, "manual")
            tribal_cfg.setdefault("approvals", {})["mode"] = tribal_mode
            if self.execute:
                self.maybe_backup(tribal_cfg_path)
                dump_yaml_file(tribal_cfg_path, tribal_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{tribal_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Tribal Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Tribal. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Tribal Config Not Overwritten)",
                "",
                "These items already existed in your Tribal config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Tribal agent discovers these",
            "directories, it may read/write to them instead of the Tribal state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `tribal claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `tribal claw cleanup` to fix it.",
            "",
            "## Tribal-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `tribal claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `tribal setup` to configure any remaining settings",
            "- Run `tribal mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `tribal cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `tribal cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `tribal gateway install` if you need the gateway service",
            "- Review `~/.tribal/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Tribal Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("TRIBAL_HOME") or str(Path.home() / ".tribal"), help="Tribal home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Tribal targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Tribal-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Tribal Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.tribal/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.tribal/config.yaml")
        print("    2. Run: tribal mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: tribal cron")
