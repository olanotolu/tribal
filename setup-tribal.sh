...
# 5. Symlinks the 'tribal' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Tribal Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/tribal-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink tribal into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up tribal command..."

TRIBAL_BIN="$SCRIPT_DIR/venv/bin/tribal"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TRIBAL_BIN" "$COMMAND_LINK_DIR/tribal"
echo -e "${GREEN}✓${NC} Symlinked tribal → $COMMAND_LINK_DISPLAY_DIR/tribal"
...
# ============================================================================
# Seed bundled skills into ~/.tribal/skills/
# ============================================================================

TRIBAL_SKILLS_DIR="${TRIBAL_HOME:-$HOME/.tribal}/skills"
mkdir -p "$TRIBAL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.tribal/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TRIBAL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  tribal status        # Check configuration"
if is_termux; then
    echo "  tribal gateway       # Run gateway in foreground"
else
    echo "  tribal gateway install # Install gateway service (messaging + cron)"
fi
echo "  tribal cron list     # View scheduled jobs"
echo "  tribal doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m tribal_cli.main setup
fi
