...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_tribal_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /tribal prefix). A single regex
            ...
            # manifest (see `tribal slack manifest`). In Socket Mode, Slack
            ...
            from tribal_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/tribal$")

            @self._app.command(_slash_pattern)
            async def handle_tribal_command(ack, command):
            ...
            for _action_id in (
                "tribal_approve_once",
                "tribal_approve_session",
                "tribal_approve_always",
                "tribal_deny",
            ):
            ...
            for _action_id in (
                "tribal_confirm_once",
                "tribal_confirm_always",
                "tribal_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Tribal handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Tribal session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from tribal_cli.commands import is_gateway_known_command
                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@tribal`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "tribal_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "tribal_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "tribal_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "tribal_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "tribal_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "tribal_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "tribal_confirm_once": "once",
            "tribal_confirm_always": "always",
            "tribal_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "tribal_approve_once": "once",
            "tribal_approve_session": "session",
            "tribal_approve_always": "always",
            "tribal_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/tribal <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/tribal
        ...
        if slash_name in {"tribal", ""}:
            # Legacy /tribal <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from tribal_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/tribal   ``).
        ...
        # "Running /cmd…" ack shown by handle_tribal_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/tribal <question>" must produce public replies so
