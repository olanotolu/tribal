...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("TRIBAL_WECOM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("TRIBAL_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=tribal&scode="
...
def qr_scan_for_bot_info(
    *,
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=tribal"

    # ── Step 1: Fetch QR code ──
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "TribalAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "TribalAgent/1.0"})
