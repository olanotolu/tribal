...
Connects Tribal Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(tribal_home: str) -> Path:
    path = Path(tribal_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(tribal_home: str, account_id: str) -> Path:
    return _account_dir(tribal_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(tribal_home, account_id)
...
def load_weixin_account(tribal_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, tribal_home: str):
        self._root = _account_dir(tribal_home)
...
def _sync_buf_path(tribal_home: str, account_id: str) -> Path:
    return _account_dir(tribal_home) / f"{account_id}.sync.json"


def _load_sync_buf(tribal_home: str, account_id: str) -> str:
    path = _sync_buf_path(tribal_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(tribal_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(tribal_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Tribal adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._tribal_home = tribal_home
        self._token_store = ContextTokenStore(tribal_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(tribal_home, self._account_id)
            if persisted:
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Tribal regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._tribal_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._tribal_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"tribal-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"tribal-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_tribal_home()))
