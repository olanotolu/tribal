"""
Email platform adapter for the Tribal gateway.

Allows users to interact with Tribal by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "tribal-agent" "version" "{_tribal_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Tribal Agent")
        ...
        msg_id = f"<tribal-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
