...
There are two ways to add a platform to the Tribal gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.tribal/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Tribal code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `tribal gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `tribal_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Tribal core.
...
And add it to the `tribal-gateway` composite:

```python
"tribal-gateway": {
    "includes": [..., "tribal-your-platform"]
}
```
...
## 12. Status Display (`tribal_cli/status.py`)
...
## 13. Gateway Setup Wizard (`tribal_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ tribal_cli/ toolsets.py \
  --include="*.py" -l | sort -u
