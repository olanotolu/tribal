"""Windows UTF-8 bootstrap for Tribal entry points.
...
should be imported at the very top of every Tribal entry point
(``tribal``, ``tribal-agent``, ``tribal-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import tribal_bootstrap``
# (or ``from tribal_bootstrap import apply_windows_utf8_bootstrap``) at
