...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Tribal Agent\",
    \"comments\": [
...
```bash
source "${TRIBAL_HOME:-$HOME/.tribal}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Tribal Agent. Code looks clean — good test coverage, no security concerns."
