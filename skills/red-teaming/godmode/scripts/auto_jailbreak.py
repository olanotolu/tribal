...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TRIBAL_HOME", os.path.expanduser("~/.tribal")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TRIBAL_HOME", Path.home() / ".tribal")) / "skills" / "red-teaming" / "godmode"
...
TRIBAL_HOME = Path(os.getenv("TRIBAL_HOME", Path.home() / ".tribal"))
CONFIG_PATH = TRIBAL_HOME / "config.yaml"
PREFILL_PATH = TRIBAL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Tribal models — already uncensored, just needs clean prompt
    "tribal": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "tribal" in model_lower or "nous" in model_lower:
        return "tribal"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Tribal config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.tribal/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Tribal for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Tribal for changes to take effect.")
