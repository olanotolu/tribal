"""ACP session manager — maps ACP sessions to Tribal AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.tribal/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Tribal itself is running in WSL.

    Windows ACP clients can launch ``tribal acp`` inside WSL while still sending
    ...
    """
    from tribal_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Tribal from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["tribal-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Tribal agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Tribal AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Tribal runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.tribal/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``TRIBAL_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_tribal_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=tribal_home / "state.db")
            return self._db_instance
        except Exception:
