"""ACP permission bridging for Tribal dangerous-command approvals."""
...
# Maps ACP permission option ids to Tribal approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Tribal approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Tribal semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_tribal(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Tribal approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_TRIBAL.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Tribal-compatible approval callback that bridges to ACP.
