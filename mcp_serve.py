"""
Tribal MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Tribal-specific extra)

Usage:
    tribal mcp serve
    tribal mcp serve --verbose
...
logger = logging.getLogger("tribal.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TRIBAL_HOME."""
    try:
        from tribal_constants import get_tribal_home
        return get_tribal_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TRIBAL_HOME", Path.home() / ".tribal")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from tribal_constants import get_tribal_home
        directory_file = get_tribal_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TRIBAL_HOME", Path.home() / ".tribal")
...
class EventBridge:
    ...
    This is the Tribal equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from tribal_constants import get_tribal_home
            db_file = get_tribal_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TRIBAL_HOME", Path.home() / ".tribal")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Tribal MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "tribal",
        instructions=(
            "Tribal Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Tribal MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
