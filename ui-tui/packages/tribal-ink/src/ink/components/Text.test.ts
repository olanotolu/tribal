...
describe('shouldUseAnsiDim', () => {
  ...
  it('honors explicit env override', () => {
    expect(shouldUseAnsiDim({ TRIBAL_TUI_DIM: '1', VTE_VERSION: '7603' } as NodeJS.ProcessEnv)).toBe(true)
    expect(shouldUseAnsiDim({ TRIBAL_TUI_DIM: '1', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)).toBe(true)
    expect(shouldUseAnsiDim({ TRIBAL_TUI_DIM: '0' } as NodeJS.ProcessEnv)).toBe(false)
  })
})
...
describe('dimColorFallback', () => {
  ...
  it('does not apply when dim is explicitly configured', () => {
    expect(dimColorFallback({ TRIBAL_TUI_DIM: '1', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)).toBeUndefined()
    expect(dimColorFallback({ TRIBAL_TUI_DIM: '0', TERM_PROGRAM: 'Apple_Terminal' } as NodeJS.ProcessEnv)).toBeUndefined()
  })
})
