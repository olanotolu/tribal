...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.TRIBAL_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.TRIBAL_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.TRIBAL_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.TRIBAL_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.TRIBAL_PYTHON?.trim() || process.env.PYTHON?.trim()
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.TRIBAL_CWD || root
  ...
  start() {
    const root = process.env.TRIBAL_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
