 ...
 * tribal-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``tribal_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but tribal-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS tribal-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on tribal-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is tribal-ink's Alt signal and accepting it would
