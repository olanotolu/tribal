...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.TRIBAL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Tribal on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via TRIBAL_TUI_TRUECOLOR=1.
