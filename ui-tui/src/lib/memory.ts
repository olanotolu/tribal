...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.TRIBAL_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.tribal', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `tribal-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
