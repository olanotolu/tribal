...
// Mirrors @tribal/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @tribal/ink is bundled separately from packages/.
...
// Terminal. Explicit TRIBAL_TUI_THEME / COLORFGBG signals above still win,
...
// `TRIBAL_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `TRIBAL_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `TRIBAL_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `TRIBAL_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Tribal palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.TRIBAL_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.TRIBAL_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.TRIBAL_TUI_BACKGROUND ?? '')
