import type { MouseTrackingMode } from '@tribal/ink'
...
export const STARTUP_RESUME_ID = (process.env.TRIBAL_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.TRIBAL_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.TRIBAL_TUI_IMAGE ?? '').trim()
...
//
// - TRIBAL_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - TRIBAL_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.TRIBAL_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.TRIBAL_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.TRIBAL_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.TRIBAL_TUI_INLINE)
...
// less fragile. Override explicitly with TRIBAL_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.TRIBAL_TUI_FPS)
