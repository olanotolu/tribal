import { forceRedraw, type MouseTrackingMode } from '@tribal/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Tribal Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `tribal update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try TRIBAL_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Tribal #${i + 1}`
