...
describe('isTermuxTuiMode', () => {
  ...
  it('allows explicit opt-out override', () => {
    expect(
      isTermuxTuiMode({ TERMUX_VERSION: '0.118.0', TRIBAL_TUI_TERMUX_MODE: '0' } as NodeJS.ProcessEnv)
    ).toBe(false)
  })

  it('stays false outside Termux even if override is set', () => {
    expect(isTermuxTuiMode({ TRIBAL_TUI_TERMUX_MODE: '1', PREFIX: '/usr' } as NodeJS.ProcessEnv)).toBe(false)
  })
})
