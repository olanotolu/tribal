...
describe('GatewayClient websocket attach mode', () => {
  ...
  beforeEach(() => {
    originalGatewayUrl = process.env.TRIBAL_TUI_GATEWAY_URL
    originalSidecarUrl = process.env.TRIBAL_TUI_SIDECAR_URL
    FakeWebSocket.reset()
  ...
  afterEach(() => {
    if (originalGatewayUrl === undefined) {
      delete process.env.TRIBAL_TUI_GATEWAY_URL
    } else {
      process.env.TRIBAL_TUI_GATEWAY_URL = originalGatewayUrl
    }

    if (originalSidecarUrl === undefined) {
      delete process.env.TRIBAL_TUI_SIDECAR_URL
    } else {
      process.env.TRIBAL_TUI_SIDECAR_URL = originalSidecarUrl
    }
  ...
  it('waits for websocket open and resolves RPC requests', async () => {
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
  ...
  it('mirrors event frames to sidecar websocket when configured', async () => {
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.TRIBAL_TUI_SIDECAR_URL = 'ws://gateway.test/api/pub?token=abc&channel=demo'
  ...
  it('reattaches when TRIBAL_TUI_GATEWAY_URL rotates between requests', async () => {
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway-old.test/api/ws?token=abc'
    const gw = new GatewayClient()
    ...
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway-new.test/api/ws?token=xyz'
  ...
  it('redacts query string secrets in attach failure logs and events', () => {
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=hunter2&channel=secret'
    delete (globalThis as { WebSocket?: unknown }).WebSocket
  ...
  it('redacts attach URL secrets when the WebSocket constructor throws', () => {
    ...
    process.env.TRIBAL_TUI_GATEWAY_URL = secretUrl
  ...
  it('redacts sidecar URL secrets when the WebSocket constructor throws', async () => {
    ...
    process.env.TRIBAL_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.TRIBAL_TUI_SIDECAR_URL = sidecarUrl
  ...
  it('redacts user-info credentials even on URLs the WHATWG parser rejects', () => {
    ...
    process.env.TRIBAL_TUI_GATEWAY_URL = fixture
    delete (globalThis as { WebSocket?: unknown }).WebSocket
