...
Uses a file-based lock (~/.tribal/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `tribal update` reloads
# the module) fail with ModuleNotFoundError for tribal_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from tribal_constants import get_tribal_home
from tribal_cli._subprocess_compat import windows_hide_flags
from tribal_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``tribal tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from tribal_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_tribal_home: Path | None = None


def _get_tribal_home() -> Path:
    """Resolve Tribal home dynamically while preserving test monkeypatch hooks."""
    return _tribal_home or get_tribal_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    tribal_home = _get_tribal_home()
    lock_dir = tribal_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Tribal profile.
    ...
    the scheduler's test/override hook and a context-local Tribal home override
    both point at the resolved profile directory so _get_tribal_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_tribal_home() callers agree on the same home.
    ...
    global _tribal_home
    prior_override = _tribal_home
    env_snapshot = os.environ.copy()

    from tribal_cli.profiles import normalize_profile_name, resolve_profile_env
    from tribal_constants import reset_tribal_home_override, set_tribal_home_override
    ...
    override_token = None
    try:
        override_token = set_tribal_home_override(profile_home)
        _tribal_home = profile_home
        logger.info(
            "Job '%s': using Tribal profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _tribal_home = prior_override
        if override_token is not None:
            reset_tribal_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("TRIBAL_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid TRIBAL_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within TRIBAL_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against TRIBAL_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_tribal_home() / "scripts"
    ...
    # escape — scripts MUST reside within TRIBAL_HOME/scripts/.
    try:
    ...
    run_env["TRIBAL_HOME"] = str(_get_tribal_home())
    try:
        from tribal_constants import get_subprocess_home
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _tribal_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_tribal_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_tribal_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["TRIBAL_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed TRIBAL_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads TRIBAL_SESSION_PLATFORM
    #     and TRIBAL_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via TRIBAL_CRON_AUTO_DELIVER_* below.
    ...
    #     TRIBAL_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read TRIBAL_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the TRIBAL_CRON_AUTO_DELIVER_* vars set
    # below, so clearing TRIBAL_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "TRIBAL_CRON_AUTO_DELIVER_PLATFORM",
        "TRIBAL_CRON_AUTO_DELIVER_CHAT_ID",
        "TRIBAL_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_tribal_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_tribal_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["TRIBAL_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["TRIBAL_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["TRIBAL_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("TRIBAL_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_tribal_home() / "config.yaml")
        ...
        prefill_file = os.getenv("TRIBAL_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_tribal_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject TRIBAL_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # TRIBAL_HOME. When a workdir is configured, also inject project
        ...
        # override via TRIBAL_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("TRIBAL_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid TRIBAL_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_tribal_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _tribal_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _tribal_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set TRIBAL_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("TRIBAL_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid TRIBAL_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Tribal home override, scheduler _tribal_home hook, and temporary
