...
Jobs are stored in ~/.tribal/cron/jobs.json
Output is saved to ~/.tribal/cron/output/{job_id}/{timestamp}.md
"""
...
TRIBAL_DIR = get_tribal_home().resolve()
CRON_DIR = TRIBAL_DIR / "cron"
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _tribal_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Tribal configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Tribal timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _tribal_now().tzinfo
    if dt.tzinfo is None:
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall tribal-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _tribal_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``tribal -p`` and must refer to an
    ...
    # so profile directories can move with the Tribal root.
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.tribal/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional Tribal profile name. When set, the job runs with
                that profile's TRIBAL_HOME so profile-specific config,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _tribal_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _tribal_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _tribal_now().strftime("%Y-%m-%d_%H-%M-%S")
