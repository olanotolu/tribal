...
Reported: "switched from openrouter provider to anthropic api key via tribal
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("tribal_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    ...
    assert agent._fallback_model == {"provider": "nous", "model": "tribal-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("tribal_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
