...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install tribal-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"tribal-agent[{extra}]" in spec
        ]
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (tribal_cli/dingtalk_auth.py)
