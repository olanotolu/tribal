...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("TRIBAL_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("TRIBAL_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("TRIBAL_LOCAL_STT_LANGUAGE", "en")
...
class TestLoadSttConfig:
    ...
    def test_real_load_returns_dict(self):
        ...
        with patch.dict("sys.modules", {"tribal_cli": None, "tribal_cli.config": None}):
            from tools.transcription_tools import _load_stt_config
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.tribal_xai_user_agent = MagicMock(return_value="tribal-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("TRIBAL_LOCAL_STT_LANGUAGE", "fr")
...
class TestShellSafety:
    ...
    def test_env_var_template_uses_shell_path(self, monkeypatch):
        """When TRIBAL_LOCAL_STT_COMMAND is set, use_shell should be True."""
        import os
