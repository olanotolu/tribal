...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.tribal/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("tribal_cli.config.load_config", return_value={"skills": {}}):
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.tribal/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `tribal curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "tribal curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
