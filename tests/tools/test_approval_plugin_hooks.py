...
def isolated_session(monkeypatch, tmp_path):
    ...
    TRIBAL_HOME so the real user's command_allowlist doesn't leak in."""
    import tools.approval as _am

    session_key = "test:session:approval_hooks"
    token = set_current_session_key(session_key)
    monkeypatch.setenv("TRIBAL_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: TRIBAL_INTERACTIVE is set, the
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("tribal_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("tribal_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: TRIBAL_GATEWAY_SESSION is set and a
