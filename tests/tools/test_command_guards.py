...
def _clean_state():
    ...
    saved = {}
    for k in ("TRIBAL_INTERACTIVE", "TRIBAL_GATEWAY_SESSION", "TRIBAL_EXEC_ASK", "TRIBAL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["TRIBAL_INTERACTIVE"] = "1"
        session_key = os.getenv("TRIBAL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No TRIBAL_INTERACTIVE or TRIBAL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
