...
1. ``TribalTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``TribalMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# TribalTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestSetTokensAbsoluteExpiry:
    def test_set_tokens_persists_absolute_expires_at(self, tmp_path, monkeypatch):
        ...
        storage = TribalTokenStorage("srv")
        before = time.time()
        asyncio.run(
            storage.set_tokens(
                OAuthToken(
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        from tools.mcp_oauth import TribalTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# TribalMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import TribalTokenStorage
    from tools.mcp_oauth_manager import _TRIBAL_PROVIDER_CLS, reset_manager_for_tests

    assert _TRIBAL_PROVIDER_CLS is not None
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Tribal Agent",
    )
    provider = _TRIBAL_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = TribalTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Tribal Agent",
    )
