...
and the running Tribal session picks up the new tokens on the next auth
...
async def test_external_refresh_picked_up_without_restart(tmp_path, monkeypatch):
    ...
    1. A running Tribal session has OAuth tokens loaded in memory.
    ...
    # automatically via the TribalMCPOAuthProvider.async_auth_flow
