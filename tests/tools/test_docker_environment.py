...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/tribal-snap-test123.sh"
    env._cwd_file = "/tmp/tribal-cwd-test123.txt"
    env._cwd_marker = "__TRIBAL_CWD_test123__"
...
def test_init_env_args_uses_tribal_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _TRIBAL_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_tribal_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_tribal_dotenv(monkeypatch):
...
def test_forward_env_overrides_docker_env_in_init_args(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_tribal_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Tribal image, or ``gosu``/``su``
