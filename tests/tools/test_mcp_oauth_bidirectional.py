"""Regression test for the ``TribalMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_tribal_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _TRIBAL_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_tribal_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
