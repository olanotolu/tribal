...
class TestTribalTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("perm-test-server")
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path))
        storage = TribalTokenStorage("../../../etc/passwd")
        path = storage._tokens_path()
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Tribal must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "TribalTokenStorage") as mock_storage_cls:
...
class TestPasteCallbackSkipToken:
    ...
    def test_skip_message_printed(self, monkeypatch, capsys):
        ...
        assert "tribal mcp login" in err
