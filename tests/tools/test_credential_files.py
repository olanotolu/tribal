...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        tribal_home.mkdir()
        (tribal_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(tribal_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.tribal/token.json"
    ...
    def test_string_entry(self, tmp_path):
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        (tribal_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        (tribal_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.tribal/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        tribal_home = tmp_path / ".tribal"
        (tribal_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"TRIBAL_HOME": str(tribal_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.tribal")

        assert mounts[0]["container_path"] == "/home/user/.tribal/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.tribal/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.tribal/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape TRIBAL_HOME."""
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path / ".tribal"))
        (tmp_path / ".tribal").mkdir()

        # Create a sensitive file one level above tribal_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside tribal_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside TRIBAL_HOME must still be registered."""
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))
        (tribal_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_tribal_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of TRIBAL_HOME must be allowed."""
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        subdir = tribal_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside TRIBAL_HOME pointing outside must be rejected."""
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))

        # Create a sensitive file outside tribal_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside tribal_home pointing outside
        symlink = tribal_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes TRIBAL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, tribal_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape TRIBAL_HOME."""
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))
        ...
        sensitive.write_text("{}")
        self._write_config(tribal_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(tribal_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside TRIBAL_HOME via config must still mount."""
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))

        (tribal_home / "oauth.json").write_text("{}")
        self._write_config(tribal_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        (tribal_home / "cache" / "documents").mkdir(parents=True)
        (tribal_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.tribal/cache/documents" in paths
        assert "/root/.tribal/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.tribal/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        tribal_home = tmp_path / ".tribal"
        tribal_home.mkdir()
        # Use legacy dir name — get_tribal_dir prefers old if it exists
        (tribal_home / "document_cache").mkdir()
        (tribal_home / "image_cache").mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(tribal_home / "document_cache") in host_paths
        assert str(tribal_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.tribal/cache/documents" in container_paths
        assert "/root/.tribal/cache/images" in container_paths

    def test_empty_tribal_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        tribal_home = tmp_path / ".tribal"
        doc_dir = tribal_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        tribal_home = tmp_path / ".tribal"
        ss_dir = tribal_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.tribal/cache/screenshots/session_abc/screen1.png"
