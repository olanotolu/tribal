...
- _seed_from_env reads API keys from ~/.tribal/.env when not in os.environ
...
@pytest.fixture
def isolated_tribal_home(tmp_path, monkeypatch):
    """Point TRIBAL_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.tribal/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.tribal/.env, not just os.environ.
    ...
    load_tribal_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_tribal_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_tribal_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_tribal_home):
        """OpenRouter path has its own branch — verify it also reads .env."""
        _write_env_file(isolated_tribal_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_empty_dotenv_no_entries(self, isolated_tribal_home):
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.tribal/.env."""

    def test_key_from_dotenv_only(self, isolated_tribal_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_tribal_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from tribal_cli.auth import _resolve_api_key_provider_secret
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_tribal_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_tribal_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_tribal_home, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_tribal_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_tribal_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
