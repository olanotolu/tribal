...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``tribal tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.tribal/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `tribal tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``tribal tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``tribal auth`` sign-in) rather than an env-var key.
    Patches at the ``tribal_cli.runtime_provider.resolve_runtime_provider``
