...
def test_x_search_posts_responses_request(monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == f"Tribal-Agent/{__version__}"
...
def _no_xai_env(monkeypatch):
    ...
    for var in ("XAI_API_KEY", "XAI_BASE_URL", "TRIBAL_XAI_BASE_URL"):
...
def test_x_search_returns_tool_error_when_no_credentials(monkeypatch):
    ...
    assert "tribal auth add xai-oauth" in result
...
def test_x_search_honors_config_model_and_timeout(monkeypatch, tmp_path):
    ...
    # Patch the in-module config loader so tests don't touch ~/.tribal/config.yaml.
    monkeypatch.setattr(
