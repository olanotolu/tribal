...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in tribal-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_tribal_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["tribal-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["tribal-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _TRIBAL_CORE_TOOLS
        assert "discord" not in _TRIBAL_CORE_TOOLS
        assert "discord_admin" not in _TRIBAL_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"tribal-discord", "tribal-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("tribal_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["tribal-discord"], quiet_mode=True)
