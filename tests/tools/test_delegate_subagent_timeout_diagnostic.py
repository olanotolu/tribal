...
``~/.tribal/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, tribal_home):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under TRIBAL_HOME/logs/
        assert p.parent == tribal_home / "logs"
    ...
    def test_truncates_very_long_goal(self, tribal_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        huge_goal = "x" * 5000
    ...
    def test_missing_worker_thread_is_handled(self, tribal_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        path = _dump_subagent_timeout_diagnostic(
            child=child,
    ...
    def test_exited_worker_thread_is_handled(self, tribal_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point TRIBAL_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".tribal"
        monkeypatch.setenv("TRIBAL_HOME", str(bogus))
        child = _StubChild()
...
class TestRunSingleChildTimeoutDump:
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, tribal_home, monkeypatch):
        ...
        assert dump_path.parent == tribal_home / "logs"
    ...
    def test_nonzero_api_calls_skips_dump_and_uses_old_message(self, tribal_home, monkeypatch):
