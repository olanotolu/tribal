...
Coverage:
  tribal_cli/clipboard.py  — platform-specific image extraction (macOS, WSL, Wayland, X11)
...
from tribal_cli.clipboard import (
...
class TestSaveClipboardImage:
    def test_dispatches_to_macos_on_darwin(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("tribal_cli.clipboard._macos_save", return_value=False) as m:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            ...
            with patch("tribal_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_dispatches_to_linux_on_linux(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            ...
            with patch("tribal_cli.clipboard._linux_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
    ...
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("tribal_cli.clipboard._linux_save", return_value=False):
                save_clipboard_image(dest)
...
class TestMacosPngpaste:
    def test_success_writes_file(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=fake_run):
            assert _macos_pngpaste(dest) is True
        assert dest.stat().st_size == len(FAKE_PNG)

    def test_not_installed(self, tmp_path):
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=FileNotFoundError):
            assert _macos_pngpaste(tmp_path / "out.png") is False

    def test_no_image_in_clipboard(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("tribal_cli.clipboard.subprocess.run") as mock_run:
    ...
    def test_timeout_returns_false(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("tribal_cli.clipboard.subprocess.run",
...
class TestMacosOsascript:
    ...
    def test_clipboard_info_fails(self, tmp_path):
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=Exception("fail")):
            assert _macos_osascript(tmp_path / "out.png") is False
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is tribal_constants.is_wsl; reset the function's own module
        # globals so this stays stable even if tribal_constants was imported
        # through a different module object earlier in a large xdist run.
        import tribal_constants
        tribal_constants._wsl_detected = None
    ...
    def test_regular_linux(self):
        ...
        # supposed to intercept tribal_constants.is_wsl's `open` call,
...
class TestWaylandSave:
    ...
    def test_bmp_extraction_with_pillow_convert(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("tribal_cli.clipboard._convert_to_png", side_effect=fake_convert):
                assert _wayland_save(dest) is True
    ...
    def test_jpeg_extraction_converts_to_real_png(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("tribal_cli.clipboard._convert_to_png", side_effect=fake_convert) as mock_convert:
                assert _wayland_save(dest) is True
    ...
    def test_non_png_conversion_failure_cleans_up(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch("tribal_cli.clipboard._convert_to_png", return_value=True):
...
class TestLinuxSave:
    ...
    def setup_method(self):
        import tribal_cli.clipboard as cb
        cb._wsl_detected = None

    def test_wsl_tried_first(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("tribal_cli.clipboard._is_wsl", return_value=True):
            with patch("tribal_cli.clipboard._wsl_save", return_value=True) as m:
    ...
    def test_wsl_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("tribal_cli.clipboard._is_wsl", return_value=True):
            with patch("tribal_cli.clipboard._wsl_save", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch("tribal_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)

    def test_wayland_tried_when_display_set(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("tribal_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("tribal_cli.clipboard._wayland_save", return_value=True) as m:
                    assert _linux_save(dest) is True
                    m.assert_called_once_with(dest)
    ...
    def test_wayland_fails_falls_through_to_xclip(self, tmp_path):
        ...
        with patch("tribal_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("tribal_cli.clipboard._wayland_save", return_value=False):
                    with patch("tribal_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)
...
class TestWindowsHasImage:
    ...
    def test_clipboard_has_image(self):
        with patch("tribal_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("tribal_cli.clipboard.subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(stdout="True\n", returncode=0)
    ...
    def test_no_powershell_available(self):
        with patch("tribal_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch("tribal_cli.clipboard._windows_has_image", return_value=True) as m:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("tribal_cli.clipboard.subprocess.run", side_effect=fake_run_fail):
    ...
    def test_imagemagick_timeout_preserves_original(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("tribal_cli.clipboard.subprocess.run", side_effect=subprocess.TimeoutExpired("convert", 5)):
                _convert_to_png(dest)
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "darwin"
            with patch("tribal_cli.clipboard._macos_has_image", return_value=True) as m:
                assert has_clipboard_image() is True
                m.assert_called_once()
    ...
    def test_linux_wsl_dispatch(self):
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("tribal_cli.clipboard._is_wsl", return_value=True):
                with patch("tribal_cli.clipboard._wsl_has_image", return_value=True) as m:
                    assert has_clipboard_image() is True
                    m.assert_called_once()
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("tribal_cli.clipboard._is_wsl", return_value=True):
                with patch("tribal_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch("tribal_cli.clipboard._wayland_has_image", return_value=True) as wl:
                            assert has_clipboard_image() is True
    ...
    def test_linux_wayland_dispatch(self):
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            ...
            with patch("tribal_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch("tribal_cli.clipboard._wayland_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
    ...
    def test_linux_x11_dispatch(self):
        with patch("tribal_cli.clipboard.sys") as mock_sys:
            ...
            with patch("tribal_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch("tribal_cli.clipboard._xclip_has_image", return_value=True) as m:
                        assert has_clipboard_image() is True
                        m.assert_called_once()
...
class TestPreprocessImagesWithVision:
    ...
    @pytest.fixture
    def cli(self):
        """Minimal TribalCLI with mocked internals."""
...
class TestTryAttachClipboardImage:
    ...
    def test_image_found_attaches(self, cli):
        with patch("tribal_cli.clipboard.save_clipboard_image", return_value=True):
    ...
    def test_no_image_doesnt_attach(self, cli):
        with patch("tribal_cli.clipboard.save_clipboard_image", return_value=False):
            result = cli._try_attach_clipboard_image()
    ...
    def test_mixed_success_and_failure(self, cli):
        ...
        with patch("tribal_cli.clipboard.save_clipboard_image", side_effect=results):
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
            cli._try_attach_clipboard_image()
    ...
    def test_image_path_follows_naming_convention(self, cli):
        ...
        assert path.parent == Path(os.environ["TRIBAL_HOME"]) / "images"
