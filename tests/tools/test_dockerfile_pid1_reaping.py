...
- Signal forwarding runs through the init so ``docker stop`` triggers
  tribal's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, tribal accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, tribal accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into tribal,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, tribal still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``tribal-ink`` workspace package is
    ...
    assert "ui-tui/packages/tribal-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``tribal-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@tribal/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@tribal/ink``)
    ...
    assert "ui-tui/packages/tribal-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled tribal-ink workspace package "
        "so ``await import('@tribal/ink')`` resolves at runtime."
    )
