...
``tribal_tools.py`` and ``script.py`` into a temp dir, and those files
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        "TRIBAL_HOME": "/home/alice/.tribal",
        "TRIBAL_INTERACTIVE": "1",
...
# ``tribal_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``tribal_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_tribal_tools_module
        stub = generate_tribal_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_tribal_tools_module
        import tempfile, ast
        stub = generate_tribal_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_tribal_tools_module(["terminal"], transport="uds")
...
# let the child import tribal_tools, a user script that printed non-ASCII
