"""Tests for tribal_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    tribal_logging._logging_initialized = False
    ...
    tribal_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    tribal_logging._logging_initialized = False
    tribal_logging.clear_session_context()


@pytest.fixture
def tribal_home(tmp_path, monkeypatch):
    """Provide an isolated TRIBAL_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_tribal_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, tribal_home):
        log_dir = tribal_logging.setup_logging(tribal_home=tribal_home)
        assert log_dir == tribal_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        tribal_logging.setup_logging(tribal_home=tribal_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        tribal_logging.setup_logging(tribal_home=tribal_home, force=True)
    ...
    def test_custom_log_level(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, tribal_home):
        tribal_logging.setup_logging(
            tribal_home=tribal_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
    ...
    def test_writes_to_agent_log(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)

        test_logger = logging.getLogger("test_tribal_logging.write_test")
        ...
        agent_log = tribal_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)

        test_logger = logging.getLogger("test_tribal_logging.warning_test")
        ...
        agent_log = tribal_home / "logs" / "agent.log"
        errors_log = tribal_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)

        test_logger = logging.getLogger("test_tribal_logging.info_test")
    ...
    def test_reads_config_yaml(self, tribal_home):
    ...
    def test_explicit_params_override_config(self, tribal_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (tribal_home / "config.yaml").write_text(yaml.dump(config))

        tribal_logging.setup_logging(tribal_home=tribal_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, tribal_home):
        """The custom record factory injects session_tag on all records."""
        tribal_logging.setup_logging(tribal_home=tribal_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_tribal_session_injector", False), (
            "Record factory should have _tribal_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, tribal_home):
        ...
        gw_log = tribal_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, tribal_home):
    ...
    def test_gateway_log_receives_gateway_records(self, tribal_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, tribal_home):
    ...
    def test_agent_log_still_receives_all(self, tribal_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, tribal_home):
        """When session context is set, log lines include [session_id]."""
        tribal_logging.setup_logging(tribal_home=tribal_home)
        tribal_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, tribal_home):
    ...
    def test_clear_session_context(self, tribal_home):
        """After clearing, session tag disappears."""
        tribal_logging.setup_logging(tribal_home=tribal_home)
        tribal_logging.set_session_context("xyz789")
        tribal_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, tribal_home):
        ...
        def thread_a():
            tribal_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            tribal_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        tribal_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        tribal_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = tribal_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = tribal_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in tribal_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # tribal_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "tribal_plugins") == tribal_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = tribal_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == tribal_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = tribal_logging.COMPONENT_PREFIXES["cli"]
        assert "tribal_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == tribal_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        tribal_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_tribal_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, tribal_home):
        tribal_logging.setup_logging(tribal_home=tribal_home)
        tribal_logging.setup_verbose_logging()
        tribal_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        tribal_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = tribal_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        tribal_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("tribal_cli.config.is_managed", return_value=True):
                tribal_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, tribal_home):
        level, max_size, backup = tribal_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, tribal_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, tribal_home):
        import yaml
        config = {"model": "test"}
        (tribal_home / "config.yaml").write_text(yaml.dump(config))
