...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._tribal_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("tribal_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def _make_run_job_patches(self, tmp_path):
        ...
        fake_db = MagicMock()
        return fake_db, [
            patch("cron.scheduler._tribal_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("dotenv.load_dotenv"),
            patch("tribal_state.SessionDB", return_value=fake_db),
            patch(
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``tribal tools`` platform config for ``cron``
        ...
        # The cron default is _TRIBAL_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``tribal tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``tribal tools`` configured to enable web+file
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("TRIBAL_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("TRIBAL_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("TRIBAL_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("TRIBAL_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("TRIBAL_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("TRIBAL_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("TRIBAL_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("TRIBAL_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("TRIBAL_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("TRIBAL_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("TRIBAL_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("TRIBAL_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("cron.scheduler._tribal_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("tribal_cli.runtime_provider.resolve_runtime_provider",
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_TRIBAL_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_TRIBAL_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_TRIBAL_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_TRIBAL_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_TRIBAL_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_TRIBAL_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_TRIBAL_TEST_CRON_UNSET_VAR}"
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._tribal_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_tribal_home", return_value=tmp_path), \
             patch("dotenv.load_dotenv"), \
             patch("tribal_state.SessionDB", return_value=fake_db), \
             patch(
                 "tribal_cli.runtime_provider.resolve_runtime_provider",
...
class TestParallelTick:
    ...
    def test_parallel_jobs_isolated_contextvars(self):
        ...
        def mock_run_job(job):
            ...
            chat_id = get_session_env("TRIBAL_SESSION_CHAT_ID")
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """TRIBAL_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("TRIBAL_CRON_MAX_PARALLEL", "1")
        call_times = []
