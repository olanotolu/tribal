...
def test_save_twilio_writes_env_and_state(tmp_path: Path, monkeypatch):
    ...
    env_text = (tmp_path / ".tribal" / ".env").read_text(encoding="utf-8")
    state = json.loads((tmp_path / ".tribal" / "telephony_state.json").read_text(encoding="utf-8"))
...
def test_diagnose_includes_decision_tree_and_saved_state(tmp_path: Path, monkeypatch):
    ...
    mod._save_state(
        ...
        tribal_home / "telephony_state.json",
    )
    (tribal_home / ".env").parent.mkdir(parents=True, exist_ok=True)
