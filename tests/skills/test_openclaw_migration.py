...
"""
    ...
    assert "Tyler Williams > Active Projects: Tribal Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `tribal cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # tribal_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.tribal/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "tribal_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Tribal``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Tribal prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Tribal to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Tribal config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Tribal uses tools well"
    # All-lowercase matches → lowercase ``tribal``; this preserves the
    # real filesystem path ``~/.tribal`` (Tribal home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "tribal should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Tribal remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "tribal prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Tribal was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "tribal uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Tribal said to ask Tribal about Tribal settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.tribal`` (the real Tribal home), not the broken
    ``.Tribal``.
    ...
    rewritten to ``~/.Tribal/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.tribal/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .tribal directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.tribal'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "tribal config path: ~/.tribal/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Tribal" in result
