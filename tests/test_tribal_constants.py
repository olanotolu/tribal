"""Tests for tribal_constants module."""
...
class TestGetDefaultTribalRoot:
    """Tests for get_default_tribal_root() — Docker/custom deployment awareness."""

    def test_no_tribal_home_returns_native(self, tmp_path, monkeypatch):
        """When TRIBAL_HOME is not set, returns ~/.tribal."""
        monkeypatch.delenv("TRIBAL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_tribal_root() == tmp_path / ".tribal"

    def test_tribal_home_is_native(self, tmp_path, monkeypatch):
        """When TRIBAL_HOME = ~/.tribal, returns ~/.tribal."""
        native = tmp_path / ".tribal"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIBAL_HOME", str(native))
        assert get_default_tribal_root() == native

    def test_tribal_home_is_profile(self, tmp_path, monkeypatch):
        """When TRIBAL_HOME is a profile under ~/.tribal, returns ~/.tribal."""
        native = tmp_path / ".tribal"
        profile = native / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIBAL_HOME", str(profile))
        assert get_default_tribal_root() == native

    def test_tribal_home_is_docker(self, tmp_path, monkeypatch):
        """When TRIBAL_HOME points outside ~/.tribal (Docker), returns TRIBAL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIBAL_HOME", str(docker_home))
        assert get_default_tribal_root() == docker_home

    def test_tribal_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any TRIBAL_HOME outside ~/.tribal is treated as the root."""
        custom = tmp_path / "my-tribal-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIBAL_HOME", str(custom))
        assert get_default_tribal_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (TRIBAL_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIBAL_HOME", str(profile))
        assert get_default_tribal_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(tribal_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(tribal_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".tribal"
