"""
Tests for timezone support (tribal_time module + integration points).
...
def _reset_tribal_time_cache():
    """Reset the tribal_time module cache (replacement for removed reset_cache)."""
    tribal_time._cached_tz = None
    tribal_time._cached_tz_name = None
    tribal_time._cache_resolved = False


# =========================================================================
# tribal_time.now() — core helper
# =========================================================================

class TestTribalTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_tribal_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["TRIBAL_TIMEZONE"] = "Asia/Kolkata"
        result = tribal_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["TRIBAL_TIMEZONE"] = "UTC"
        result = tribal_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["TRIBAL_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="tribal_time"):
            result = tribal_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["TRIBAL_TIMEZONE"] = "Europe/London"
        tz = tribal_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["TRIBAL_TIMEZONE"] = "Not/A/Timezone"
        tz = tribal_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When TRIBAL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) TRIBAL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("TRIBAL_TIMEZONE=" + os.environ.get("TRIBAL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "TRIBAL_TIMEZONE=NOT_SET" in result["output"], (
            "TRIBAL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When TRIBAL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("TRIBAL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=tribal_tz) which shifted
        absolute time when system-local tz != Tribal tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_tribal_tz(self):
        """Already-aware datetimes should be normalized to Tribal tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Tribal tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Tribal tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Tribal is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Tribal tz
        ...
        # Use a Tribal timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _tribal_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["TRIBAL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_tribal_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Tribal timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["TRIBAL_TIMEZONE"] = "US/Eastern"
        _reset_tribal_time_cache()
