...
def test_tribal_src_root_inserted_at_front():
    original = sys.path[:]
    try:
        fake_root = "/fake/tribal/src"
        with patch.dict(os.environ, {"TRIBAL_PYTHON_SRC_ROOT": fake_root}):
            _src_root = os.environ.get("TRIBAL_PYTHON_SRC_ROOT", "")
            if _src_root and _src_root not in sys.path:
                sys.path.insert(0, _src_root)
            sys.path = [p for p in sys.path if p not in {"", "."}]
...
def test_no_src_root_env_does_not_crash():
    original = sys.path[:]
    try:
        env = {k: v for k, v in os.environ.items() if k != "TRIBAL_PYTHON_SRC_ROOT"}
