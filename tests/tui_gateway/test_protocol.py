...
def server():
    with patch.dict("sys.modules", {
        "tribal_constants": MagicMock(get_tribal_home=MagicMock(return_value="/tmp/tribal_test")),
        "tribal_cli.env_loader": MagicMock(),
        "tribal_cli.banner": MagicMock(),
        "tribal_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `TRIBAL_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("TRIBAL_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("TRIBAL_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/tribal-agent-dev": {"name": "tribal-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "tribal-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "tribal_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "tribal-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "tribal-agent-dev"
