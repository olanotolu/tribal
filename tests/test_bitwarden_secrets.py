...
binary download are exercised manually by `tribal secrets bitwarden
...
def tribal_home(tmp_path, monkeypatch):
    """Point Tribal at an isolated home directory."""
    home = tmp_path / ".tribal"
    home.mkdir()
    monkeypatch.setenv("TRIBAL_HOME", str(home))
    # Some modules cache get_tribal_home; clear if needed.
    import tribal_constants
    if hasattr(tribal_constants, "_TRIBAL_HOME_CACHE"):
        tribal_constants._TRIBAL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(tribal_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(tribal_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(tribal_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from tribal_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under tribal_home/cache/."""
    home = tmp_path / ".tribal"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
