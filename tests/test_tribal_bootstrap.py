"""Tests for tribal_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Tribal entry point
(tribal, tribal-agent, tribal-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Tribal entry point imports tribal_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported tribal_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("tribal_bootstrap", None)
    import tribal_bootstrap  # noqa: WPS433
    return tribal_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Tribal CLI
...
class TestEntryPointsImportBootstrap:
    """Every Tribal entry point must import tribal_bootstrap as its
    ...
    # Entry points that invoke Tribal as a process.  Each one must
    # import tribal_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "tribal_cli/main.py",   # tribal CLI (console_script)
        "run_agent.py",          # tribal-agent (console_script)
        "acp_adapter/entry.py",  # tribal-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import tribal_bootstrap' and that
        ...
        half-finished ``tribal update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``tribal_bootstrap`` as a top-level module) leaves tribal
        ...
        """
        # Resolve relative to the tribal-agent repo root.  Tests live
        # at tests/test_tribal_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # tribal start even when tribal_bootstrap hasn't been
        ...
        assert first_import_name == "tribal_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'tribal_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import tribal_bootstrap' line to be the first import."
        )
