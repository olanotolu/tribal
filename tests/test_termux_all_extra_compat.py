...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"tribal-agent[termux]"' in text
    assert '"tribal-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"tribal-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
