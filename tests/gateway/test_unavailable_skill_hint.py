...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`tribal_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + TRIBAL_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `tribal skills config` writes).
    monkeypatch.setattr(
    ...
    assert "tribal skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "tribal_constants.get_optional_skills_dir",
