...
def adapter(tmp_path):
    ...
    ~/.tribal/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.tribal/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Tribal host. The relay flattens Chat
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Tribal is thinking…' stuck in chat. Race fix via
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/tribal-chat"
            ),
        ...
        monkeypatch.setattr("tribal_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("tribal_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("tribal_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "tribal_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "tribal_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "tribal_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "tribal_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
