...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert created["object"] == "tribal.session"
        assert created["session"]["title"] == "Mobile chat"

        session_db.append_message(session_id, "user", "hello from phone")
        session_db.append_message(session_id, "assistant", "hello from tribal")
        ...
        assert deleted == {"object": "tribal.session.deleted", "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == "tribal.session"
    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", "X-Tribal-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-Tribal-Session-Id"] == session_id
    assert resp.headers["X-Tribal-Session-Key"] == "client-42"
    assert payload["object"] == "tribal.session.chat.completion"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-Tribal-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-Tribal-Session-Key requires API key" in data["error"]["message"]
