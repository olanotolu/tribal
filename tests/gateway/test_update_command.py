...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("TRIBAL_MANAGED", "homebrew")

        result = await runner._handle_update_command(event)

        assert "managed by Homebrew" in result
        assert "brew upgrade tribal-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _tribal_home to tmp_path and project_root to a dir without .git
        ...
        with patch("gateway.run._tribal_home", tmp_path):
            ...
            # project_root will be the real tribal-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_tribal_binary(self, tmp_path):
        """Returns error when tribal is not on PATH and tribal_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "tribal update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m tribal_cli.main when tribal not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting Tribal update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m tribal_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "tribal_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_tribal_bin_prefers_which(self, tmp_path):
        """_resolve_tribal_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_tribal_bin

        with patch("shutil.which", return_value="/custom/path/tribal"):
            result = _resolve_tribal_bin()

        assert result == ["/custom/path/tribal"]

    @pytest.mark.asyncio
    async def test_resolve_tribal_bin_fallback(self):
        """_resolve_tribal_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_tribal_bin
        ...
        assert result == [sys.executable, "-m", "tribal_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_tribal_bin_returns_none_when_both_fail(self):
        """_resolve_tribal_bin returns None when both strategies fail."""
        from gateway.run import _resolve_tribal_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._tribal_home", tribal_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/tribal" if x == "tribal" else "/usr/bin/setsid"), \
        ...
        assert not (tribal_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((tribal_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (tribal_home / ".update_pending.json").exists()
        assert not (tribal_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (tribal_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = tribal_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (tribal_home / ".update_pending.json").write_text(json.dumps(pending))
        (tribal_home / ".update_output.txt").write_text("x" * 5000)
        (tribal_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (tribal_home / ".update_pending.json").write_text(json.dumps(pending))
        (tribal_home / ".update_output.txt").write_text("Traceback: boom")
