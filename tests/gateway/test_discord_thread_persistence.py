...
being persisted to ~/.tribal/discord_threads.json.
"""
...
class TestDiscordThreadPersistence:
    ...
    def _make_adapter(self, tmp_path):
        """Build a minimal DiscordAdapter with TRIBAL_HOME pointed at tmp_path."""
        from gateway.config import PlatformConfig
        from plugins.platforms.discord.adapter import DiscordAdapter

        config = PlatformConfig(enabled=True, token="test-token")
        with patch.dict(os.environ, {"TRIBAL_HOME": str(tmp_path)}):
    ...
    def test_missing_tribal_home_does_not_crash(self, tmp_path):
        """Load/save tolerate missing directories."""
        fake_home = tmp_path / "nonexistent" / "deep"
        with patch.dict(os.environ, {"TRIBAL_HOME": str(fake_home)}):
