...
class TestConfigEnvOverrides(unittest.TestCase):
    ...
    def test_email_config_loaded_from_env(self):
        ...
        self.assertEqual(config.platforms[Platform.EMAIL].extra["address"], "tribal@test.com")
...
class TestDispatchMessage(unittest.TestCase):
    ...
    def test_non_allowlisted_sender_dropped(self):
        ...
        import asyncio
        with patch.dict(os.environ, {
            "EMAIL_ALLOWED_USERS": "tribal@test.com,admin@test.com",
        }):
            adapter = self._make_adapter()
            adapter._message_handler = MagicMock()
...
class TestThreadContext(unittest.TestCase):
    ...
    def test_no_thread_context_uses_default_subject(self):
        """Without thread context, subject should be 'Re: Tribal Agent'."""
        adapter = self._make_adapter()
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Re: Tribal Agent")
            self.assertIn("Date", send_call)
...
class TestSendMethods(unittest.TestCase):
    ...
    def test_send_calls_smtp(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                adapter.send("user@test.com", "Hello from Tribal!")
            )

            self.assertTrue(result.success)
            mock_server.starttls.assert_called_once()
            mock_server.login.assert_called_once_with("tribal@test.com", "secret")
...
class TestSendEmailStandalone(unittest.TestCase):
    ...
    def test_send_email_tool_success(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                _send_email({"address": "tribal@test.com", "smtp_host": "smtp.test.com"}, "user@test.com", "Hello")
            )
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Tribal Agent")
            self.assertIn("Date", send_call)
            self.assertEqual(send_call["To"], "user@test.com")
            self.assertEqual(send_call["From"], "tribal@test.com")
...
class TestImapIdExtensionForNetEase(unittest.TestCase):
    ...
    def test_connect_sends_imap_id_after_login(self):
        ...
        self.assertIn("tribal-agent", payload)
