...
def _mention_entity(text, mention="@tribal_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@tribal_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@tribal_bot",
            entities=[_bot_command_entity("/new@tribal_bot", "/new@tribal_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@tribal_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @tribal_bot", entities=[_mention_entity("hi @tribal_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@tribal_bot",
            entities=[_bot_command_entity("/status@tribal_bot", "/status@tribal_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="tribal_bot")

    assert adapter._should_process_message(_group_message("email ops@tribal_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@tribal_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @tribal_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @tribal_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @tribal_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @tribal_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @tribal_bot")])
    ) is False
