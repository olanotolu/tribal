...
class TestCommandRegistry:
    ...
    def test_every_entry_has_valid_category(self):
        valid_categories = {"Session", "Configuration", "Tools & Skills", "Info", "Tribal", "Exit"}
        for cmd in COMMAND_REGISTRY:
...
class TestResolveCommand:
    def test_canonical_name_resolves(self):
        ...
        assert resolve_command("genesis").name == "genesis"
        assert resolve_command("tribe").name == "tribe"
...
class TestDerivedDicts:
    ...
    def test_genesis_command_is_in_tribal_category(self):
        assert "/genesis" in COMMANDS
        assert "/genesis" in COMMANDS_BY_CATEGORY["Tribal"]

    def test_tribe_command_is_in_tribal_category(self):
        assert "/tribe" in COMMANDS
        assert "/tribe" in COMMANDS_BY_CATEGORY["Tribal"]
        tribe = next(cmd for cmd in COMMAND_REGISTRY if cmd.name == "tribe")
        assert tribe.subcommands == ("ask", "status", "roles")
...
class TestSlackNativeSlashes:
    ...
    def test_tribal_catchall_is_first(self):
        """``/tribal`` must be reserved as the first slot so the legacy
        ``/tribal <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "tribal"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /tribal <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `tribal slack manifest`)."""
...
from tribal_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch tribal_cli.plugins.get_plugin_commands() to a fixed dict."""
        from tribal_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/tribal metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
