...
See: https://github.com/Tribal/tribal/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for tribal_constants.get_subprocess_home()."""

    def test_returns_none_when_tribal_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TRIBAL_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("TRIBAL_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from tribal_constants import (
            get_tribal_home,
            reset_tribal_home_override,
            set_tribal_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_tribal_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_tribal_home_override(profile)
        try:
            assert get_tribal_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_tribal_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_tribal_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(tribal_home / "home")
    ...
    def test_no_injection_when_tribal_home_unset(self, monkeypatch):
        monkeypatch.delenv("TRIBAL_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["TRIBAL_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from tribal_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from tribal_cli.profiles import create_profile
