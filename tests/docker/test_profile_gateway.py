...
Phase 4 wires `tribal -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"tribal profile create {PROFILE}")
    ...
    r = _sh(container_name, f"tribal -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"tribal -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"tribal profile create {PROFILE}")
    _sh(container_name, f"tribal -p {PROFILE} gateway start", timeout=60)
    time.sleep(3)

    r = _sh(
        container_name,
        f"tribal profile delete {PROFILE} --yes",
        timeout=30,
    )
