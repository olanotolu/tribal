...
def test_no_args_starts_tribal(built_image: str) -> None:
    """``docker run <image>`` should start tribal cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``tribal chat --help``.
...
def test_bare_executable_passthrough(built_image: str) -> None:
    ...
    tribal wrapper. Useful for long-lived sandbox mode and for testing.
    """
    r = subprocess.run(
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> tribal batch ...`` returns a
