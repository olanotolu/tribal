...
$TRIBAL_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $TRIBAL_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"tribal-restart-{safe}"
    volume = f"tribal-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = _exec(container, "tribal", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = _exec(container, "tribal", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    _exec(container, "tribal", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    _exec(container, "tribal", "profile", "create", "ghost").check_returncode()
