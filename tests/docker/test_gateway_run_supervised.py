...
Before the s6 migration, ``docker run nousresearch/tribal-agent gateway
...
crash and a supervised dashboard alongside (when ``TRIBAL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``TRIBAL_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``TRIBAL_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``tribal`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``tribal gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "TRIBAL_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "TRIBAL_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The TRIBAL_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``tribal gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``tribal gateway
    ...
    # Count python processes running `tribal gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*tribal.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `tribal gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-tribal`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``TRIBAL_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${TRIBAL_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Tribal Gateway Starting…"
    ...
    assert "⚕" in combined or "Tribal Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Tribal Gateway Starting" in file_contents, (
