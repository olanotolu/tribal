...
Override the image with ``TRIBAL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("TRIBAL_TEST_IMAGE", "tribal-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``TRIBAL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("TRIBAL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged tribal user
# ---------------------------------------------------------------------------
#
# Background: every Tribal runtime path inside the container drops to UID
# 10000 (the ``tribal`` user) via ``s6-setuidgid tribal``. ``docker exec``
...
# fails (PermissionError swallowed) for tribal, so a test that ran as root
...
# helpers below run every probe under ``-u tribal`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: tribal).
