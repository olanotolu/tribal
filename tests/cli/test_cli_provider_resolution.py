...
from tribal_cli import main as tribal_main
...
def test_tribal_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in TribalCLI.__init__")

    monkeypatch.setattr("tribal_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("tribal_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.TribalCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("tribal_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("tribal_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.TribalCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("tribal_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "tribal_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.TribalCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "tribal_cli.nous_subscription.managed_nous_tools_enabled",
    ...
    monkeypatch.setattr(
        "tribal_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "tribal_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "tribal_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("tribal_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("tribal_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("tribal_cli.auth._update_config_for_provider", lambda provider, url: None)

    tribal_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.TribalCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.TribalCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("tribal_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("tribal_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("tribal_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(tribal_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    tribal_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "tribal_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("tribal_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("tribal_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("tribal_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("tribal_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "tribal_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("tribal_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    tribal_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("tribal_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("tribal_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "tribal_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("tribal_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    tribal_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(tribal_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "tribal_cli.config.load_config",
    ...
    monkeypatch.setattr("tribal_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("tribal_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("tribal_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("tribal_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(tribal_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("tribal_cli.auth._login_nous", _fake_login)

    tribal_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="tribal-local",
...
def test_auto_provider_name_localhost():
    from tribal_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from tribal_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "tribal_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("tribal_cli.config.save_config", _save)
