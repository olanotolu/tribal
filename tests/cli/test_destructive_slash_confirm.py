"""Tests for cli.TribalCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full TribalCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        TribalCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        ...
        result = _bound(TribalCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(TribalCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(TribalCLI._get_slash_confirm_display_fragments, self_)()
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert TribalCLI._split_destructive_skip("/reset now") == ("", True)
    assert TribalCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert TribalCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_strips_command_word():
    ...
    assert TribalCLI._split_destructive_skip("/new My title") == ("My title", False)
    assert TribalCLI._split_destructive_skip("/new --yes My title") == ("My title", True)
...
def test_split_destructive_skip_case_insensitive():
    ...
    assert TribalCLI._split_destructive_skip("/new NOW") == ("", True)
    # Substring match must NOT trigger — "Now-Title" is a literal title token.
    assert TribalCLI._split_destructive_skip("/new Now-Title") == ("Now-Title", False)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert TribalCLI._split_destructive_skip(None) == ("", False)
    assert TribalCLI._split_destructive_skip("") == ("", False)
    assert TribalCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    self_._split_destructive_skip = TribalCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_yes_flag_skips_modal():
    ...
    self_._split_destructive_skip = TribalCLI._split_destructive_skip
