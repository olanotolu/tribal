...
class TestExitSummaryResumeHint:
    ...
    profile boundaries — sessions live under `~/.tribal-profiles/<profile>/`,
    ...
    def test_resume_hint_no_profile_flag_on_default(self, capsys):
        ...
        assert "tribal --resume 20260524_000001_abc123" in out
        assert " -p " not in out
    ...
    def test_resume_hint_no_profile_flag_on_custom(self, capsys):
        ...
        with patch("tribal_cli.profiles.get_active_profile_name", return_value="custom"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
        # "custom" is the standard TRIBAL_HOME indicator — no -p needed.
        assert "tribal --resume 20260524_000001_abc123" in out
        assert " -p " not in out

    def test_resume_hint_includes_profile_flag_for_named_profile(self, capsys):
        cli_obj = _make_cli()
        with patch("tribal_cli.profiles.get_active_profile_name", return_value="dev"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
        assert "tribal --resume 20260524_000001_abc123 -p dev" in out

    def test_resume_hint_includes_profile_flag_on_title_hint_too(self, capsys, tmp_path):
        """When a session title is available, the `tribal -c "title"` hint
        ...
        with patch("tribal_cli.profiles.get_active_profile_name", return_value="dev"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
        assert 'tribal -c "My Cool Session" -p dev' in out
        assert "tribal --resume 20260524_000001_abc123 -p dev" in out
    ...
    def test_resume_hint_falls_back_when_profile_lookup_fails(self, capsys):
        ...
        missing during ``tribal update`` mid-flight), fall back to no
        flag rather than crashing the exit summary.
        """
        cli_obj = _make_cli()
        with patch(
            "tribal_cli.profiles.get_active_profile_name",
