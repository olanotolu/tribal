...
default 60s to TRIBAL_API_TIMEOUT (1800s) to avoid premature connection
...
class TestLocalStreamReadTimeout:
    ...
    def test_local_endpoint_bumps_read_timeout(self, base_url):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TRIBAL_STREAM_READ_TIMEOUT", None)
            _base_timeout = float(os.getenv("TRIBAL_API_TIMEOUT", 1800.0))
    ...
    def test_user_override_respected_for_local(self):
        """User sets TRIBAL_STREAM_READ_TIMEOUT -> keep their value even for local."""
        with patch.dict(os.environ, {"TRIBAL_STREAM_READ_TIMEOUT": "300"}, clear=False):
            _base_timeout = float(os.getenv("TRIBAL_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("TRIBAL_STREAM_READ_TIMEOUT", 120.0))
