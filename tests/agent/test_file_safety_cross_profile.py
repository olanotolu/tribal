"""Tests for the cross-Tribal-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Tribal profile's
...
Reference: May 2026 incident — a tribal-security profile session
accidentally edited skills under both ~/.tribal/profiles/tribal-security/skills/
AND ~/.tribal/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Tribal root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_tribal(tmp_path, monkeypatch):
    """Build a fake Tribal layout:
...
def _set_active_home(monkeypatch, tribal_home: Path):
    """Point file_safety._tribal_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_tribal_home_path", lambda: tribal_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "tribal-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_tribal, monkeypatch):
        """If TRIBAL_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_tribal_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_tribal["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_tribal, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_tribal["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "tribal-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_tribal, monkeypatch):
        ...
        assert result["target_profile"] == "tribal-security"

    def test_named_to_named_cross_profile(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_tribal["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_tribal, monkeypatch, area):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_tribal["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_tribal_path_returns_none(self, fake_tribal, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Tribal root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_tribal_config_not_classified_as_cross_profile(self, fake_tribal, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_tribal["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "tribal-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_tribal, monkeypatch):
        _set_active_home(monkeypatch, fake_tribal["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_tribal["default_home"] / "skills" / "foo" / "SKILL.md")
