"""Tests for TRIBAL_HOME credential-file read blocking in file_safety.

Regression for https://github.com/Tribal/tribal/issues/17656 —
``read_file`` was previously only sandboxed against ``TRIBAL_HOME`` itself,
...
for the credential stores while leaving arbitrary ``TRIBAL_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_tribal_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "tribal_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_tribal_home_path", lambda: home)
    return home
...
def test_arbitrary_tribal_home_file_not_blocked(fake_home):
    """Non-credential files inside TRIBAL_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Tribal cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into TRIBAL_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "tribal_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at TRIBAL_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == TRIBAL_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at TRIBAL_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in TRIBAL_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_tribal_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Tribal-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside TRIBAL_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside TRIBAL_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside TRIBAL_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, TRIBAL_HOME = <root>/profiles/<name>, but
    <root>/auth.json must ALSO be blocked — credentials at root are
    inherited by every profile."""
    import agent.file_safety as fs

    root = tmp_path / "tribal"
    profile = root / "profiles" / "coder"
    profile.mkdir(parents=True)
    monkeypatch.setattr(fs, "_tribal_home_path", lambda: profile)
