...
to a generic "Your API key was rejected... run tribal setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "tribal setup".
    assert "tribal auth add nous --type oauth" in source
