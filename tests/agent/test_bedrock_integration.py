...
Note: Tests that import ``tribal_cli.auth`` or ``tribal_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from tribal_cli.models import _PROVIDER_ALIASES
...
class TestProviderLabels:
    ...
    def test_bedrock_label(self):
        from tribal_cli.models import _PROVIDER_LABELS
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("tribal_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("tribal_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("tribal_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("tribal_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("tribal_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("tribal_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into tribal_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from tribal_cli.providers import ALIASES
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "tribal-agent[bedrock]" not in extras["all"]
