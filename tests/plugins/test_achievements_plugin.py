"""Tests for the bundled tribal-achievements dashboard plugin.
...
``plugins/tribal-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the tribal-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.tribal so state/snapshot files don't collide.
    ...
    # swap ``sys.modules['tribal_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from tribal_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for tribal_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['tribal_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("tribal_state")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "tribal_state", fake_module)
