...
(SQLITE_PROTOCOL — typical on NFS/SMB), Tribal must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``TRIBAL_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    tribal_state._set_last_init_error(None)
    yield
    tribal_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    tribal_state._wal_fallback_warned_paths.clear()
    yield
    tribal_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        ...
        with caplog.at_level("WARNING", logger="tribal_state"):
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see tribal_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        tribal_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("tribal_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        tribal_state._set_last_init_error("OperationalError: generic SQLite error")
