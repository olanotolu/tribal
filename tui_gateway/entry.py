...
# tribal modules.  tribal_cli sets TRIBAL_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("TRIBAL_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `TRIBAL_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("TRIBAL_TUI_SIDECAR_URL")
...
# tested; override via ``TRIBAL_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("TRIBAL_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``TRIBAL_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``tribal --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
