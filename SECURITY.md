# Tribal Agent Security Policy

This document describes Tribal Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Tribal/tribal/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Tribal Agent does not operate a bug
bounty program.**
...
- Environment details (`tribal version`, commit SHA, OS, Python
  version).
...
Tribal Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Tribal Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Tribal Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Tribal Agent's documentation
...
Tribal Agent supports two OS-level isolation postures. They address
...
Tribal Agent supports this in two ways:

- **Tribal Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Tribal Agent's in-process heuristics
...
Tribal Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Tribal Agent
itself. Bugs in Tribal Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Tribal Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Tribal Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Tribal Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  tribal-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Tribal Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Tribal Agent. Bugs in Tribal
...
- Tribal Agent includes supply-chain guards for MCP server
