...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateTribal: "Обновить Tribal",
    updatingTribal: "Обновление Tribal…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов Tribal (аналог `tribal plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.tribal/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.tribal/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.tribal/skills/",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки Tribal, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий Tribal. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте Tribal чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только Tribal обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "Tribal сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
