...
export const zh: Translations = {
  ...
  status: {
    ...
    stopped: "已停止",
    updateTribal: "更新 Tribal",
    updatingTribal: "正在更新 Tribal…",
  ...
  pluginsPage: {
    ...
    headline: "发现、安装、启用和更新 Tribal 插件（对齐 `tribal plugins` CLI）。",
    ...
    removeConfirm: "从 ~/.tribal/plugins/ 删除此插件？",
    removeHint: "仅可移除用户安装在 ~/.tribal/plugins 下的插件。",
  ...
  skills: {
    ...
    categories: "分类",
    filters: "筛选",
    noSkills: "未找到技能。技能从 ~/.tribal/skills/ 加载",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "从真实会话历史中获得的 Tribal 可收集徽章。已知尚未达成的成就显示为「已发现」；秘密成就在首次出现匹配行为之前保持隐藏。",
      scan_subtitle:
        "正在扫描 Tribal 会话历史。在历史记录较多时，首次扫描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新扫描",
    },
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就会隐藏其确切触发条件。一旦 Tribal 检测到相关信号，卡片将变为「已发现」并显示其要求。",
      scan_status_header: "扫描状态",
      scan_status_body:
        "Tribal 正在对本地历史进行一次扫描，之后卡片会自动出现。即使这需要几秒钟，也没有卡住。",
